...
def spatial_inputs(tmp_path: Path) -> dict[str, Path]:
    def make_calib(shift: float) -> pd.DataFrame:
        ...
        for sid, (x, y) in enumerate(
            ((113.40 + shift, 22.15 + shift), (113.42 + shift, 22.17 + shift)),
            start=1,
        ):
            rows.append(
                {
    ...
    def make_future(shift: float) -> pd.DataFrame:
        ...
        for sid, (x, y) in enumerate(
            ...
            for step, year, q50 in ((1, 2025, 12.0 + sid), (2, 2026, 13.5 + sid)):
                rows.append(
                    {
                        "sample_idx": sid,
                        "forecast_step": step,
...
def test_plot_spatial_forecasts_main_writes_outputs(
    ...
    mod = pytest.importorskip("geoprior._scripts.plot_spatial_forecasts")

    monkeypatch.setattr(mod, "_load_calib_df", lambda path: pd.read_csv(path))
    monkeypatch.setattr(mod, "_load_future_df", lambda path: pd.read_csv(path))
    target_utils = getattr(mod, "u", None) or getattr(mod, "utils")
    ...
    if hasattr(target_utils, "resolve_fig_out"):
        monkeypatch.setattr(target_utils, "resolve_fig_out", _fig_out)
    ...
    assert (script_test_env["figs_dir"] / "spatial_forecast_case.png").exists()
    assert (script_test_env["figs_dir"] / "spatial_forecast_case.svg").exists()
    hs = script_test_env["tables_dir"] / "spatial_forecast_hotspots.csv"
    assert hs.exists()
    df = pd.read_csv(hs)
...
def test_coord_kind_detects_lonlat_ranges():
    mod = pytest.importorskip("geoprior._scripts.plot_spatial_forecasts")
    df = pd.DataFrame({"coord_x": [113.4, 113.5], "coord_y": [22.1, 22.2]})
