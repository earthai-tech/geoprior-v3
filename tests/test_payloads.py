...
def test_maybe_subsample_keeps_row_aligned_arrays(
    monkeypatch,
):
    payload = {
        "tau": np.arange(5),
    ...
    monkeypatch.setattr(
        mod.np.random,
        "choice",
        lambda n, size, replace: np.array([1, 3]),
    )
...
def test_identifiability_diagnostics_from_payload_returns_expected_blocks():
    ...
    assert (
        abs(out["offsets"]["vs_true"]["delta_K"]["q50"])
        < 1e-12
    )


def test_save_and_load_physics_payload_roundtrip_npz(
    tmp_path: Path,
):
    payload = _payload()
    meta = {"city": "nansha"}
    ...
    saved = mod.save_physics_payload(
        payload, meta, str(path), format="npz", overwrite=True
    )
    ...
    np.testing.assert_allclose(
        loaded["tau_closure"], payload["tau_prior"]
    )


def test_save_and_load_physics_payload_roundtrip_csv(
    tmp_path: Path,
):
    payload = _payload()
    meta = {"city": "zhongshan"}
...
def test_load_physics_payload_adds_compat_aliases(
    tmp_path: Path,
):
    ...
    np.testing.assert_allclose(
        loaded["tau_prior"], loaded["tau_closure"]
    )
