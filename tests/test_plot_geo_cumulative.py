...
@pytest.fixture
def geo_cumulative_inputs(tmp_path: Path) -> dict[str, Path]:
...
def test_plot_geo_cumulative_main_writes_png_and_pdf(
    script_test_env,
    fast_script_figures,
    monkeypatch,
    geo_cumulative_inputs: dict[str, Path],
):
    ...
    mod.plot_geo_cumulative_main(
        [
            "--ns-val",
            str(geo_cumulative_inputs["ns_val"]),
            "--zh-val",
            str(geo_cumulative_inputs["zh_val"]),
            "--ns-future",
            str(geo_cumulative_inputs["ns_future"]),
            "--zh-future",
            str(geo_cumulative_inputs["zh_future"]),
            "--start-year",
            "2020",
            "--year-val",
            "2022",
