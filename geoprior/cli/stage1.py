...
def _build_stage1_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="stage1-preprocess",
        description=(
            "Run GeoPrior Stage-1 preprocessing "
            "using nat.com/config.py."
        ),
    )
    p.add_argument(
        "--config",
...
def run_stage1(
    ...
    *,
    ...
    BUILD_FUTURE_NPZ = bool(
        cfg.get("BUILD_FUTURE_NPZ", False)
    )
    ...
    TRACK_AUX_METRICS = bool(
        cfg.get("TRACK_AUX_METRICS", True)
    )
    ...
    cfg.get("GWL_RAW_COL", None)  # e.g. "GWL_depth_bgs"
    ...
    MV_SCHEDULE_UNIT = (
        str(cfg.get("MV_SCHEDULE_UNIT", "epoch"))
        .strip()
        .lower()
    )
    ...
    def _dataset_to_numpy_pair(
        ...
        x0 = xs[0]
        x_np = {
            k: np.stack([xi[k] for xi in xs], axis=0)
            for k in x0
        }
        y0 = ys[0]
        y_np = {
            k: np.stack([yi[k] for yi in ys], axis=0)
            for k in y0
        }
        return x_np, y_np
    ...
    print(
        f"  NaNs after clean : {df_clean.isna().sum().sum()}"
    )
    ...
    for sp in CENSORING_SPECS or []:
        if sp.get("col") == H_FIELD_COL_NAME:
            eff = H_FIELD_COL_NAME + sp.get(
                "eff_suffix", "_eff"
            )
            if USE_EFFECTIVE_H_FIELD and (
                eff in df_proc.columns
            ):
                H_FIELD_SRC_COL = eff
            break
    ...
    SUBS_SI_COL = (
        f"{SUBSIDENCE_COL}__si"  # settlement in meters
    )
    DEPTH_SI_COL = (
    ...
    if COORD_MODE in ("degrees", "lonlat", "geographic"):
        ...
        try:
            ...
            lon = float(
                df_proc.loc[df_proc.index[0], LON_COL]
            )
            lat = float(
                df_proc.loc[df_proc.index[0], LAT_COL]
            )
        ...
        coord_epsg = int(
            cfg.get("COORD_EPSG", COORD_TARGET_EPSG)
        )
        print(
    ...
    val_groups_csv = os.path.join(
        holdout_dir, "val_groups.csv"
    )
    test_groups_csv = os.path.join(
    ...
    def _pack_targets(targets):
        return {
            "subs_pred": targets["subsidence"].astype(
                np.float32
            ),
    ...
    if coord_shift_pack is not None:
        ...
        scaling_kwargs["coord_shift_meta"] = (
            coord_shift_pack.meta
        )
    ...
    manifest = {
        ...
        "artifacts": {
            ...
            "shapes": {
                ...
                "val_inputs": {
                    k: list(v.shape)
                    for k, v in val_np_x.items()
                },
                "val_targets": {
                    k: list(v.shape)
                    for k, v in val_np_y.items()
                },
            },
        },
        "paths": {
    ...
    manifest["config"]["holdout"] = {
        "strategy": str(
            cfg.get("HOLDOUT_STRATEGY", "random")
        ),
        "seed": int(cfg.get("SPLIT_SEED", 42)),
        "val_frac": float(cfg.get("VAL_FRAC", 0.2)),
        "test_frac": float(cfg.get("TEST_FRAC", 0.1)),
...
def stage1_main(
    ...
    args = _build_stage1_parser().parse_args(argv)
    run_stage1(
