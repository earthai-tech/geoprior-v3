...
def escape_dataframe_elements(
    ...
    if escape_columns and not is_numeric_type(
        df, target="columns"
    ):
        escaped_df.columns = [
            str(col).replace(item_to_escape, escape_char)
            for col in df.columns
        ]

    # Escape index if it is not numeric and escaping is enabled
    if escape_index and not is_numeric_type(
        df, target="index"
    ):
        escaped_df.index = [
            str(idx).replace(item_to_escape, escape_char)
            for idx in df.index
        ]
...
def distribute_column_widths(*dfs, **kws):
    ...
    *dfs : unpacked tuple of pd.DataFrame
    ...
    max_widths = _compute_maximum_widths(
        block_columns, column_widths
    )

    adjusted_widths = _apply_maximum_widths(
        max_widths, block_columns
    )
...
def _assemble_column_blocks(dfs):
    ...
    uniform processing."""
    if any(
        len(df.columns) != len(dfs[0].columns) for df in dfs
    ):
        return None
    return np.array(
        [df.columns.tolist() for df in dfs], dtype=object
    )
...
def _compute_maximum_widths(block_columns, column_widths):
    ...
    for (
        columns
    ) in block_columns.T:  # Transpose to iterate column-wise
        widths = [
            column_widths.get(col, 0) for col in columns
        ]
...
def _apply_maximum_widths(max_widths, block_columns):
    ...
    updated_widths = {}
    for width, columns in zip(
        max_widths, block_columns.T, strict=False
    ):
        for col in columns:
...
def check_dataframe_columns(
    *dfs, error="raise", return_dfs=False
):
    """
    ...
    *dfs : unpacked tuple of pd.DataFrame
    ...
    all_match = all(
        df.columns.equals(first_df_columns)
        for df in valid_dfs
    )
...
def _consolidate_column_widths(
    ...
    if all_columns_match:
        ...
        for cw in columns_widths:
            for col, width in cw.items():
                consolidated_widths[col] = max(
                    consolidated_widths.get(col, 0), width
                )
    else:
...
def _filter_columns_by_list(
    ...
    if missing_columns:
        ...
        if error == "raise":
            ...
            warnings.warn(
                message + " Skipping these columns.",
                stacklevel=2,
            )
...
def get_column_widths_in(
    ...
    *dfs : unpacked tuple of pd.DataFrame
    ...
    """
    # Check if all dataframes have the same columns
    all_columns_match = check_dataframe_columns(
        *dfs, error=error
    )
    ...
    max_index_length = 0
    for df in dfs:
        cw, idx_length = calculate_widths(
    ...
    if return_widths_only:
        consolidated_widths = list(
            consolidated_widths.values()
        )
    ...
    if include_index:
        return (
            ([max_index_length] + consolidated_widths)
            if isinstance(consolidated_widths, list)
            else (max_index_length, consolidated_widths)
        )
...
def get_display_dimensions(
    ...
    *dfs : tuple of pd.DataFrame
    ...
    def get_single_df_metrics(df):
        ...
        auto_rows, auto_cols = propose_layout(
    ...
    df_max_rows, df_max_cols = zip(*metrics, strict=False)
...
def is_numeric_type(df, target="index"):
    ...
    if target not in ["index", "columns"]:
        raise ValueError(
            "target must be 'index' or 'columns'"
        )
...
def extract_truncate_df(
    ...
    indices = re.findall(
        r"^\s*(\d+)", df_string, flags=re.MULTILINE
    )
    ...
    subset_df = get_dataframe_subset(
        truncated_df, indices, columns
    )
...
def insert_ellipsis_to_df(
    sub_df, full_df=None, include_index=True
):
    """
    ...
    if full_df is not None and len(sub_df.columns) < len(
        full_df.columns
    ):
        modified_df.insert(
            mid_col_index, "...", ["..."] * len(modified_df)
        )

    # Determine if ellipsis should be inserted for rows
    if (
        full_df is not None
        and len(sub_df) < len(full_df)
        and include_index
    ):
        ...
        modified_df = pd.concat(
            [top_half, ellipsis_row, bottom_half],
            ignore_index=True,
        )

        # Adjust the index to include ellipsis if required
        new_index = (
            list(sub_df.index[:mid_row_index])
            + ["..."]
            + list(sub_df.index[mid_row_index:])
        )
...
def flex_df_formatter(
    ...
    max_rows, max_cols = resolve_auto_settings(
        max_rows, max_cols
    )
    ...
    if output_format == "html":
        ...
        df = make_format_df(
            df, GOFAST_ESCAPE, apply_to_column=True
        )
...
def select_df_styles(style, df, **kwargs):
    ...
    **kwargs : dict
    ...
    if style == "auto":
        style = (
            "advanced"
            if is_dataframe_long(
                df, return_rows_cols_size=False, **kwargs
            )
            else "base"
        )
    else:
        ...
        for key, regex in style_map.items():
            ...
            valid_styles = ", ".join(
                [
                    regex
                    for patterns in style_map.values()
                    for regex in patterns.split("|")
                ]
            )
            raise ValueError(
                f"Invalid style specified. Choose from: {valid_styles}"
            )
...
def is_dataframe_long(
    ...
    """
    df = validate_data(df)
    auto_rows, auto_cols = propose_layouts(
        df, minimize_cols=minimize_cols
    )
...
def propose_layouts(
    ...
    *dfs : pandas.DataFrame
    ...
    nrows, ncols = zip(*layout_params, strict=False)
...
def df_base_style(
    ...
    max_line_width = max(
        len(line) for line in formatted_df.split("\n")
    )
...
def _robust_df_display(
    ...
    auto_max_index_length, *auto_column_widths = (
        calculate_column_widths(
            lines,
            include_index=True,
    ...
    max_index_length += (
        buffer_space  # Add extra spaces for alignment
    )

    # Format the header line with adjusted widths
    header_parts = lines[0].split()
    header_line_formatted = (
        " " * max_index_length
        + "  ".join(
    ...
    for i, line in enumerate(lines):
        ...
        if len(parts) > 1:
            ...
            formatted_data_part = "  ".join(
                data.rjust(column_widths[idx])
                for idx, data in enumerate(data_part.split())
            )
            if index:
    ...
    table_width = (
        ...
        if table_width == "auto"
            ...
            min(table_width, max_line_width),
            len(header_line_formatted),
        )
    )
...
def df_advanced_style(
    ...
    for i, line in enumerate(lines):
        if (
            i == 0 and header
        ):  # Adjust header line to include vertical bar
            header_parts = line.split()
            header_line_formatted = (
                " " * max_index_length
                + "  ".join(
                    header_part.rjust(column_widths[idx])
                    for idx, header_part in enumerate(
                        header_parts
                    )
                )
            )
            ...
            i == 1 and header
        ):  # Insert sub-line after headers ( reduce extraspace -1)
...
def auto_adjust_dataframe_display(
    df, header=True, index=True, sample_size=100
):
    """
    ...
    sample = df.sample(
        n=min(sample_size, len(df)), random_state=1
    )
    sample_flat = pd.Series(dtype=str)
    if index:
        series_to_concat = [
            sample.index.to_series().astype(str)
        ]
    else:
        series_to_concat = []
    for column in sample.columns:
        series_to_concat.append(sample[column].astype(str))
    sample_flat = pd.concat(
        series_to_concat, ignore_index=True
    )
    avg_entry_width = (
        int(sample_flat.str.len().mean()) + 1
    )  # Plus one for spacing

    # Determine the width used by the index
    index_width = (
        max(len(str(idx)) for idx in df.index) if index else 0
    )
...
def find_best_display_params2(
    *dfs, index=True, header=True, sample_size=100
):
    """
    ...
    *dfs : tuple of DataFrame
    ...
    def resolve_auto(max_rows="auto", max_cols="auto"):
        ...
        final_max_rows = (
            optimal_max_rows
            if max_rows in (None, "auto")
            else max_rows
        )
        final_max_cols = (
            optimal_max_cols
            if max_cols in (None, "auto")
            else max_cols
        )
...
def find_maximum_table_width(
    summary_contents, header_marker="="
):
    """
...
def format_text(
    ...
    while text:
        ...
        if (
            len(text)
            <= effective_max_char_text - buffer_space
            or not key_str
        ):
            ...
            if (
                break_point == -1
            ):  # No spaces found, force break
                break_point = (
                    effective_max_char_text - buffer_space
                )
            # Add the line to formatted_text
            formatted_text += (
                key_str + text[:break_point].rstrip() + "\n"
            )
...
def df_to_custom_dict(df, key_prefix="Row", frame_char="["):
    ...
    key_format = (
        ...
        and df.index.inferred_type == "integer"
        else "{}"
    )

    return {
        key_format.format(
            f"{frame_open}{index}{frame_close}"
            if key_format.startswith(key_prefix)
            else index
        ): " ".join(
...
def calculate_widths(df, max_text_length=50):
    ...
    max_col_widths = {
        col: max(
            len(col),
            max(len(x) for x in formatted_cells[col]),
        )
        for col in df.columns
    }
    max_index_width = max(
        len(str(index)) for index in df.index
    )
    max_col_widths = {
        col: min(width, max_text_length)
...
def format_df(
    ...
    max_col_widths, max_index_width = calculate_widths(
        df, max_text_length
    )

    # Formatting the header
    header = " " * (max_index_width + 4) + "  ".join(
        col.center(max_col_widths[col]) for col in df.columns
    )
    separator = " " * (max_index_width + 1) + "-" * (
        len(header) - (max_index_width + 1)
    )
    ...
    data_rows = [
        ...
        + "  ".join(
            format_cell(
                row[col], max_text_length, max_col_widths[col]
            ).ljust(max_col_widths[col])
            for col in df.columns
        )
        for index, row in df.iterrows()
    ]
...
def is_autofit_needed(
    df, check_rows=True, include_index="auto", **layout_kws
):
    """
...
def autofit_display(
    ...
    **layout_kws : dict, optional
    ...
    df_escaped = extract_truncate_df(
        df, max_rows=nrows, max_cols=ncols
    )
...
def validate_data(data, columns=None, error_mode="raise"):
    ...
    def validate_columns(data_columns, expected_columns):
        if expected_columns is None:
            return [
                f"feature_{i}" for i in range(data_columns)
            ]
        ...
        if len(expected_columns) != data_columns:
            ...
            if error_mode == "raise":
                ...
                warnings.warn(
                    f"{message} Default columns will be used.",
                    UserWarning,
                    stacklevel=2,
                )
                return [
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if data.ndim == 2:
            ...
            raise ValueError(
                "Array with more than two dimensions is not supported."
            )
...
def format_correlations(
    ...
    if len(df.columns) == 1:
        if error_mode == "warn":
            warnings.warn(
                "Cannot compute correlations for a single column.",
                stacklevel=2,
            )
        ...
        return (
            ...
            else "No correlations to display."
        )
    ...
    if precomputed:
        ...
        if numeric_df.empty:
            if error_mode == "warn":
                warnings.warn(
                    "No numeric data found in the DataFrame.",
                    stacklevel=2,
                )
            elif error_mode == "raise":
                raise ValueError(
            ...
            return (
                ""
                if error_mode == "ignore"
                else "No numeric data available."
            )
    ...
    def format_value(value):
        if pd.isna(value):  # Handle NaN for diagonals
            return (
                ""
                if hide_diag
                else ("o" if use_symbols else pd.isna(value))
            )
        if abs(value) < min_corr:
            return (
                str(no_corr_placeholder).ljust(4)
                if use_symbols
                else f"{value:.4f}"
            )
        if use_symbols:
    ...
    if autofit == "auto":
        autofit = is_autofit_needed(
            corr_matrix, minimize_cols=True
        )
    ...
    formatted_df = format_df(
        formatted_corr, autofit=autofit, minimize_cols=True
    )
...
def get_table_width_from(
    ...
    border_lines = [
        line
        for line in formatted_str.splitlines()
        if border_char in line
    ]

    if not border_lines:
        if error == "warn":
            warnings.warn(
                " No border line found in the formatted string.",
                stacklevel=2,
            )
        elif error == "raise":
            raise ValueError(
...
def generate_legend(
    ...
    if custom_markers is not None and not isinstance(
        custom_markers, dict
    ):
        raise TypeError(
...
def get_table_size(
    width="auto", error="warn", return_height=False
):
    """
...
def optimize_col_width(max_cols=4, df=None, min_col_width=10):
    ...
    if df is not None:
        df = validate_data(df)
        if (
            isinstance(min_col_width, str)
            and min_col_width == "auto"
        ):
            min_col_width = min(
    ...
    total_required_width = max_cols * (
        min_col_width + 4
    )  # +4 for space between columns
    ...
    if max_cols > 0:
        ...
        max_col_width = (
            terminal_width - buffer_width
        ) // max_cols
        return max(
            max_col_width, min_col_width
        )  # Ensure at least min_col_width
    else:
        return min_col_width  # Return min_col_width if no columns fit


def get_displayable_columns(
    cols_or_df, /, buffer_space=2, min_col_width=10
):
    """
    ...
    if isinstance(cols_or_df, pd.DataFrame):
        ...
        cols = [
            cols_or_df
        ]  # Treat a single string as a list with one element
    else:
        # Raise an error if the input type is unexpected
        raise TypeError(
            "cols_or_df must be either a list, or a pandas.DataFrame."
        )
    ...
    max_col_width = optimize_col_width(
        max_cols=num_cols, df=df, min_col_width=min_col_width
    )
    ...
    try:
        ...
        available_space_per_column = (
            max_col_width + buffer_space
        )
        # Determine the maximum number of columns that can fit in the terminal
        max_displayable_cols = (
            terminal_width // available_space_per_column
        )
...
def beautify_dict(d, space=4, key=None, max_char=None):
    ...
    for dkey, value in sorted(d.items()):
        ...
        formatted_row = (
            f"{str(dkey):>{max_key_length}}: '{value_str}'"
        )
    ...
    formatted_dict = (
        "{\n"
        + indent
        + inner_join.join(formatted_rows)
        + "\n}"
    )
    ...
    if key:
        ...
        outer_indent = " " * (
            space - 2 + len(key) + max_key_length
        )
        ...
        for i in range(1, len(lines)):
            ...
            if (
                max_char is not None
                and len(lines[i]) > max_char
            ):
...
def format_dict_result(
    ...
    """
    max_key_length = max(
        len(str(key)) for key in dictionary.keys()
    )
    ...
    remainder = (
        f"[Use <{dict_name}.key> to get the full value ...]"
    )
...
def max_column_lengths(
    df, include_index="auto", max_text_char=50
):
    """
    ...
    if include_index == "auto":
        include_index = not df.index.equals(
...
def find_max_display(
    df, include_index=True, buffer_space=2, max_text_char=50
):
    """
    ...
    col_lengths, index_length = max_column_lengths(
        df, include_index, max_text_char
    )

    # Total width calculation
    total_width = sum(
        col_lengths.values()
    ) + buffer_space * len(col_lengths)
    if include_index:
    ...
    if total_width > term_width:
        col_widths = {
            col: length + buffer_space
            for col, length in col_lengths.items()
        }
        display_cols = determine_display_columns(
            col_widths, term_width, buffer_space
        )
    else:
    ...
    max_rows = min(
        len(df), term_height - 1
    )  # 1 row for the header

    return max_rows, max_cols


def determine_display_columns(
    col_widths, max_width, buffer_space
):
    """
    ...
    for col, width in reversed(
        col_items[len(display_cols) :]
    ):
    ...
    display_cols = (
        display_cols[: len(display_cols) // 2]
        + display_cols[-len(display_cols) // 2 :]
    )
...
def rearrange(obj):
    ...
    if isinstance(obj, dict):
        ...
        raise TypeError(
            "Input must be a dictionary, DataFrame, or list."
        )
...
def propose_layout(
    ...
    terminal_width, terminal_height = get_table_size(
        return_height=True
    )
    # terminal_width, terminal_height = get_terminal_size()
    col_lengths, index_length = max_column_lengths(
    ...
    if total_width <= terminal_width:
        ...
        cumulative_width = (
            (index_length + buffer_space)
            if include_index
            else 0
        )
        num_columns = 0
        col_lengths = rearrange(col_lengths)
        for _col, length in col_lengths.items():
    ...
    num_rows = (
        terminal_height - 2
    )  # Subtracting 2 for header row and padding
...
def count_functions(
    ...
    try:
        ...
        ImportError
    ) as e:  # Catch the specific ImportError exception
        raise ImportError(
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            if (
                ...
                include_local
                or not is_local_function(node.parent)
            ):
                functions.append(node.name)
        elif isinstance(node, ast.ClassDef) and include_class:
            if include_private or not node.name.startswith(
                "_"
            ):
