...
from .util import format_dict_result, format_value
...
class Bunch:
    ...
    def _is_series(self, attr):
        ...
        mean = round(attr.mean(), 4)
        return (
            f"Series (minval={minval}, maxval={maxval}, mean={mean},"
    ...
    def _is_dataframe(self, attr):
        ...
        dtypes = (
            "object"
            if len(dtypes_set) > 1
            else list(dtypes_set)[0]
        )
        if "object" not in dtypes_set:
            numeric_cols = attr.select_dtypes(
                include=np.number
            ).columns.tolist()
            ...
            return (
                f"DataFrame (minval={minval}, maxval={maxval}, mean={mean}, "
    ...
    def _is_dict(self, attr):
        ...
        if all(isinstance(v, int | float) for v in values):
            ...
            return (
                f"Dict (minval={minval}, maxval={maxval}, mean={mean}, "
                f"items={len(attr)})"
            )
        else:
...
class XBunch(dict):
    ...
    def __getattr__(self, key):
        ...
        try:
            ...
            raise AttributeError(
                f"'Boxspace' object has no attribute '{key}'"
            )
