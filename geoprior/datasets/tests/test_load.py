...
try:
    ...
    from fusionlab.utils.spatial_utils import (
        spatial_sampling,  # Noqa D:504
    )
    ...
    def fetch_zhongshan_data(*args, **kwargs):
        raise ImportError

    def fetch_nansha_data(*args, **kwargs):
        raise ImportError

    def load_processed_subsidence_data(*args, **kwargs):
        raise ImportError
...
skip_if_zhongshan_missing = pytest.mark.skipif(
    not data_file_exists("zhongshan"),
    reason="Zhongshan data file missing",
)
skip_if_nansha_missing = pytest.mark.skipif(
    not data_file_exists("nansha"),
    reason="Nansha data file missing",
)
...
def test_fetch_data_as_frame(dataset_name):
    ...
    if (
        ...
        and not data_file_exists("zhongshan")
    ) or (
        dataset_name == "nansha"
        and not data_file_exists("nansha")
    ):
    ...
    assert (
        len(df) > 1000
    )  # Check it loaded a reasonable amount of data
...
def test_fetch_data_sampling(dataset_name):
    ...
    with pytest.warns(
        UserWarning,
        match="Requested n_samples .* larger than",
    ):
        df_full = fetch_func(
            as_frame=True,
            n_samples=5000,
    ...
    assert (
        len(df_full) > 1000
    )  # Should be full dataset size (~2000)
    ...
    with pytest.raises(
        ValueError
    ):  # Or ValueError depending on validation
...
def test_fetch_data_includes(dataset_name):
    ...
    df_no_coords = fetch_func(
        as_frame=True, include_coords=False, verbose=False
    )
    assert "longitude" not in df_no_coords.columns
    assert "latitude" not in df_no_coords.columns
    assert (
        "subsidence" in df_no_coords.columns
    )  # Target included by default

    # Test excluding target
    df_no_target = fetch_func(
        as_frame=True, include_target=False, verbose=False
    )
...
def test_load_processed_frame_bunch(dataset_name, as_frame):
    ...
    try:
        ...
        pytest.fail(
            f"load_processed failed (as_frame={as_frame}, "
    ...
    if as_frame:
        ...
        assert len(result) > 0
        assert (
            "geology" not in result.columns
        )  # Should be encoded/dropped
        assert any(
            c.startswith("geology_") for c in result.columns
        )
        assert (
            "subsidence" in result.columns
    ...
    print(
        f"Load Processed OK: dataset={dataset_name}, as_frame={as_frame}"
    )
...
def test_load_processed_sequences(dataset_name):
    ...
    try:
        ...
        pytest.fail(
            f"load_processed failed (return_sequences=True, "
    ...
    assert N > 0
    assert (
        s.shape[0] == d.shape[0] == f.shape[0] == t.shape[0]
    )
    assert d.shape[1] == T
    assert (
        f.shape[1] == T + H
    )  # reshape_xtft_data output for future; H+T spanned
    assert t.shape[1] == H
...
def test_load_processed_cache(dataset_name, tmp_path):
    ...
    proc_cache_file = (
        cache_dir / f"{dataset_name}_processed{suffix}.joblib"
    )
    ...
    with pytest.warns(
        None
    ) as record:  # Check no reprocessing warnings
        df_loaded = load_processed_subsidence_data(
            ...
            as_frame=True,
            save_processed_frame=False,
            use_processed_cache=True,  # Use cache
            cache_suffix=suffix,
            verbose=False,
            download_if_missing=False,
        )
    assert (
        len(record) == 0
    )  # Should not warn about reprocessing
    ...
    seq_cache_file = cache_dir / (
        f"{dataset_name}_sequences_T{T}_H{H}{suffix}.joblib"
    )
    ...
    with pytest.warns(None) as record_seq:
        seq_loaded = load_processed_subsidence_data(
            ...
            forecast_horizon=H,
            save_sequences=False,
            use_sequence_cache=True,  # Use cache
            cache_suffix=suffix,
            verbose=False,
            download_if_missing=False,
        )
    ...
    assert (
        isinstance(seq_loaded, tuple) and len(seq_loaded) == 4
    )
