...
pytestmark = pytest.mark.skipif(
    ...
    reason="fusionlab-learn is not installed",
)
...
def mock_data_home(tmp_path):
    ...
    expected_filename = (
        _ZHONGSHAN_METADATA.file
    )  # e.g., 'zhongshan_2000.csv'
    ...
    dummy_dict["geology"] = np.random.choice(
        ["Clay", "Sand", "Gravel"], 200
    )
...
def test_preprocessing_flags(mock_data_home, encode, scale):
    ...
    if encode:
        assert (
            "geology" not in df.columns
            and "geology_Clay" in df.columns
        )
    else:
        assert (
            "geology" in df.columns
            and "geology_Clay" not in df.columns
        )
    ...
    if scale:
        ...
        assert (
            df["rainfall_mm"].min() >= 0
            and df["rainfall_mm"].max() <= 1
        )
    else:
...
def test_caching(tmp_path):
    ...
    expected_cache_file = (
        tmp_path / "zhongshan_zhongshan_2000_processed.joblib"
    )
...
if __name__ == "__main__":  # pragma : no cover
    pytest.main(
        [__file__, "--maxfail=1 ", "--disable-warnings", "-q"]
    )
