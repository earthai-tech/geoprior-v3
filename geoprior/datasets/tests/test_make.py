...
@pytest.mark.parametrize(
    "anomaly_type", ["spike", "level_shift"]
)
def test_make_anomaly_data_types(anomaly_type):
    ...
    try:
        ...
        pytest.fail(
            f"make_anomaly_data failed for type='{anomaly_type}'. Error: {e}"
        )
...
def test_make_anomaly_data_errors():
    ...
    with pytest.raises(
        ValueError,
        match="Currently only supports n_features=1",
    ):
        make_anomaly_data(n_features=2)
    with pytest.raises(
        ValueError,
        match="'anomaly_fraction' must be between 0 and 1",
    ):
        make_anomaly_data(anomaly_fraction=1.1)
    with pytest.raises(
        ValueError,
        match="anomaly_type must be 'spike' or 'level_shift'",
    ):
...
def test_make_trend_seasonal_trend_order(trend_order):
    ...
    try:
        ...
        pytest.fail(
            f"make_trend_seasonal_data failed for trend_order={trend_order}. Error: {e}"
        )
...
def test_make_trend_seasonal_errors():
    ...
    with pytest.raises(
        ValueError, match="Lengths of 'seasonal_periods' and"
    ):
        make_trend_seasonal_data(
            seasonal_periods=[7], seasonal_amplitudes=[5, 10]
        )
    with pytest.raises(
        ValueError, match="Length of 'trend_coeffs'"
    ):
        make_trend_seasonal_data(
            trend_order=1, trend_coeffs=[10, 0.1, 0.01]
        )
    with pytest.raises(
        ValueError, match="'trend_order' must be >= 0"
    ):
...
def test_make_multivariate_target_structure():
    ...
    assert all(
        c in bunch.frame.columns for c in bunch.target_names
    )
