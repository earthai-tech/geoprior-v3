...
class NNBackend(BaseClass):
    ...
    **Supported Methods**:
    ...
    def __init__(
        self, backend="auto", init=True, verbose=False
    ):
        """
    ...
    def _initialize_backend(self, backend):
        ...
        if backend == "auto":
            try:
                ...
                if self.verbose:
                    print(
                        "TensorFlow backend selected automatically."
                    )
            except ImportError:
                try:
            ...
            raise ValueError(
                "Invalid backend specified. Choose either"
    ...
    def dense_layer(self, *args, **kwargs):
        ...
        **kwargs : keyword arguments
        ...
        if self.backend == "tensorflow":
            return self.nn_module.layers.Dense(
                *args, **kwargs
            )
    ...
    def loss_function(self, name="mse"):
        ...
        if self.backend == "tensorflow":
            ...
            if loss_name:
                return getattr(
                    self.nn_module.losses, loss_name
                )()
            else:
                raise ValueError(
        ...
        # PyTorch Loss Functions
            ...
            loss_function = pytorch_loss_functions.get(
                name.lower()
            )
            if loss_function:
    ...
    def optimizer(self, name="adam", model=None, **kwargs):
        ...
        if self.backend == "tensorflow":
            ...
            if optimizer_name:
                return getattr(
                    self.nn_module.optimizers, optimizer_name
                )(**kwargs)
            else:
                raise ValueError(
                    f"Optimizer '{name}' is not supported in TensorFlow."
                )
        ...
        # PyTorch Optimizers
            ...
            optimizer_class = pytorch_optimizers.get(
                name.lower()
            )
            if optimizer_class:
                ...
                raise ValueError(
                    f"Optimizer '{name}' is not supported in PyTorch."
                )
    ...
    def activation(self, name="relu"):
        ...
        # PyTorch Activations
            ...
            pytorch_activations = {
                ...
                "softmax": nn.Softmax(
                    dim=1
                ),  # Softmax requires dim argument in PyTorch
            ...
            activation_fn = pytorch_activations.get(
                name.lower()
            )
            if activation_fn:
                ...
                raise ValueError(
                    f"Activation '{name}' is not supported in PyTorch."
                )
    ...
    def train_step(self, x, y):
        ...
        if self.backend == "tensorflow":
            ...
            return (
                loss_value.numpy()
            )  # Return loss as float for consistency
        elif self.backend == "pytorch":
    ...
    def early_stopping_callback(self, patience=5):
        ...
        if self.backend == "tensorflow":
            ...
            raise RuntimeError(
                "Early stopping is only available in TensorFlow."
            )
    ...
    def log_training(self, log_dir="./logs"):
        ...
        if self.backend == "tensorflow":
            ...
            raise RuntimeError(
                "TensorBoard logging is only supported in TensorFlow."
            )

    def learning_rate_scheduler(
        self, schedule_type="exponential", **kwargs
    ):
        """
    ...
    def checkpoint_callback(
        self, filepath, monitor="val_loss"
    ):
        """
    ...
    def summary(self, input_shape=None):
        ...
        if self.backend == "tensorflow":
            ...
            if input_shape is None:
                raise RuntimeError(
                    "Input shape must be provided for PyTorch models."
                )
            try:
...
if __name__ == "__main__":
    ...
    early_stopping = nn_backend.early_stopping_callback(
        patience=3
    )
    tensorboard_logging = nn_backend.log_training(
        log_dir="./logs"
    )
