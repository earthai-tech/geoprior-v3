...
class NumpyBackend(BaseBackend):
    ...
    def __init__(self):
        ...
        self._np = (
            np  # Directly use the imported NumPy module
        )
    ...
    def random_uniform(self, low=0.0, high=1.0, size=None):
        ...
        """
        return self._np.random.uniform(
    ...
    def to_datetime(self, a, format=None):
        ...
        if format:
            return self._np.array(
                [
                    self._np.datetime64(
                        datetime.strptime(str(x), format)
                    )
                    for x in a
                ]
            )
