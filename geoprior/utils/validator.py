...
def check_donut_inputs(
    ...
    :func:`~geoprior.core.checks.check_numeric_dtype` for numeric 
    type validation.
    ...
    ops = parameter_validator(
        "mode",
        target_strs={"check", "build", "make"},
        error_msg=f"Invalid mode {ops!r}. Use 'check' or 'build'.",
    )(ops)

    value_name = (
        "Value"
        if value_name in ("auto", None)
        else value_name
    )  # use in build mode
    ...
    if ops == "check":
        ...
        if data is not None and isinstance(
            ...
            try:
                ...
                new_df = pd.DataFrame(
                    {value_name: values}, index=df.index
                )
            except:
                # otherwise return df
                return df
        ...
        # Case 2: Data is None; build DataFrame from values and labels.
            ...
            if labels_as_index:
                df = pd.DataFrame(
                    {value_name: values}, index=labels
                )
            else:
                df = pd.DataFrame(
                    {"Label": labels, value_name: values}
                )
            return df


def validate_strategy(
    strategy: str | dict[str, str] | None = None,
    error: str = "raise",
    ops: str = "validate",
    ...
    **kwargs
    ...
    if ops == "passthrough":
        if isinstance(strategy, dict):
            ...
            return (
                parse_strategy_str(strategy)
                if strategy
                else default_strategy.copy()
            )
        ...
        if isinstance(strategy, dict):
            ...
            try:
                ...
                if error == "raise":
                    ...
                    warnings.warn(str(ve), stacklevel=2)
...
def has_methods(
    ...
    if not isinstance(methods, list) or not all(
        ...
        raise TypeError(
            "'methods' should be a list of method name strings."
        )
    ...
    for model in models:
        ...
        for method in methods:
            if not hasattr(model, method) or not callable(
...
def check_is_runned(
    ...
    if hasattr(estimator, "__gofast_is_runned__"):
        ...
        is_runned = all_or_any(
            [
                getattr(estimator, attr, False)
...
def check_has_run_method(
    estimator, msg=None, method_name="run"
):
    """
    ...
    if isinstance(
        getattr(estimator.__class__, method_name, None),
    ...
    ) and not hasattr(method, "__self__"):
...
def validate_batch_size(
    ...
    batch_size = validate_positive_integer(
        batch_size,
        "Batch size",
        msg=(
...
def validate_sets(
    ...
    if mode == "base":
        ...
        if not allow_empty and not data:
            raise ValueError(
                "Data validation failed: empty set is not allowed"
            )
        if element_type is not None and any(
            not isinstance(el, element_type) for el in data
        ):
            raise ValueError(
                "Data validation failed: all elements must"
        ...
        if any(
            not isinstance(k, key_type) for k in data.keys()
        ):
            raise ValueError(
        ...
        if any(not isinstance(v, set) for v in data.values()):
            raise ValueError(
                "Data validation failed: expected all values to be sets"
            )
        if element_type is not None and any(
        ...
        raise ValueError(
            "Mode must be either 'base' or 'deep'."
        )
...
def validate_scores(
    ...
    if isinstance(scores, list):
        if not accept_multi_output:
            raise ValueError(
                "Multi-output scores provided but not accepted."
            )
        if true_labels is not None and len(scores) != len(
            true_labels
        ):
            raise ValueError(
                "Mismatch in the number of outputs between"
                " scores and true_labels."
            )
        if any(
            not _is_probability_distribution(score, mode=mode)
            for score in scores
        ):
            raise ValueError(
                "Each set of scores must be a valid"
                " probability distribution."
            )
    else:
        if not _is_probability_distribution(
            scores, mode=mode
        ):
            raise ValueError(
                "Scores must be a valid probability distribution."
            )
        if true_labels is not None:
            if accept_multi_output and not isinstance(
                true_labels, list
            ):
                raise ValueError(
                    "Expected multi-output for true_labels"
                    " but got a single output."
                )
            if not accept_multi_output and isinstance(
                true_labels, list
            ):
                raise ValueError(
                    "Non-multi-output scores with multi-output"
                    " true_labels."
                )
...
def _is_probability_distribution(
    ...
    Examples
    --------
    >>> from geoprior.utils.validator import _is_probability_distribution
    >>> y = np.array([0.3, 0.7])
    ...
    if mode == "strict":
        ...
        is_valid = np.all(np.sum(y, axis=-1) <= 1) and np.all(
            y >= 0
        )
        mode_status = (
        ...
        raise ValueError(
            f"Invalid validation mode: '{mode}'. Valid modes"
    ...
    if not is_valid:
        if error == "raise":
            raise ValueError(
                f"Input array does not meet the {mode} mode "
...
def validate_square_matrix(
    data, align=False, align_mode="auto", message=""
):
    """
    ...
    if not is_square_matrix(data):
        raise ValueError(
            f"Input must be a square matrix. {message}"
        )
    if align:
        data = validate_comparison_data(
            data, alignment=align_mode
        )
    return data
...
def is_square_matrix(data, data_type=None):
    ...
    if data_type not in ["array", "dataframe"]:
        raise ValueError(
            "data_type must be either 'array' or 'dataframe'"
        )
    ...
    if data_type == "array":
        ...
        raise TypeError(
            f"Unsupported or mismatched data type: {data_type}"
        )
...
def validate_multiclass_target(
    ...
    if not accept_multioutput and y.ndim > 1:
        raise ValueError(
            "Target array must be one-dimensional unless"
            " multi-output is accepted."
        )

    # Validate that all elements are non-negative integers
    if not (
        np.issubdtype(y_eval.dtype, np.integer)
        and np.all(y_eval >= 0)
    ):
        raise ValueError(
            "All elements in the target array must be non-negative integers."
    ...
    if unique_classes.size < 2:
        raise ValueError(
            "Target array must contain at least two distinct classes."
        )
...
def validate_sample_weights(weights, y, normalize=False):
    ...
    """
    if isinstance(
        weights, int | float | np.integer | np.floating
    ):
        weights = np.full_like(
            y, fill_value=weights, dtype=np.float
        )
    ...
    if weights.ndim != 1:
        raise ValueError(
            "Sample weights must be one-dimensional."
        )
    ...
    if weights.size != y.size:
        raise ValueError(
            "Length of sample weights must match length of y."
        )

    weights = normalize_array(
        weights, normalize=normalize, method="sum"
    )
...
def validate_weights(
    ...
    try:
        ...
        raise ValueError(
            "Weights must be provided in a format that can be"
    ...
    if weights_array.ndim not in allowed_dims:
        raise ValueError(
            f"Weights must have dimensions in {allowed_dims}."
        )
    ...
    if min_value is None:
        if np.any(weights_array < 0):
            raise ValueError(
                "Weights must be non-negative unless 'min_value'"
                " is explicitly set to allow negative values."
            )
        min_value = 0.0
    if np.any(weights_array < min_value) or (
        max_value is not None
        and np.any(weights_array > max_value)
    ):
        raise ValueError(
            f"Weights must be between {min_value} and"
    ...
    if normalize:
        if np.sum(weights_array) == 0:
            raise ValueError(
                "Cannot normalize weights because their sum is zero."
            )
...
def is_normalized(arr, method="sum"):
    ...
    if method == "01":
        ...
        return (
            np.all((arr >= 0) & (arr <= 1))
            and np.isclose(np.min(arr), 0)
            and np.isclose(np.max(arr), 1)
        )
    elif method == "zscore":
...
def normalize_array(arr, normalize="auto", method="01"):
    ...
    normalize = parameter_validator(
        "normalize", target_strs={True, False, "auto"}
    )(normalize)
    ...
    if normalize:
        if method == "01":
            ...
            if min_val == max_val:
                raise ValueError(
                    "Normalization impossible with zero variance."
                )
            ...
            if std == 0:
                raise ValueError(
                    "Standardization impossible with zero variance."
                )
            arr = (arr - mean) / std
        elif method == "sum":
            total = np.sum(arr)
            if total == 0:
                raise ValueError(
                    "Normalization by sum impossible with zero sum."
                )
            arr = arr / total
...
def is_binary_class(y, accept_multioutput=False):
    ...
    if not accept_multioutput:
        ...
        return len(unique_values) == 2 and np.all(
...
def handle_zero_division(
    ...
    if np.any(zeros_mask):
        if zero_division == "warn":
            warnings.warn(
                f"Encountered zero in y_true, which may lead to"
            ...
            raise ValueError(
                f"Encountered zero in y_true, leading to division"
...
def convert_to_numeric(
    value, preserve_integers=True, context_description="Data"
):
    """
    ...
    Examples
    --------
    >>> from geoprior.utils.validator import convert_to_numeric
    ...
    try:
        ...
        raise ValueError(
            f"{context_description} expected numeric values,"
...
def validate_performance_data(
    ...
    def actual_validate_performance_data(data):
        ...
        if isinstance(data, dict):
            ...
            raise ValueError(
                "Input data must be either a dictionary or a DataFrame."
            )
    ...
    if model_performance_data is not None and callable(
        ...
        def wrapper(*args, **kwargs):
            data = args[0]
            validated_data = actual_validate_performance_data(
                data
            )
        ...
        return actual_validate_performance_data(
            model_performance_data
        )
...
def validate_sequences(
    ...
    ---------
    >>> import numpy as np
    >>> from geoprior.utils.validator import validate_sequences
    ...
    try:
        ...
        raise TypeError(
            f"The sequences input must be a numpy.ndarray. {e}"
        )

    # Check if the sequences array is 3D
    if len(sequences.shape) != 3:
        raise ValueError(
            "The sequences array must have 3 dimensions: "
            "(batch_size, sequence_length, num_features)."
        )
    ...
    if check_shape:
        ...
        if (
            ...
            and current_sequence_length != sequence_length
        ):
            raise ValueError(
        ...
        if (
            ...
            and current_n_features != n_features
        ):
            raise ValueError(
    ...
    if batch_size is not None or sequence_length is not None:
        sequences = sequences[
            :batch_size, :sequence_length, :current_n_features
        ]
...
def validate_comparison_data(df, alignment="auto"):
    ...
    if not isinstance(df, pd.DataFrame):
        raise TypeError(
            f"Performance data expects a DataFrame; got {type(df).__name__!r}"
        )
    # Check if DataFrame is square
    if df.shape[0] != df.shape[1]:
        raise ValueError(
            "DataFrame must be square (equal number of rows and columns)."
        )
...
def validate_data_types(
    ...
    expected_type = parameter_validator(
        "expected_type",
        target_strs={"numeric", "categoric", "both"},
    )(expected_type)
    ...
    if nan_policy == "raise" and data.isnull().any().any():
        ...
        and data.isnull().any().any()
    ):
        warnings.warn(
            "NaN values found in the data, but processing will continue.",
            stacklevel=2,
        )
    ...
    def _handle_categoric(data, return_data):
        is_categoric = data.apply(
            lambda x: isinstance(x, str | pd.CategoricalDtype)
        )
        if not is_categoric.all():
            if error == "raise":
                raise TypeError(
                    "Mixed types detected with unexpected numeric data."
                )
            elif error == "warn":
                warnings.warn(
                    "Expected categoric types but found numeric data.",
                    stacklevel=2,
                )
                if return_data:
    ...
    for column in data.columns:
        ...
        if expected_type == "numeric":
            ...
            raise ValueError(
                "Unsupported expected_type provided. Choose"
...
def ensure_2d(X, output_format="auto"):
    ...
    if isinstance(X, np.ndarray):
        ...
        if (
            X.shape[1] == 0
        ):  # Implies an empty DataFrame or misshapen
            X = X.values.reshape(
                -1, 1
            )  # reshape and handle as array
            is_dataframe = False
    else:
        X = np.array(
            X
        )  # Convert other types like lists to np.array
        if X.ndim == 1:
            X = X[:, np.newaxis]
...
def parameter_validator(
    ...
    **kws: dict,
       Keyword arguments passed to :func:`geoprior.core.utils.normalize_string`.
    Returns
    -------
    function
    ...
    - The function leverages a custom utility function `normalize_string`
      from a module named `geoprior.core.utils`. This utility is assumed to handle
...
def validate_distribution(
    ...
    if elements is not None:
        if isinstance(elements, list | tuple | np.ndarray):
            ...
            elements, float | int | np.integer | np.floating
        ):
            distributed_elements = int(elements)
        else:
            raise ValueError(
                "'elements' must be an integer or a list of strings."
            )
    ...
    if str(distribution).lower() == "auto":
        if distributed_elements is None:
            raise ValueError(
                "'distributed_elements' must be specified when"
        ...
        if (
            distributed_elements is not None
            and len(distribution) != distributed_elements
        ):
            raise ValueError(
        ...
        for value in distribution:
            if not isinstance(value, int | float):
                raise ValueError(
                    "All distribution values must be numeric."
                )
            validated_distribution.append(float(value))

        # Check if the distribution is normalized
        if check_normalization and not np.isclose(
            sum(validated_distribution), 1
        ):
            raise ValueError(
                "The sum of the distribution values must be equal to 1."
            )
    ...
    if kind == "probs":
        _is_probability_distribution(
            distribution, mode="strict", error="raise"
        )
...
def validate_length_range(
    length_range, sorted_values=True, param_name=None
):
    """
    ...
    if (
        not isinstance(length_range, list | tuple)
        or len(length_range) != 2
    ):
        raise ValueError(
            f"{param_name} must be a tuple with two elements."
        )
    ...
    if not all(
        ...
        raise ValueError(
            f"Both elements in {param_name} must be numeric."
        )
...
def contains_nested_objects(
    lst, strict=False, allowed_types=None
):
    """
...
def validate_nan_policy(
    nan_policy, *arrays, sample_weights=None
):
    """
    ...
    *arrays : array-like
    ...
    if nan_policy == "omit":
        ...
        not_nan_mask = ~np.isnan(np.column_stack(arrays)).any(
            axis=1
        )
        ...
        arrays = tuple(
            array[not_nan_mask] for array in arrays
        )
        if sample_weights is not None:
        ...
        if any(np.isnan(array).any() for array in arrays) or (
            ...
            and np.isnan(sample_weights).any()
        ):
            raise ValueError(
                "Input values contain NaNs and nan_policy is 'raise'."
            )
...
def validate_fit_weights(
    y, sample_weight=None, weighted_y=False
):
    """
...
def validate_multioutput(value, extra=""):
    ...
    if value_lower == "average_uniform":
        value_lower = "uniform_average"
    if value_lower in ["raise", "warn"]:
        warn_msg = (
            "The `multioutput` parameter is not applicable"
            + extra
...
def check_epsilon(
    ...
    if str(eps).lower() == "auto" and y_arrays:
        ...
        try:
            ...
            raise ValueError(
                f"Epsilon must be 'auto' or convertible to float. Got '{eps}'"
            )
...
def check_classification_targets(
    *y, target_type="numeric", strategy="auto", verbose=False
):
    """
    ...
    *y : array-like
    ...
    """
    validated_targets = [
        _check_y(target, strategy=strategy) for target in y
    ]
    ...
    if target_type == "numeric":
        ...
        for i, target in enumerate(validated_targets):
            if all(
                ...
                try:
                    ...
                    validated_targets[i] = target.astype(
                        np.int64
                    )
                except ValueError as e:
                    raise ValueError(
                        f"Error converting target array at index {i} to integers. "
                ...
                raise ValueError(
                    f"Target array at index {i} contains non-numeric values, "
        ...
        raise ValueError(
            f"Unsupported target_type '{target_type}'. Use 'numeric' or 'object'."
        )
...
def _check_y(y, strategy="auto"):
    ...
    if y.dtype.kind in numeric_types:
        numeric_y = y.astype(
            float, casting="safe"
        )  # Safely cast numeric types to float
        if not np.all(np.isfinite(numeric_y)):
            raise ValueError(
                "Numeric target values contain NaN or infinite numbers,"
        ...
        if any(el is None or el is np.nan for el in y):
            raise ValueError(
                "Non-numeric target values contain None or NaN,"
                " not suitable for classification."
            )
    unique_values = np.unique(y)
    ...
    if strategy != "auto":
        ...
        if target_type == "continuous":
            raise ValueError(
                "Continuous data not suitable for classification"
                " without explicit binning."
            )
        elif target_type == "multilabel-indicator":
            raise ValueError(
                "Multilabel-indicator format detected,"
        ...
        if unique_values.shape[0] > np.sqrt(len(y)):
            raise ValueError(
                "Automatic strategy detected too many unique values"
    ...
    if y.dtype.kind not in numeric_types:
        if not all(
            isinstance(val, str | bool | int)
            for val in unique_values
        ):
            raise ValueError(
                "Target values must be categorical, numeric,"
...
def validate_dates(
    ...
    def parse_year_input(year_input):
        ...
        raise TypeError(
            f"Invalid input '{year_input}'."
            " Expected format: YYYY or YYYY-MM-DD."
        )

    start_date, end_date = map(
        parse_year_input, [start_date, end_date]
    )
    ...
    for year in (start_date.year, end_date.year):
        if not 1900 <= year <= current_year:
            raise ValueError(
                f"Year {year} is out of the valid"
...
def validate_positive_integer(
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            raise ValueError(
                f"Value {value} is not convertible to an integer."
            )
    ...
    if not isinstance(
        ...
        msg = (
            msg
            or f"{variable_name} must be an integer or float. Got {value}"
        )
        raise ValueError(msg)
    ...
    if isinstance(value, float):
        if round_float == "ceil":
            ...
            raise ValueError(
                f"Invalid rounding method '{round_float}'."
    ...
    if value < min_value:
        ...
        raise ValueError(
            f"{variable_name} must be {condition}, got {value}."
        )
...
def validate_and_adjust_ranges(**kwargs):
    ...
    **kwargs : dict
    ...
    for range_name, range_tuple in kwargs.items():
        if (
            not isinstance(range_tuple, tuple)
            or len(range_tuple) != 2
        ):
            raise ValueError(
                f"{range_name} must be a tuple of two values."
            )

        if not all(
            isinstance(value, int | float)
            for value in range_tuple
        ):
            raise ValueError(
                f"{range_name} must contain numerical values."
            )
...
def recheck_data_types(
    ...
    if (
        return_as_numpy is True
    ):  # Explicitly set to True since "auto" is True
...
def is_time_series(data, time_col, check_time_interval=False):
    ...
    if not pd.api.types.is_datetime64_any_dtype(
        ...
        try:
            ...
            print(
                f"Column '{time_col}' does not contain datetime objects."
            )
            return False
    ...
    if check_time_interval:
        ...
        intervals = (
            pd.to_datetime(data[time_col]).diff().dropna()
        )
...
def check_is_fitted2(estimator, attributes, *, msg=None):
    ...
    if not hasattr(estimator, "fit"):
        raise TypeError(
            f"{estimator} is not an estimator instance."
        )
    ...
    fitted = all(
        [hasattr(estimator, attr) for attr in attributes]
    )
    ...
    if not fitted:
        if msg is None:
            ...
            msg = (
                f"This {cls_name} instance is not fitted yet. Call 'fit' with appropriate "
...
def assert_xy_in(
    ...
    **kws,
    ...
    if data is not None:
        if not hasattr(data, "__array__") and not hasattr(
            data, "columns"
        ):
            raise TypeError(
    ...
    if (
        ...
        raise TypeError(
            "Data cannot be None when x and y have string"
            " arguments."
        )
    if (x is None or y is None) and data is None:
        raise TypeError(
            "Missing x and y. NoneType not supported."
        )

    if isinstance(x, str):
        exist_features(data, x)
        x = data[x]
    if isinstance(y, str):
        exist_features(data, y)
        y = data[y]
    ...
    if xy_numeric:
        if not _is_numeric_dtype(
            ...
            raise ValueError(
                "x and y must be a numeric array."
            )
...
def _validate_input(ignore: str, x, y, _is_arraylike_1d):
    ...
    check = validation_checks.get(
        ignore, validation_checks["both"]
    )
    if not check():
        if ignore in ["x", "y"]:
            raise ValueError(
                f"Expected '{'y' if ignore == 'x' else 'x'}' to be"
                " a one-dimensional array-like structure."
            )
        else:
            raise ValueError(
                "Expected both 'x' and 'y' to be one-dimensional "
...
def validate_numeric(
    ...
    if isinstance(value, np.ndarray):
        if value.size != 1:
            raise ValueError(
                "Numpy array must contain exactly one element."
            )
        value = value.item()
    ...
    if isinstance(value, str):
        try:
            ...
            raise ValueError(
                f"Value '{value}' is not a valid numeric string."
            )

    # Check if the value is numeric
    if not isinstance(value, int | float):
        raise ValueError(
            f"Value '{value}' is not a numeric type."
        )
    ...
    if not allow_negative and value < 0:
        raise ValueError(
            f"Negative values are not allowed: {value}"
        )
...
def is_array_like(obj, numpy_check=False):
    ...
    if numpy_check:
        ...
        return isinstance(
            obj, np.ndarray | list | tuple | np.generic
        )
...
def _is_numeric_dtype(o, to_array=False):
    ...
    if not hasattr(o, "__array__"):
        raise ValueError(
            f"Expect type array, got: {type(o).__name__!r}"
        )
...
def _check_consistency_size(ar1, ar2, error="raise"):
    ...
    if error == "raise":
        ...
        assert len(ar1) == len(ar2), msg.format(
            len(ar1), len(ar2)
        )
...
def _is_buildin(o, mode="soft"):
    ...
    :param module:
    """
    assert mode in {"strict", "soft"}, (
        f"Unsupports mode {mode!r}, expects 'strict'or 'soft'"
    )

    return (
        (
            isinstance(o, types.BuiltinFunctionType)
            and inspect.isbuiltin(o)
        )
        if mode == "strict"
        else type(o).__module__ == "builtins"
    )
...
def _check_array_in(obj, arr_name):
    ...
    message = (
        f"Unable to find the name {arr_name!r}"
    ...
    if hasattr(X, "shape"):
        if (
            not hasattr(X.shape, "__len__")
            or len(X.shape) <= 1
        ):
            warnings.warn(
                f"A singleton array {X!r} cannot be considered a valid collection.",
                stacklevel=2,
            )
    ...
    * will issue a warning when passed as a positional argument.
    ...
    def _inner_deprecate_positional_args(f):
        ...
        def inner_f(*args, **kwargs):
            ...
            kwargs.update(
                zip(sig.parameters, args, strict=False)
            )
            return f(**kwargs)
...
def to_dtype_str(arr, return_values=False):
    ...
    if not hasattr(arr, "__array__"):
        raise TypeError(
            f"Expects an array, got: {type(arr).__name__!r}"
        )
    if return_values:
...
def _is_arraylike_1d(x):
    ...
    return _is_arraylike_not_scalar(x) and (
        len(x.shape) < 2
        or (len(x.shape) == 2 and x.shape[1] == 1)
    )
...
def check_consistent_length(*arrays):
    ...
    *arrays : list or tuple of input objects.
    ...
    lengths = [
        _num_samples(X) for X in arrays if X is not None
    ]
    uniques = np.unique(lengths)
    if len(uniques) > 1:
        raise ValueError(
    ...
    *,
    ...
    if not isinstance(x, target_type):
        if isinstance(target_type, tuple):
            types_str = ", ".join(
                type_name(t) for t in target_type
            )
    ...
    if min_val is not None and comparison_operator(
        x, min_val
    ):
        raise ValueError(
            f"{name} == {x}, must be"
    ...
    if max_val is not None and comparison_operator(
        x, max_val
    ):
        raise ValueError(
            f"{name} == {x}, must be"
...
def _get_feature_names(X):
    ...
    types = sorted(
        t.__qualname__
        for t in set(type(v) for v in feature_names)
    )
...
def check_is_fitted(
    ...
    if isclass(estimator):
        raise TypeError(
            f"{estimator} is a class, not an instance."
        )
    if msg is None:
        msg = (
...
def _check_feature_names_in(
    ...
    if input_features is not None:
        input_features = np.asarray(
            input_features, dtype=object
        )
        if (
            feature_names_in_ is not None
    ...
    return np.asarray(
        [f"x{i}" for i in range(n_features_in_)], dtype=object
    )
...
def _pandas_dtype_needs_early_conversion(pd_dtype):
    ...
    if isinstance(
        pd_dtype, pd.SparseDtype
    ) or not is_extension_array_dtype(pd_dtype):
        # Sparse arrays will be converted later in `check_array`
...
def set_array_back(
    ...
    if not (hasattr(X, "__array__") or sp.issparse(X)):
        raise TypeError(
            f"{input_name + ' o' if input_name != '' else 'O'}nly "
    ...
    if to_frame and not sp.issparse(X):
        if columns is None:
            raise ValueError(
                "Name or columns must be supplied for"
                " frame conversion."
            )
        ...
        if _is_arraylike_1d(X):
            if not isinstance(columns, str) and hasattr(
                columns, "__len__"
            ):
                if len(columns) > 1:
                    raise ValueError(
            ...
            if not hasattr(columns, "__len__"):
                raise TypeError(
                    " Columns for {input_name!r} expects "
...
def convert_array_to_pandas(
    ...
    if to_frame and not sp.issparse(X):
        ...
        if not hasattr(columns, "__len__") or isinstance(
            columns, str
        ):
            raise TypeError(
...
def is_frame(
    ...
    if df_only:
        obj_is_frame = hasattr(arr, "__array__") and hasattr(
            arr, "columns"
        )
    else:
...
def check_array(
    ...
    *,
    ...
    array, column_orig = convert_array_to_pandas(
    ...
    dtype_numeric = (
        isinstance(dtype, str) and dtype == "numeric"
    )
    ...
    if hasattr(array, "dtypes") and hasattr(
        array.dtypes, "__array__"
    ):
        ...
        with suppress(ImportError):
            ...
            if not hasattr(array, "sparse") and isinstance(
                array, pd.SparseDtype
            ):
                warnings.warn(
        ...
        pandas_requires_conversion = any(
            _pandas_dtype_needs_early_conversion(i)
            for i in dtypes_orig
        )
        if all(
            isinstance(dtype_iter, np.dtype)
            for dtype_iter in dtypes_orig
        ):
    ...
    if ensure_min_samples > 0:
        ...
        if n_samples < ensure_min_samples:
            raise ValueError(
                ...
                f"(shape={array.shape}) while a minimum of "
...
def validate_dtype_selector(dtype_selector: str) -> str:
    ...
    suffix = (
        "_only"
        if "only" in str(dtype_selector).lower()
        else ""
    )

    if re.match(
        numeric_pattern, dtype_selector, re.IGNORECASE
    ):
        return f"numeric{suffix}"
    elif re.match(
        categoric_pattern, dtype_selector, re.IGNORECASE
    ):
        return f"categoric{suffix}"
    elif re.match(
        datetime_pattern, dtype_selector, re.IGNORECASE
    ):
        return "datetime"
    elif re.match(
        biselect_pattern, dtype_selector, re.IGNORECASE
    ):
        return "biselect"
...
def build_series_if(
    ...
    *arr : array-like or DataFrame
    ...
    Examples
    --------
    >>> from geoprior.utils.validator import build_series_if
    >>> data = [1, 2, 3]
...
def _check_series_names(series_names, data_len, error_policy):
    ...
    """
    if (
        isinstance(series_names, list | tuple)
        and len(series_names) != data_len
    ):
...
def build_data_if(
    ...
    **kw,
    ...
    See more in :func:`geoprior.utils.data_utils.build_df` for
    ...
    force = (
        True
        if (force == "auto" and columns is None)
        else force
    )
...
def _convert_int_columns_to_str(
    df: pd.DataFrame, col_prefix: str | None = "col_"
) -> pd.DataFrame:
    """
    ...
    if all(isinstance(col, int) for col in df.columns):
        ...
        if col_prefix is None:
            ...
            df_converted.columns = (
                df_converted.columns.astype(str)
            )
        else:
...
def array_to_frame2(
    ...
    *,
    ...
    if to_frame and not isf and columns is None:
        if force:
            columns = [
                f"{input_name + str(i)}"
            ...
            msg = (
                f"Array {input_name} is originally not a frame. Frame "
    ...
    elif to_frame and columns is not None:
        isf = True
...
def _check_y_1d(y, *, warn=False, input_name="y"):
    ...
    raise ValueError(
        f"{input_name} should be a 1d array, got"
        ...
        for key in index_keys:
            ...
            if indices_datatype not in supported_indices:
                raise ValueError(
                    ...
                    f" indices are accepted. Got {indices_datatype} indices."
                )
...
def _ensure_sparse_format(
    ...
    if force_all_finite:
        if not hasattr(spmatrix, "data"):
            warnings.warn(
                f"Can't check {spmatrix.format} sparse matrix for nan or inf.",
                stacklevel=2,
            )
        else:
            _assert_all_finite(
...
def _assert_all_finite(
    ...
    err_msg = (
        ...
        " 'soft_imputer' in 'geoprior.utils.mlutils.soft_imputer'."
    )
    ...
    if has_inf or has_nan_error:
        ...
        msg_err = (
            f"Input {padded_input_name}contains {type_err}."
        )
        if (
            estimator_name
            and input_name == "X"
...
def _generate_get_feature_names_out(
    ...
    _check_feature_names_in(
        estimator, input_features, generate_names=False
    )
