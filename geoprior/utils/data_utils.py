...
def widen_temporal_columns(
    ...
    *“Index contains duplicate entries”* errors.
    ...
    data : PathLike object or pandas.DataFrame
        ...
        :pyfunc:`geoprior.utils.format_pihalnet_predictions`.
    dt_col : str
    ...
    * If *ignore_cols* is provided, their first non‑null value per
    ...
    geoprior.plot.forecast.forecast_view : Visualisation routine that
      consumes the resulting wide frame.
    """
    # basic presence check
    if dt_col not in data.columns:
        raise KeyError(
            f"'{dt_col}' not present in DataFrame."
        )
    ...
    if spatial_cols and set(spatial_cols).issubset(
        df_proc.columns
    ):
    ...
    pat = re.compile(
        r"^(?P<base>[A-Za-z0-9_]+?)(?:_(q\d+|actual))?$"
    )
    bases, col_map = set(), {}
    ...
    for base in sorted(bases):
        base_cols = [
            c for c, (b, _) in col_map.items() if b == base
        ]
        sub_df = df_proc[base_cols]

        if verbose >= 3:
            print(
                f"[INFO] Widening base '{base}' ({len(base_cols)} columns)"
            )
        ...
        for col in base_cols:
            ...
            if series.index.duplicated().any():
                ...
                if verbose >= 2:
                    print(
                        f"  └─ {dup_count} duplicate rows in '{col}' → aggregated"
                    )
                series = series.groupby(
                    level=series.index.names
                ).mean()

            # Pivot into wide format
            wide_piece = series.unstack(level=dt_col)
            wide_piece.columns = [
                f"{base_name}_{yr}{suffix}"
                for yr in wide_piece.columns
            ]
    ...
    if ignore_cols:
        group_lvls = [
            lvl
            for lvl in wide_df.index.names
            if lvl != dt_col
        ]
        static_df = (
            df_proc[ignore_cols]
        ...
        if nan_op in {"drop", "both"}:
            if nan_thresh is None:
                ...
                if not 0.0 <= nan_thresh <= 1.0:
                    raise ValueError(
                        "nan_thresh must be between 0 and 1."
                    )
                min_non_na = int(
                    np.ceil(
                        (1 - nan_thresh) * wide_df.shape[1]
                    )
                )
                wide_df = wide_df.dropna(thresh=min_non_na)
            if verbose >= 2:
                print(
                    "[INFO] Rows with excessive NaNs dropped."
                )
...
def nan_ops(
    ...
    def drop_nan(d, wval=None):
        if auxi_data is not None:
            d_cleaned, w_cleaned = drop_nan_in(
                d, wval, axis=0
            )
        else:
            d_cleaned = drop_nan_in(
                d, solo_return=True, axis=0
            )
            w_cleaned = None
    ...
    w_contains_nan = (
        has_nan(auxi_data) if auxi_data is not None else False
    )
    ...
    subject = (
        ...
        else data_kind.capitalize()
    )
    w_subject = (
        "Auxiliary data"
        if data_kind is None
        else ("Feature" if subject == "Target" else "Target")
    )
    ...
    if ops == "validate":
        ...
        if w_contains_nan:
            ...
            if error == "raise":
                ...
                warnings.warn(message, stacklevel=2)
        log(
            "Validation complete. No NaNs detected or handled.",
            2,
        )
    ...
    if ops == "sanitize":
        ...
        collected = array_preserver(
            data, auxi_data, action="collect"
        )
        ...
        if action is None:
            action = "drop"
            log(
                "No action specified. Defaulting to 'drop'.",
                2,
            )
        ...
        if action == "fill":
            ...
            data_filled = fill_NaN(
                data_converted, method="both"
            )
            if auxi_data is not None:
                auxi_filled = fill_NaN(
                    auxi_converted, method="both"
                )
            else:
                auxi_filled = None
        ...
        # Handle 'drop' action: drop NaNs based on 'data_kind' and 'process'.
            ...
            log(
                f"NaN count: {nan_count}, Data length: {data_length}",
                3,
            )
            ...
            if data_kind == "target":
                ...
                if not condition:
                    if process == "do_anyway":
                        ...
                        data_cleaned, auxi_cleaned = drop_nan(
                            data, auxi_data
                        )
                    else:
                        warning_msg = (
            ...
            # Handling when 'data_kind' is 'feature' or None.
                ...
                if condition is None:
                    ...
                    if not condition_met:
                        if process == "do_anyway":
                            log(
                                "Condition not met. Dropping NaNs anyway.",
                                2,
                            )
                            data_cleaned, auxi_cleaned = (
                                drop_nan(data, auxi_data)
                            )
                        else:
                            warning_msg = (
            ...
            if verbose >= 2:
                log(
                    "NaN values have been dropped from the data.",
                    2,
                )
                if auxi_filled is not None:
                    log(
                        "NaN values have been dropped from the witness data.",
                        2,
                    )
...
def mask_by_reference(
    ...
    fill_value: Any = 0,
    mask_columns: str | list[str] | None = None,
    error: str = "raise",
    verbose: int = 0,
    inplace: bool = False,
    savefile: str | None = None,
    ...
    if ref_col not in data.columns:
        msg = (
            f"[mask_by_reference] Column '{ref_col}' not found "
            f"in the DataFrame."
        )
        if error == "raise":
            raise KeyError(msg)
    ...
    if mask_columns is None:
        ...
        if len(not_found) > 0:
            msg_cols = (
                f"[mask_by_reference] The following columns were "
                f"not found in DataFrame: {not_found}."
            )
            if error == "raise":
                raise KeyError(msg_cols)
            elif error == "warn":
                warnings.warn(msg_cols, stacklevel=2)
                # Remove them from mask list if ignoring/warning
                mask_columns = [
                    c for c in mask_columns if c in df.columns
                ]
            else:
    ...
    if verbose > 1:
        print(
            f"[mask_by_reference] Columns to be masked: {mask_cols}"
        )
    ...
    if values is None:
        if verbose > 0:
            print(
                "[mask_by_reference] 'values' is None. Masking ALL rows."
            )
        if fill_value == "auto":
            # 'auto' doesn't make sense with None => fill with None
            if verbose > 0:
                print(
                    "[mask_by_reference] 'fill_value=auto' but no values "
    ...
    if find_closest and not is_numeric:
        if verbose > 0:
            print(
                "[mask_by_reference] 'find_closest=True' but reference "
    ...
    total_matched_rows = (
        set()
    )  # track distinct row indices matched
    ...
    for val in values:
        ...
        if len(matched_idx) == 0:
            ...
            if find_closest:
                msg_val = (
                    f"[mask_by_reference] Could not approximate '{val}' "
                    f"in numeric column '{ref_col}'."
                )
            if error == "raise":
                raise ValueError(msg_val)
            elif error == "warn":
                warnings.warn(msg_val, stacklevel=2)
                continue  # skip
            else:
    ...
    if verbose > 0:
        ...
        print(
            f"[mask_by_reference] Distinct matched rows: {distinct_count}"
        )
...
def pop_labels_in(
    ...
    are_all_frames_valid(
        df, df_only=True
    )  # Ensure that the dataframe is valid.
    ...
    exist_features(
        df, features=columns, name="Category columns"
    )
    exist_labels(
        df,
        labels=labels,
    ...
    if as_categories:
        ...
        df_copy[columns] = df_copy[columns].astype(
            original_dtype
        )
