...
def extract_spatial_roi(
    ...
    **kwargs,
    ...
    from geoprior.core.checks import check_numeric_dtype
...
def gen_negative_samples_plus(
    ...
    strategy = parameter_validator(
        "strategy", target_strs=add_strategies, deep=True
    )(strategy)
    ...
    if strategy == "random_global":
        ...
        for _ in range(len(df) * num_neg_per_pos):
            sample = {
                lon_col: np.random.uniform(lon_min, lon_max),
                lat_col: np.random.uniform(lat_min, lat_max),
                target_col: 0,
            }
    ...
    # -----------------------
        ...
        for _ in range(len(df) * num_neg_per_pos):
            random_vector = np.random.uniform(
                0, 1, size=len(similarity_features)
            )
            dist, idx = nn.kneighbors([random_vector])
            ref = df.iloc[idx[0][0]]
            sample = ref.copy()
            sample[target_col] = 0
            ...
            negatives.append(sample.to_dict())
    ...
    # -----------------------
        ...
        sample_count = min(
            len(df) * num_neg_per_pos, len(low_slope_areas)
        )
    ...
    # -----------------------
        ...
        combined = pd.concat(
            [part1, part2], ignore_index=True
        )

        # Deduplicate based on spatial + feature + target columns
        dedup_cols = (
            spatial_cols + (feature_cols or []) + [target_col]
        )
    ...
    if verbose >= 1:
        ...
        print(
            f"[INFO] Total dataset: {len(combined)} samples"
        )
...
def gen_buffered_negative_samples(
    ...
    strategy = (
        "landslide"
        if strategy.lower() in ("landslide", "event")
        else strategy
    )
    np.random.seed(seed)
    ...
    if verbose >= 1:
        print(
            f"[INFO] Generating negative samples using strategy: {strategy}"
        )
    ...
    base_points = (
        df if strategy == "landslide" else gauge_data
    )

    if base_points is None or not all(
        c in base_points.columns for c in spatial_cols
    ):
        raise ValueError(
    ...
    if verbose >= 1:
        print(
            f"[INFO] Generated {len(df_neg)} negative samples"
        )
        print(
            f"[INFO] Total dataset: {len(combined)} samples"
        )
...
def _visualize_negative_sampling(
    ...
    ax.set_title(
        title
        or f"Negative Sample Generation Strategy: {strategy}"
    )
    ax.legend(
        handles=[
...
def gen_negative_samples(
    ...
    if verbose >= 2:
        print(
            f"[INFO] Generating negative samples within "
    ...
    combined = pd.concat(
        [df_positive, df_negative], ignore_index=True
    )
    ...
    if view:
        ...
        ax.set_title(
            "Negative Sample Generation with Spatial Buffers"
        )
        ax.set_xlabel("Longitude")
        ax.set_ylabel("Latitude")
        ax.legend(
            handles=[
                Patch(color="red", label="Positive Samples"),
                Patch(
                    color="green", label="Negative Samples"
                ),
                Patch(
                    color="blue",
                    alpha=0.2,
...
def _validate_negative_sampling(
    ...
    # Validate input and columns
    exist_features(
        df, features=target_col, name=f"Target '{target_col}'"
    )
    ...
    neg_feature_range = validate_length_range(
        neg_feature_range, param_name="neg_feature_range"
    )
    ...
    if str(id_col).lower() == "auto":
        id_col = find_id_column(
            df, strategy="naive", errors="ignore"
        )
    ...
    if feature_cols is None:
        excluded_cols = (
            list(spatial_cols) + [target_col] + id_col
        )
        feature_cols = [
            col
        ...
        if verbose >= 2:
            print(
                f"[INFO] Auto-detected feature_cols:"
                f" {feature_cols}"
            )

    feature_cols = columns_manager(
        feature_cols, empty_as_none=False
    )

    if len(feature_cols) == 0:
        raise ValueError(
            "No feature columns found. Please "
...
def create_spatial_clusters(
    ...
    **kwargs
    ...
    if algorithm == "kmeans":
        ...
        clusterer = KMeans(
            n_clusters=n_clusters, random_state=42, **kwargs
        )
    elif algorithm == "dbscan":
        kwargs = filter_valid_kwargs(DBSCAN, kwargs)
        clusterer = DBSCAN(**kwargs)
    else:  # algorithm == 'agglo'
        kwargs = filter_valid_kwargs(
...
def _auto_detect_k(
    ...
    for k in K_range:
        ...
        if verbose >= 2:
            output = "k={:<5} | Distortion={:^28} | Silhouette={:^20}".format(
                k,
                f"{distortions[-1]:.3f}",
                f"{silhouettes[-1]:.3f}",
            )
...
def filter_position(
    ...
    def _handle_error(msg, ex_msg=""):
        if error == "raise":
            ...
            warnings.warn(f"{msg}{ex_msg}", stacklevel=2)
    ...
    if pos_cols is None:
        ...
        idx = pos[
            0
        ]  # Consider only the first if multiple are given
    ...
    if len(pos) != len(pos_cols):
        ...
        _handle_error(
            ...
            "do not match.",
            " Extra values or columns will be ignored.",
        )
    ...
    if not find_closest:
        ...
        mask = True
        for p_val, col in zip(pos, pos_cols, strict=False):
...
@validate_params(
    {
        ...
        "condition": [
            StrOptions({"auto", "above", "below", "between"})
        ],
...
def extract_zones_from(
    ...
    **kwargs,
    ...
    if (
        ...
        and threshold.lower() == "auto"
    ):
        if use_negative_criteria:
            thr_value = np.percentile(
                z, percentile
            )  # e.g., lower percentile
            condition = "below"
        else:
    ...
    if (
        ...
        threshold = validate_length_range(
            threshold, param_name="Threshold"
        )

        if condition == "auto":
            condition = (
                "between"  # default if user didn't specify
            )
        ...
        if condition == "auto":
            condition = (
                "below" if use_negative_criteria else "above"
            )
    ...
    if x_filtered is not None and y_filtered is not None:
        ...
        result = pd.DataFrame(
            {x_name: x_filtered, z_name: z_filtered}
        )
    else:
...
def dual_merge(
    ...
    --------
    >>> import pandas as pd
    >>> from geoprior.utils.datautils import dual_merge
    >>> df1 = pd.DataFrame({
    ...
    if find_closest:
        ...
        df2_coords_closest = df2_coords.iloc[
            indices[valid_idx]
        ]

        if force_coords:
            # Force coordinates of df1 to overwrite df2
            df2_coords_closest[feature2_1] = (
                df1_coords_closest[feature1_1].values
            )
            df2_coords_closest[feature2_2] = (
                df1_coords_closest[feature1_2].values
            )
...
def extract_coordinates(
    ...
    if not (
        ...
        emsg = (
            "Expected a DataFrame containing coordinates (`longitude`/"
    ...
    if verbose > 0:
        print(
            "###",
            "No" if not coord_columns else coord_columns,
            "coordinates found.",
        )
...
def get_xy_coordinates(
    ...
    ----------
    >>> import gofast as gf
    >>> from geoprior.utils.spatial_utils import get_xy_coordinates
    >>> testdata = gf.datasets.make_erp ( samples =7, seed =42 , as_frame=True)
    ...
    def get_value_in(val, col, default):
        ...
        if len(x) != 0:
            # now rename col
            df.rename(
                columns={x[0]: str(default)}, inplace=True
            )
    ...
    xy = (
        df[coord_columns] if len(coord_columns) != 0 else None
    )
...
def batch_spatial_sampling(
    ...
    **Random Sampling Results:**
    ...
    if isinstance(sample_size, float):
        ...
        raise ValueError(
            "sample_size must be a float or int."
        )

    if total_samples > len(data):
        raise ValueError(
            "sample_size is larger than the dataset."
        )
    ...
    for col, n_bins, axis in zip(
        ...
        data[axis] = pd.qcut(
            data[col], q=n_bins, duplicates="drop"
        )
    ...
    if stratify_by is not None:
        ...
        if verbose and len(data) > 10_000:
            print(
                "Stratification keys generated"
                f" successfully for {len(data):,} records."
            )
    else:
        data["strat_key"] = (
            "all_data"  # Single group for random sampling
        )
    ...
    for batch_idx in range(n_batches):
        ...
        batch_sampled_data = remaining_data.loc[
            sampled_indices
        ]
        batches.append(
            batch_sampled_data.drop(
                columns=["strat_key"]
                + [
                    axis
                    for axis in ["x_bin", "y_bin"][
...
def spatial_sampling(
    ...
    if isinstance(spatial_bins, int):
        ...
        n_bins_list = [
            validate_positive_integer(n, "spatial_bins")
    ...
    stratify_by = columns_manager(
        stratify_by, empty_as_none=False
    )
    # Create combined stratification key
    strat_columns = stratify_by + [
        axis
        for axis in ["x_bin", "y_bin"][: len(spatial_cols)]
    ]
    if verbose and len(data) > 10_000:
        print("\nGenerating stratification keys...")
        print(
            f"This may take some time for {len(data):,}"
    ...
    data["strat_key"] = (
        data[strat_columns].astype(str).agg("_".join, axis=1)
    )

    # Verbose message when done
    if verbose and len(data) > 10_000:
        print(
            "Stratification keys generated successfully"
    ...
    if verbose:
        print(
            f"Data grouped into {len(grouped):,} stratified bins."
        )
    ...
    if stratify_by is not None:
        ...
        sampled_indices = np.random.choice(
            data.index, size=n_samples, replace=False
        )
    ...
    sampled_data = sampled_data.drop(columns=cols_to_drop)
    if verbose:
        print(
