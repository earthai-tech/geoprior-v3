...
def run_return(
    ...
    if config_return_type == "self":
        ...
        return (
            getattr(self, attribute_name, default_value)
            if attribute_name
            else self
        )
    ...
    if attribute_name:
        ...
        if not allow_private and attribute_name.startswith(
            "_"
        ):
            custom_msg = msg or (
        ...
        if hasattr(self, attribute_name):
            ...
            if check_callable and isinstance(
                attr_value, Callable
            ):
                try:
...
def gen_X_y_batches(
    ...
    *,
    ...
    if shuffle:
        ...
        sample_idx = sk_shuffle(
            indices, random_state=random_state
        )

    for _batch_idx, batch_slice in enumerate(batches):
        # Slice the training data to obtain the current batch
        if shuffle:
            X_batch = _safe_indexing(
                X, sample_idx[batch_slice]
            )
    ...
    try:
        for epoch in range(1, epochs + 1):
            ...
            for step in range(1, steps_per_epoch + 1):
                time.sleep(
                    delay
                )  # Simulate time delay for each step
        ...
        print(
            f"{obj_name}Best Metrics: {best_metric_display}"
        )
    ...
    **dt_kws : dict, optional
    ...
    try:
        data[date_col] = pd.to_datetime(
            data[date_col], **dt_kws
        )
        if verbose:
            print(
                f"Column '{date_col}' successfully converted to datetime format."
            )
    except Exception as e:
        print(
            f"Error converting '{date_col}' to datetime format: {e}"
        )
        return data
...
def unpack_list_of_dicts(list_of_dicts):
    ...
    return dict(
        unpacked_dict
    )  # Convert defaultdict back to dict if required
...
def fancy_printer(
    result, report_name="Data Quality Check Report"
):
    """
    ...
    if not isinstance(result, dict):
        raise TypeError(
            "fancy_printer accepts only a dictionnary type."
    ...
    for key, value in result.items():
        if value:  # Only display categories with findings
            ...
            try:
                for sub_key, sub_value in value.items():
                    ...
                    formatted_value = str(sub_value)[:20] + (
                        ".."
                        if len(str(sub_value)) > 22
                        else ""
                    )
                    print(
                        f"{formatted_key:<17} | {formatted_value}"
                    )
            except:
                formatted_key = (
                    (key[:15] + "..")
            ...
            print(
                f"--- No {key.replace('_', ' ').title()} Found ---\n"
            )
...
def shrunkformat(
    ...
    return re.sub(
        r"[\[,'\]]", "", "".join(spllst), flags=re.IGNORECASE
    )
...
def smart_format(iter_obj, choice="and"):
    ...
    if choice not in ["and", "or"]:
        raise ValueError(
            f"Invalid choice '{choice}'. Must be 'and' or 'or'."
        )
    ...
    try:
        ...
        raise TypeError(
            "The provided input is not an iterable."
        )
...
def make_introspection(
    ...
    for key, value in subObj.__dict__.items():
        if not hasattr(Obj, key) and key != "".join(
            ["__", str(key), "__"]
        ):
...
def format_notes(
    text: str, cover_str: str = "~", inline: int = 70, **kws
):
    """
...
def interpol_scipy(
    ...
    )  # Check for scipy compatibility
    if spi is None:
        return None

    try:
        func_ = spi.interp1d(
            x_value, y_value, kind=kind, fill_value=fill_value
        )
        y_new = func_(x_new)

        if plot:
            plt.plot(
                x_value, y_value, "o", x_new, y_new, "--"
            )
            plt.legend(
                ["Data", kind.capitalize()], loc="best"
            )
...
def stn_check_split_type(data_lines):
    ...
    if isinstance(data_lines, list):
        for _ii, item in enumerate(
            data_lines[: int(len(data_lines) / 3)]
        ):
            data_to_read.append(item)
            # be sure the list is str item .
            data_to_read = [
                "".join([str(item) for item in data_to_read])
            ]
...
def fr_en_parser(f, delimiter=":"):
    ...
    if not is_file:
        raise OSError(
            f"Input {f} is not a file. Please check your file."
        )
...
def drawn_boundaries(erp_data, appRes, index):
    ...
    if f == 0 or f == 2:
        left_term = erp_data[:index][
            ::-1
        ]  # flip left term  for looping
    ...
    if f == 2:
        ...
        anomalyBounds = np.array(
            [appRes] + right_limit.tolist()
        )
    else:
        left_limit = np.append(left_limit, appRes)
        anomalyBounds = np.concatenate(
...
def fmt_text(anFeatures=None, title=None, **kwargs):
    ...
    for anFeatures in newF:
        strfeatures = "|".join(
            [f"{str(i):^15}" for i in anFeatures[:-1]]
        )
        try:
            iter(anFeatures[-1])
        except:
...
def ismissing(
    ...
    miss_val_or_ix = (
        mask if return_index == "mask" else miss_val_or_ix
    )
...
def strip_item(item_to_clean, item=None, multi_space=12):
    ...
    cleaner = [
        ("" + ii * f"{item}") for ii in range(multi_space)
    ]
    ...
    try:
        ...
        raise TypeError(
            "argument <multplier> must be an integer"
...
def sanitize_frame_cols(
    ...
    if isinstance(d, pd.Series):
        ...
        c = is_iterable(
            d, exclude_string=True, transform=True
        )
    ...
    if inspect.isfunction(func):
        ...
        c = list(
            map(
                lambda r: regex.sub(fill_pattern, r.strip()),
                c,
            )
        )
...
def get_confidence_ratio(
    ...
    axis = validate_name_in(
        axis,
        ("1", "rows", "sites", "stations"),
        expect_name=1,
    )
    if not axis:
        axis = 0
    ...
    ratio = np.zeros(
        (
            (ar.shape[0] if axis == 1 else ar.shape[1])
            if ar.ndim == 2
            else 1,
        ),
        dtype=np.float64,
...
def make_obj_consistent_if(
    ...
    default = list(
        is_iterable(
            default, exclude_string=True, transform=True
        )
    )
    ...
    if item not in (..., None):
        item = list(
            is_iterable(
                item, exclude_string=True, transform=True
            )
        )
    else:
        item = []
    ...
    if size is not None:
        size = int(
            _assert_all_types(
                size, int, float, objname="Item 'size'"
            )
        )
        item = item[:size]
...
def ellipsis2false(*parameters, default_value: Any = True):
    """
    ...
    """
    return tuple(
        default_value if param is ... else param
...
def type_of_target(y):
    ...
    # Continuous data
    if any(
        isinstance(i, float) for i in np.array(y).flatten()
    ):
        return "continuous"
    ...
    # Multilabel indicator
    if (
        isinstance(y[0], np.ndarray | list | Sequence)
        and len(y[0]) > 1
    ):
...
def fancier_repr_formatter(obj, max_attrs=7):
    ...
    attrs = [
        ...
        and (
            ...
            or not hasattr(getattr(obj, name), "__iter__")
        )
    ]
    ...
    attr_str = ", ".join(
        [
            f"{name}={value!r}"
            for name, value in displayed_attrs
        ]
    )
...
def normalize_string(
    ...
    """
    normalized_str = (
        str(input_str).lower() if ignore_case else input_str
    )
    ...
    normalized_targets = (
        [str(t).lower() for t in target_strs]
        if ignore_case
        else target_strs
    )
    matched_target = None
    ...
    for target in normalized_targets:
        if num_chars_check is not None:
            condition = (
                normalized_str[:num_chars_check]
                == target[:num_chars_check]
            )
        elif deep:
...
def format_and_print_dict(data_dict, front_space=4):
    ...
    for label, options in data_dict.items():
        print(f"{label}:")
        options = is_iterable(
            options, exclude_string=True, transform=True
        )
...
def fill_nan_in(
    ...
    fill_methods = {
        ...
        "ffill": lambda: data.fillna(
            method="ffill", inplace=True
        ),
        "bfill": lambda: data.fillna(
            method="bfill", inplace=True
        ),
        "mean": lambda: data.fillna(
            data.mean(), inplace=True
        ),
        "median": lambda: data.fillna(
            data.median(), inplace=True
        ),
        "mode": lambda: data.apply(
    ...
    if fill_action:
        ...
        raise ValueError(
            f"Method '{method}' not recognized for filling NaN values."
        )
...
def lowertify(
    ...
    *values : Any
    ...
    if return_origin:
        ...
        return tuple(
            lowered for lowered, _ in processed_values
        )
...
def fetch_value_in(
    ...
    if mode == "soft":
        ...
        patterns = [
            f"^{re.escape(key)}{re.escape(suffix)}$"
        ...
        raise ValueError(
            f"Unsupported mode '{mode}'. Use 'soft' or 'strict'."
        )
    ...
    for k in data.keys():
        if any(
            pattern.match(k) for pattern in regex_patterns
        ):
            matches.append(k)
    ...
    if not matches:
        if error == "raise":
            ...
            raise ValueError(
                f"Unsupported error handling option '{error}'."
...
def error_policy(
    ...
    *,
    ...
    valid_policies = valid_policies or {
        "warn",
        "raise",
        "ignore",
    }
