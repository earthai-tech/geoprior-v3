...
def check_forecast_mode(
    mode, q=None, error="raise", ops="validate", **kw
):
    r"""
...
def validate_q_dict(q_dict, recheck=False):
    ...
    if recheck:
        ...
        validate_quantiles(
            list(new_q_cols.keys()), dtype="float64"
        )
...
def validate_quantiles(
    ...
    """
    quantiles = to_iterable(
        quantiles, transform=True, flatten=True
    )
    ...
    quantiles_np = np.round(
        quantiles_np, decimals=round_digits
    )
...
def _process_soft_quantiles(quantiles, scale_method):
    ...
    if scale_candidates:
        scaled = _apply_scaling(
            scale_candidates, scale_method=scale_method
        )
        scaled_values = _merge_scaled_values(
...
def _process_single_quantile(q):
    ...
    if q < 0:
        raise ValueError(
            f"Negative quantile value: {original}"
        )
...
def _apply_scaling(scale_candidates, scale_method):
    ...
    if scale_method == "uniform":
        max_digits = max(
            len(str(q)) for q in scale_candidates
        )
    ...
    if scale_method == "individual":
        return [
            q / (10 ** len(str(q))) for q in scale_candidates
        ]
...
def detect_quantiles_in(
    ...
    _log_verbose(
        f"Scanning DataFrame columns with prefix: {col_prefix}",
        verbose,
        1,
    )
    ...
    _log_verbose(
        f"Quantiles detected: {sorted(found_quantiles)}",
        verbose,
        3,
    )

    return _format_output(
        quantile_columns, found_quantiles, return_types, df
    )
...
def _process_column(
    ...
    if prefix:
        ...
        col_match = (
            col  # No prefix, use the column name directly
        )
    ...
    try:
        ...
        _log_verbose(
            f"Invalid quantile value in {col}: {e}",
            verbose,
            2,
        )
        return None
    ...
    _log_verbose(
        f"Found quantile match: {col} with value: {q_val}",
        verbose,
        2,
    )
    ...
    if dt_values:
        for d_str in dt_values:
            ...
            if remainder.startswith(date_pattern):
                ...
                return (
                    (True, m.group(1)) if m else (False, None)
                )
...
def build_q_column_names(
    ...
    dt_value: list[str | int] | None = None,
...
def detect_digits(
    ...
    for text in input_data:
        ...
        if verbose >= 5:
            print(
                f"[DEBUG] Processing '{text}' => Matches: {matches}"
            )
...
def validate_consistency_q(
    ...
    if verbose >= 5:
        print(
            f"[DEBUG] Detected quantile values: {detected_q_values}"
        )
    ...
    if valid_quantiles:
        ...
        if error == "raise":
            ...
            warnings.warn(
                err_msg + f"{suff}", UserWarning, stacklevel=2
            )
...
def validate_qcols(
    q_cols: str | int | Sequence[Any],
    ncols_exp: str | None = None,
    ...
    if ncols_exp:
        ...
        for sym in sorted(_ops, key=len, reverse=True):
            ...
            raise ValueError(
                f"Invalid `ncols_exp` format: {ncols_exp}"
            )
...
def build_qcols_multiple(
    q_cols: Sequence[tuple[str, ...]] | None = None,
    qlow_cols: Sequence[str] | None = None,
    qup_cols: Sequence[str] | None = None,
    qmed_cols: Sequence[str] | None = None,
    *,
    ...
    if q_cols is not None:
        if not all(
            isinstance(t, list | tuple) for t in q_cols
        ):
            raise ValueError(
    ...
    if qmed_cols is not None and not allow_pair_when_median:
        ...
        tuples = list(
            zip(qlow_cols, qmed_cols, qup_cols, strict=False)
        )
    else:
        if enforce_triplet:
            raise ValueError(
        ...
        tuples = list(zip(qlow_cols, qup_cols, strict=False))

    return [tuple(t) for t in tuples]


def resolve_quantiles(
    qs: Sequence[int | float | str] | int | float | str,
) -> list[float]:
    """
