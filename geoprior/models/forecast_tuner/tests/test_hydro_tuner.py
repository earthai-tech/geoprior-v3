...
try:
    ...
    from fusionlab.nn.forecast_tuner._hydro_tuner import (
        HydroTuner,
    )
    from fusionlab.nn.pinn.models import (
        PIHALNet,
        TransFlowSubsNet,
    )
...
def dummy_data(
    ...
    inputs = {
        "coords": np.random.rand(
            d["n_samples"], d["horizon"], 3
        ).astype(np.float32),
        "static_features": np.random.rand(
            d["n_samples"], d["static_dim"]
        ).astype(np.float32),
        "dynamic_features": np.random.rand(
            d["n_samples"], d["past_steps"], d["dynamic_dim"]
        ).astype(np.float32),
        "future_features": np.random.rand(
            d["n_samples"], d["horizon"], d["future_dim"]
        ).astype(np.float32),
    }
    targets = {
        "subsidence": np.random.rand(
            d["n_samples"], d["horizon"], d["subs_dim"]
        ).astype(np.float32),
        "gwl": np.random.rand(
            d["n_samples"], d["horizon"], d["gwl_dim"]
        ).astype(np.float32),
    }
...
def test_init_with_invalid_model_name(
    fixed_params, minimal_search_space
):
    """Tests that tuner raises an error for an unknown model string."""
    with pytest.raises(
        ValueError, match="Unknown model name 'InvalidModel'"
    ):
        HydroTuner(
...
def test_init_with_missing_fixed_params(minimal_search_space):
    ...
    with pytest.raises(
        ValueError,
        match="The `fixed_params` dictionary is missing the required key: 'dynamic_input_dim'",
    ):
        HydroTuner(
            model_name_or_cls=PIHALNet,
...
def test_param_space_deprecation(fixed_params):
    ...
    with pytest.warns(
        FutureWarning,
        match="`param_space` argument is deprecated",
    ):
        tuner = HydroTuner(
            model_name_or_cls=PIHALNet,
            fixed_params=fixed_params,
...
def test_create_method_infers_dims(
    dummy_data, minimal_search_space
):
    ...
    assert (
        tuner.fixed_params["dynamic_input_dim"]
        == dummy_data["inputs"]["dynamic_features"].shape[-1]
    )
    assert (
        tuner.fixed_params["output_gwl_dim"]
        == dummy_data["targets"]["gwl"].shape[-1]
    )
    assert (
        tuner.fixed_params["forecast_horizon"]
        == dummy_data["targets"]["gwl"].shape[1]
    )
...
@pytest.mark.parametrize(
    "model_cls", [PIHALNet, TransFlowSubsNet]
)
...
def test_build_with_model_specific_hps(
    fixed_params, dummy_data, tmp_path
):
    """
