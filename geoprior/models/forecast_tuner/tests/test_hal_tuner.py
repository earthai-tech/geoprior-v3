...
try:
    ...
    from fusionlab.nn.forecast_tuner._hal_tuner import (
        HALTuner,
    )
...
def generate_dummy_data(
    ...
    targets = np.random.rand(
        batch_size, horizon, params["output_dim"]
    ).astype(np.float32)
...
@pytest.mark.skipif(
    kt is None, reason="KerasTuner is not installed."
)
def test_haltuner_initialization(default_fixed_params):
    ...
    with pytest.raises(
        ValueError, match="Missing required key"
    ):
...
def test_haltuner_create_method(default_fixed_params):
    ...
    assert (
        ...
        == inputs_data[0].shape[-1]
    )
    assert (
        tuner.fixed_model_params["dynamic_input_dim"]
        == inputs_data[1].shape[-1]
    )
    ...
    assert (
        ...
        == inputs_data[2].shape[-1]
    )
    assert (
        tuner.fixed_model_params["output_dim"]
        == targets_data.shape[-1]
    )
    assert (
    ...
    assert (
        tuner.fixed_model_params["max_window_size"]
        == inputs_data[1].shape[1]
    )
...
def test_haltuner_build_model_compiles(default_fixed_params):
    ...
    try:
        ...
        pytest.fail(
            f"HALTuner.build(hp) failed with an exception: {e}"
        )
