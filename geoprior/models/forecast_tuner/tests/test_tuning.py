...
try:
    ...
    from fusionlab.nn.forecast_tuner._base_tuner import (
        PINNTunerBase,
    )
    from fusionlab.nn.forecast_tuner._pihal_tuner import (
    ...
    print(
        f"Could not import fusionlab PINN components for tuning tests: {e}"
    )
...
def generate_raw_numpy_data_for_tuner(
    ...
    t_past=T_PAST_TUNER,
    t_horizon=T_HORIZON_TUNER,
    s_dim=S_DIM_TUNER,
    d_dim=D_DIM_TUNER,
    f_dim=F_DIM_TUNER,
    out_s_dim=OUT_S_DIM_TUNER,
    out_g_dim=OUT_G_DIM_TUNER,
) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
    inputs_np = {}
    inputs_np["coords"] = np.random.rand(
        batch_size, t_horizon, 3
    ).astype(np.float32)
    inputs_np["dynamic_features"] = np.random.rand(
        batch_size, t_past, d_dim
    ).astype(np.float32)

    if s_dim > 0:
        inputs_np["static_features"] = np.random.rand(
            batch_size, s_dim
        ).astype(np.float32)
    ...
    targets_np = {
        "subsidence": np.random.rand(
            batch_size, t_horizon, out_s_dim
        ).astype(np.float32),
        "gwl": np.random.rand(
            batch_size, t_horizon, out_g_dim
        ).astype(np.float32),
    }
...
def test_pihaltuner_create_with_data_inference():
    ...
    raw_inputs, raw_targets = (
        generate_raw_numpy_data_for_tuner(
            s_dim=S_DIM_TUNER,
            d_dim=D_DIM_TUNER,
            f_dim=F_DIM_TUNER,
        )
    ...
    assert (
        tuner.fixed_model_params["static_input_dim"]
        == S_DIM_TUNER
    )
    assert (
        tuner.fixed_model_params["dynamic_input_dim"]
        == D_DIM_TUNER
    )
    assert (
        tuner.fixed_model_params["future_input_dim"]
        == F_DIM_TUNER
    )
    assert (
        tuner.fixed_model_params["output_subsidence_dim"]
        == OUT_S_DIM_TUNER
    )
    assert (
        tuner.fixed_model_params["output_gwl_dim"]
        == OUT_G_DIM_TUNER
    )
    assert (
        tuner.fixed_model_params["forecast_horizon"]
        == T_HORIZON_TUNER
    )
...
def test_pihaltuner_run_calls_search(mocker):
    ...
    raw_inputs_np, raw_targets_np = (
        generate_raw_numpy_data_for_tuner()
    )

    # Mock the super().search() method
    mocked_base_search = mocker.patch.object(
        PINNTunerBase, "search", autospec=True
    )
    mocked_base_search.return_value = (
        None,
        None,
        None,
    )  # Mock its return
...
def test_pihaltuner_run_short_search_smoke_test():
    ...
    tuner = PIHALTuner.create(
        ...
        project_name="pihal_smoke_e2e",
        directory="tuner_smoke_results_e2e",
    ...
    try:
        ...
        traceback.print_exc()
        pytest.fail(
            f"PIHALTuner smoke test (run method) failed: {e}"
        )
...
if __name__ == "__main__":
    if FUSIONLAB_AVAILABLE and HAS_KT_LIB:
        ...
        print(
            "Skipping PIHALTuner tests: fusionlab components or keras-tuner not available."
        )
