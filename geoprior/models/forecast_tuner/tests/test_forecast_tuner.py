...
def dummy_tuner_data(tuner_shared_config):
    ...
    T_past = (
        6  # Lookback period (e.g., model's max_window_size)
    )
    ...
    X_dynamic = np.random.rand(B, T_past, D_dyn).astype(
        np.float32
    )
    X_future = np.random.rand(
        B, T_future_total, D_fut
    ).astype(np.float32)
    y = np.random.rand(B, H_out, 1).astype(
        np.float32
    )  # OutputDim=1
...
def _run_tuner_and_asserts(
    tuner_func,
    model_class_expected,  # Pass expected class type
    ...
    if model_name_arg in ["xtft", "superxtft", "super_xtft"]:
        param_space["embed_dim"] = [
            base_model_config["embed_dim"]
        ]
        param_space["lstm_units"] = [
            base_model_config["lstm_units"]
        ]
    elif model_name_arg in ["tft", "tft_flex"]:
        param_space["num_lstm_layers"] = [
            base_model_config["num_lstm_layers"]
        ]
        param_space["lstm_units"] = [
            base_model_config["lstm_units"]
        ]
    ...
    for k, v in base_model_config.items():
        if (
            k not in param_space and k not in case_info
        ):  # Avoid overriding
            case_info[k] = v
    ...
    if (
        model_name_arg == "tft_flex" and not use_quantiles
    ):  # Example: point forecast no static/future
        inputs_list_flex = [
            None,
            data_dict["X_dynamic"],
            None,
    ...
    assert best_hps is not None, (
        "Tuner did not return best_hps."
    )
    assert isinstance(best_hps, dict)
    assert best_model is not None, (
        "Tuner did not return a best model."
    )
    ...
    try:
        ...
        pytest.fail(
            f"Best model from {tuner_func.__name__} failed to predict."
            f" Error: {e}"
        )
    ...
    shutil.rmtree(
        temp_dir_path / project_name, ignore_errors=True
    )
...
def test_xtft_tuner_invalid_inputs(dummy_tuner_data):
    ...
    with pytest.raises(
        ValueError,
        match="Inputs must be a list/tuple of 3 elements",
    ):
        xtft_tuner(
            inputs=[data["X_static"], data["X_dynamic"]],
            y=data["y"],
            forecast_horizon=data["forecast_horizon"],
        )
...
@pytest.mark.parametrize(
    "model_name_to_test", ["tft", "tft_flex"]
)  # Test both TFT variants
@pytest.mark.parametrize("tuner_type", ["random", "bayesian"])
def test_tft_tuner_variants_run(
    ...
    _run_tuner_and_asserts(
        ...
        temp_tuner_dir,
        use_quantiles=use_quantiles,
        model_name_arg=model_name_to_test,
        tuner_type_arg=tuner_type,
    )
