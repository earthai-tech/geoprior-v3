...
def _split_flat_array_for_tft(
    ...
    dim_static = getattr(
        model_instance, "static_input_dim", 0
    )
    horizon = getattr(model_instance, "forecast_horizon", 0)
    num_dynamic = getattr(
        model_instance, "num_dynamic_vars", 0
    )
    dim_dynamic = getattr(
        model_instance, "dynamic_input_dim", 0
    )

    # Calculate the expected split point
    static_size = num_static * dim_static
    expected_total_size = static_size + (
        horizon * num_dynamic * dim_dynamic
    )
...
def _split_flat_array_for_base_attentive(
    ...
    dim_future = getattr(
        model_instance, "future_input_dim", 0
    )
    ...
    expected_total_size = (
        dim_static + size_dynamic + size_future
    )
    if X_flat.shape[1] != expected_total_size:
        raise ValueError(
    ...
    X_dynamic = X_flat[:, split1:split2].reshape(
        (-1, past_steps, dim_dynamic)
    )
    X_future = X_flat[:, split2:].reshape(
        (-1, past_steps + horizon, dim_future)
    )
...
def concatenate_fusionlab_inputs(
    ...
    if not isinstance(input_list, list | tuple):
        raise TypeError(
            "`input_list` must be a list or tuple of NumPy arrays."
        )
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if isinstance(X_input, np.ndarray):
                if model_type == "tft":
                    X_reshaped = _split_flat_array_for_tft(
                        X_input, model_instance
                    )
                elif model_type in [
                    "xtft",
                    "halnet",
                    ...
                    X_reshaped = (
                        _split_flat_array_for_base_attentive(
                            X_input, model_instance
                        )
                    )
                else:
                    raise ValueError(
                        f"Unsupported model_type for decorator: {model_type}"
                    )
            ...
            if ops == "concat":
                ...
                if isinstance(X, list | tuple):
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Error concatenating inputs: {e}"
                        )
                ...
                if not isinstance(X, np.ndarray):
                    raise ValueError(
                        "For 'split', X must be a single numpy array."
                    )
                ...
                if model_type == "tft":
                    ...
                    if X.shape[1] != total_size:
                        raise ValueError(
                            f"Expected {total_size} features, got {X.shape[1]}"
                        )
                    ...
                    if X.shape[1] != total_expected:
                        raise ValueError(
                            f"Expected {total_expected} features, got {X.shape[1]}"
                        )

                    X_static = X[:, :static_size]
                    X_static = X_static.reshape(
                        (X_static.shape[0], static_input_dim)
                    )
