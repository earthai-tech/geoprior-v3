...
def patch_dependencies(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda *dfs, ops=None: dfs,
    )

    # 2. validate_quantiles → identity
    monkeypatch.setattr(
        utils, "validate_quantiles", lambda q, dtype=None: q
    )

    # 3. Stub the three metric functions to return known constants
    monkeypatch.setattr(
        utils, "coverage_score", lambda actual, lo, hi: 0.123
    )
    monkeypatch.setattr(
        utils,
    ...
    monkeypatch.setattr(
        ...
        lambda y_true, y_pred, quantiles: 0.789,
    )
    ...
    monkeypatch.setattr(
        gu, "vlog", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        gu,
        "insert_affix_in",
        lambda filename, affix, separator: filename,
    )
    ...
    def fake_to_txt(
        results, format, indent, filename, savepath
    ):
...
def test_stack_quantile_predictions_1d():
    ...
    median = [0.5, 1.5, 2.5]
    upper = [1, 2, 3]
    y_pred = utils.stack_quantile_predictions(
        lower, median, upper
    )
...
def test_stack_quantile_predictions_2d():
    ...
    upper = [[1, 2, 3], [4, 5, 6]]
    y_pred = utils.stack_quantile_predictions(
        np.array(lower), np.array(median), np.array(upper)
    )
...
def test_stack_quantile_predictions_mismatched_raises():
    ...
    with pytest.raises(ValueError):
        utils.stack_quantile_predictions(
            [0, 1], [0.5, 1.5, 2.5], [1, 2]
        )


def test_compute_quantile_diagnostics_success(
    patch_dependencies,
):
...
def test_compute_quantile_diagnostics_invalid_index(
    patch_dependencies,
):
...
def test_compute_quantile_diagnostics_missing_column(
    patch_dependencies,
):
