...
try:
    ...
    def _placeholder_plot_func(
        *args, **kwargs
    ) -> Axes | None:
        if kwargs.get("ax") is not None:
            return kwargs["ax"]
        warnings.warn(
            f"Plot function for {args[0] if args else 'metric'} "
...
Y_LOWER_WIS_1D_K2_PLOT = Y_LOWER_1D[:, np.newaxis] + np.array(
    [-0.5, -1]
)  # (4,2)
Y_UPPER_WIS_1D_K2_PLOT = Y_UPPER_1D[:, np.newaxis] + np.array(
    [0.5, 1]
)  # (4,2)

Y_LOWER_WIS_2D_K2_PLOT = Y_LOWER_2D[
    ..., np.newaxis
] + np.array([[[-0.5, -1]]])  # (3,2,2)
Y_UPPER_WIS_2D_K2_PLOT = Y_UPPER_2D[
    ..., np.newaxis
] + np.array([[[0.5, 1]]])  # (3,2,2)
...
Y_MEDIAN_TS_S2_O2_T3 = (
    Y_PRED_TS_S2_O2_T3  # Simplified for TWIS
)
...
Y_PRED_PSS_S2_O2_T3 = np.array(
    [
        [
            [1, 2, 1],
            [5, 5, 5],
        ],  # Sample 0: Output 0, Output 1
        [
            [3, 2, 3],
            [0, 1, 0],
        ],  # Sample 1: Output 0, Output 1
    ]
)  # (2 samples, 2 outputs, 3 timesteps)
Y_PRED_PSS_NAN = np.array(
    [[1, np.nan, 2], [3, 3, 3]]
...
Y_LOWER_TWIS_S2_O2_K1_T3 = (
    Y_PRED_TS_S2_O2_T3[:, :, np.newaxis, :] - 0.5
)
Y_UPPER_TWIS_S2_O2_K1_T3 = (
    Y_PRED_TS_S2_O2_T3[:, :, np.newaxis, :] + 0.5
)
...
class TestPlotCoverage:
    ...
    def test_summary_bar_kind_2d_raw(self):
        ...
        assert isinstance(ax, Axes)
        assert (
            len(ax.patches) == Y_TRUE_2D_BASIC.shape[1]
        )  # Bars per output
...
class TestPlotCRPS:
    def test_summary_bar_1d(self):
        ax = plot_crps(
            Y_TRUE_1D_BASIC, Y_PRED_ENS_1D, kind="summary_bar"
        )
        assert isinstance(ax, Axes)
...
class TestPlotMeanIntervalWidth:
    def test_summary_bar_1d(self):
        ax = plot_mean_interval_width(
            Y_LOWER_1D, Y_UPPER_1D, kind="summary_bar"
        )
        assert isinstance(ax, Axes)
...
class TestPlotPredictionStability:
    def test_summary_bar_2d_timeseries(self):  # y_pred (N,T)
        ax = plot_prediction_stability(
            Y_PRED_PSS_S3_T5, kind="summary_bar"
        )
        assert isinstance(ax, Axes)

    def test_histogram_3d_timeseries_selected_output(
        self,
    ):  # y_pred (N,O,T)
        ax = plot_prediction_stability(
            Y_PRED_PSS_S2_O2_T3,
...
class TestPlotTheilsUScore:
    def test_summary_bar_2d_timeseries(
        self,
    ):  # y_true/pred (N,T)
        ax = plot_theils_u_score(
            Y_TRUE_TS_S2_O1_T4.squeeze(axis=1),
...
class TestPlotTimeWeightedMetric:
    ...
    def test_twa_time_profile(self):
        y_true_class = (
            Y_TRUE_TS_S2_O1_T4 > np.median(Y_TRUE_TS_S2_O1_T4)
        ).astype(int)
        y_pred_class = (
            Y_PRED_TS_S2_O1_T4 > np.median(Y_TRUE_TS_S2_O1_T4)
        ).astype(int)
        ax = plot_time_weighted_metric(
...
class TestPlotWeightedIntervalScore:  # Non-time-weighted WIS
    def test_summary_bar_1d(self):
        ax = plot_weighted_interval_score(
            Y_TRUE_1D_BASIC,
            Y_MEDIAN_1D,
            Y_LOWER_WIS_1D_K2_PLOT,
            Y_UPPER_WIS_1D_K2_PLOT,
            ALPHAS_K2,
            kind="summary_bar",
        )
        assert isinstance(ax, Axes)
    ...
    def test_histogram_2d_selected_output(self):
        ax = plot_weighted_interval_score(
            Y_TRUE_2D_BASIC,
            Y_MEDIAN_2D,
            Y_LOWER_WIS_2D_K2_PLOT,
            Y_UPPER_WIS_2D_K2_PLOT,
            ALPHAS_K2,
            kind="scores_histogram",
            output_idx=0,
        )
