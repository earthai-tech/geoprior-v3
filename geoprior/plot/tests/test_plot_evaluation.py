...
@pytest.fixture
def forecast_df_point_single_plot(
    base_predictions_for_plot,
) -> pd.DataFrame:
    df = format_predictions_to_dataframe(
        predictions=base_predictions_for_plot[
            "point_single_pred"
        ],
        y_true_sequences=base_predictions_for_plot[
            "y_true_single"
        ],
    ...
    df[SPATIAL_COLS_TEST[0]] = np.repeat(
        np.arange(SAMPLES), H
    )
...
@pytest.fixture
def forecast_df_quantile_multi_plot(
    base_predictions_for_plot,
) -> pd.DataFrame:
    df = format_predictions_to_dataframe(
        predictions=base_predictions_for_plot[
    ...
    df[SPATIAL_COLS_TEST[0]] = np.repeat(
        [f"store_{i}" for i in range(SAMPLES)], H
    )
    df[SPATIAL_COLS_TEST[1]] = np.tile(
        [f"region_{j}" for j in range(H)], SAMPLES
    )
    df["month"] = np.tile([1, 2, 3] * (len(df) // 3), 1)[
        : len(df)
    ]  # Example segment
    return df
...
def test_plot_forecast_comparison_temporal_point(
    ...
    try:
        ...
        pytest.fail(
            f"plot_forecast_comparison (temporal, point) failed: {e}"
        )
...
def test_plot_forecast_comparison_spatial_quantile(
    ...
    try:
        ...
        assert (
            mock_fig.call_count >= 1
        )  # One fig per output_dim or combined
        assert mock_show.call_count >= 1
    except Exception as e:
        pytest.fail(
            f"plot_forecast_comparison (spatial, quantile) failed: {e}"
        )
    print("plot_forecast_comparison (spatial, quantile): OK")


def test_plot_forecast_comparison_errors(
    forecast_df_point_single_plot,
):
    """Test error handling in plot_forecast_comparison."""
    with pytest.raises(
        TypeError, match="must be a pandas DataFrame"
    ):
    ...
    with pytest.raises(
        ...
        plot_forecast_comparison(
            forecast_df=pd.DataFrame({"test": [1]})
        )
    with pytest.raises(
        ValueError,
        match=re.escape(
        ...
        plot_forecast_comparison(
            forecast_df_point_single_plot, kind="bad_kind"
        )
    with pytest.raises(
        ValueError,
        match=re.escape(
...
def test_plot_metric_over_horizon_bar_mae(
    ...
    try:
        ...
        pytest.fail(
            f"plot_metric_over_horizon (bar, mae) failed: {e}"
        )
    ...
    try:
        plot_metric_over_horizon(
            ...
            metrics=[
                "mae",
                "rmse",
            ],  # RMSE requires MSE from sklearn
        ...
        pytest.fail(
            f"plot_metric_over_horizon (line, multi, grouped) failed: {e}"
        )
    print(
        "plot_metric_over_horizon (line, multi, grouped): OK"
    )
...
def test_plot_metric_radar_mae_by_segment(
    ...
    if (
        segment_col_radar
        not in forecast_df_quantile_multi_plot.columns
    ):
        # Add dummy if not present from fixture generation
        forecast_df_quantile_multi_plot[segment_col_radar] = (
            np.tile(np.arange(1, H + 1), SAMPLES * O_MULTI)[
                : len(forecast_df_quantile_multi_plot)
            ]
        )
    ...
    try:
        ...
        pytest.fail(
            f"plot_metric_radar (mae by segment) failed: {e}"
        )
    print("plot_metric_radar (mae by segment): OK")


def test_plot_metric_radar_errors(
    forecast_df_point_single_plot,
):
    ...
    with pytest.raises(
        ...
        plot_metric_radar(
            forecast_df_point_single_plot,
            segment_col="non_existent",
        )
    with pytest.raises(ValueError):
        plot_metric_radar(
            forecast_df_point_single_plot,
            segment_col=SPATIAL_COLS_TEST[0],
