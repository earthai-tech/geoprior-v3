...
try:
    ...
    print(
        f"Skipping plot_forecasts tests due to import error: {e}"
    )
    FUSIONLAB_INSTALLED = False

    class Tensor:
        pass  # Dummy for collection
...
B, H, O_SINGLE, O_MULTI = (
    4,
    3,
    1,
    2,
)  # Batch, Horizon, OutputDims
Q_LIST = [0.1, 0.5, 0.9]
N_Q = len(Q_LIST)
SAMPLES = (
    B  # Number of sequences (distinct sample_idx values)
)
...
def base_predictions() -> dict[str, np.ndarray]:
    ...
    return {
        "point_single_pred": np.random.rand(
            SAMPLES, H, O_SINGLE
        ).astype(np.float32),
        "point_multi_pred": np.random.rand(
            SAMPLES, H, O_MULTI
        ).astype(np.float32),
        "quant_single_pred": np.random.rand(
            SAMPLES, H, N_Q
        ).astype(np.float32),  # O=1
        "quant_multi_pred_3d": np.random.rand(
            SAMPLES, H, N_Q * O_MULTI
        ).astype(np.float32),
        "quant_multi_pred_4d": np.random.rand(
            SAMPLES, H, N_Q, O_MULTI
        ).astype(np.float32),
        "y_true_single": np.random.rand(
            SAMPLES, H, O_SINGLE
        ).astype(np.float32),
        "y_true_multi": np.random.rand(
            SAMPLES, H, O_MULTI
        ).astype(np.float32),
    }
...
@pytest.fixture
def forecast_df_quantile_single(
    base_predictions,
) -> pd.DataFrame:
...
@pytest.fixture
def forecast_df_point_multi_with_spatial(
    base_predictions,
) -> pd.DataFrame:
    ...
    df[SPATIAL_COLS_TEST[0]] = np.tile(
        np.random.rand(SAMPLES), H
    )
    df[SPATIAL_COLS_TEST[1]] = np.tile(
        np.random.rand(SAMPLES), H
    )
    return df
        ...
        def inverse_transform(self, X):
            if X.shape[1] != self.n_features_in_:
                if (
                    ...
                    return (
                        X + 100.0
                    )  # Simplified for single target column
                raise ValueError(
                    f"Scaler expected {self.n_features_in_} features, got {X.shape[1]}"
                )
...
)  # Mock plt.show to prevent plots appearing
...
def test_temporal_point_single_output(
    ...
    try:
        ...
        pytest.fail(
            f"plot_forecasts (temporal, point, single_out) failed: {e}"
        )
...
def test_temporal_quantile_single_output(
    ...
    try:
        ...
        pytest.fail(
            f"plot_forecasts (temporal, quantile, single_out) failed: {e}"
        )
...
def test_spatial_point_multi_output(
    ...
    try:
        ...
        pytest.fail(
            f"plot_forecasts (spatial, point, multi_out) failed: {e}"
        )
...
def test_spatial_quantile_all_horizon_steps(
    ...
    df_quant_spatial[SPATIAL_COLS_TEST[0]] = np.tile(
        np.random.rand(SAMPLES), H
    )
    df_quant_spatial[SPATIAL_COLS_TEST[1]] = np.tile(
        np.random.rand(SAMPLES), H
    )
    ...
    try:
        ...
        pytest.fail(
            f"plot_forecasts (spatial, quantile, all_steps) failed: {e}"
        )
...
def test_scaler_functionality_temporal(
    ...
    df_copy[f"{TARGET_NAME}_pred"] = (
        df_copy[f"{TARGET_NAME}_pred"] - 90
    )
    df_copy[f"{TARGET_NAME}_actual"] = (
        df_copy[f"{TARGET_NAME}_actual"] - 90
    )

    try:
        with patch(
            "matplotlib.pyplot.plot"
        ):  # Mock plot to check data
            plot_forecasts(
                forecast_df=df_copy,
                target_name=TARGET_NAME,
                kind="temporal",
...
def test_error_invalid_kind(forecast_df_point_single):
    ...
    with pytest.raises(
        ...
        plot_forecasts(
            forecast_df_point_single, kind="unknown"
        )
    print("Error invalid kind: OK")


def test_error_missing_spatial_cols_for_spatial_plot(
    forecast_df_point_single,
):
    ...
    print(
        "Error missing/invalid spatial_cols for spatial plot: OK"
    )


def test_error_missing_core_df_columns():
    """Test ValueError if forecast_df misses 'sample_idx' or 'forecast_step'."""
    bad_df1 = pd.DataFrame(
        {"forecast_step": [1, 2], "value_pred": [0, 1]}
    )
    with pytest.raises(
        ValueError, match="must contain 'sample_idx'"
    ):
        plot_forecasts(bad_df1)
    bad_df2 = pd.DataFrame(
        {"sample_idx": [0, 0], "value_pred": [0, 1]}
    )
    with pytest.raises(
        ValueError, match="must contain .* 'forecast_step'"
    ):
