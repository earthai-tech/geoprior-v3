...
class TestPlotNestedQuantiles:
    ...
    def test_multiple_periods_nested_donuts(self):
        ...
        expected_wedges = len(PERIODS_TO_PLOT_NESTED) * len(
            QUANTILE_LEVELS_NESTED
        )
    ...
    def test_nan_handling_omit_in_period(self):
        ...
        df_with_nan.loc[0, ACTUAL_COL_NESTED] = (
            np.nan
        )  # Affects 2022
        df_with_nan.loc[5, QUANTILE_COLS_NESTED[0]] = (
            np.nan
        )  # Affects 2022
    ...
    @pytest.mark.skip(
        "Skip nan propagation error. No mandatory for plotting"
    )
    def test_nan_handling_propagate_results_in_nan_scores(
        self,
    ):
        df_with_nan = DF_NESTED_DONUT.copy()
        df_with_nan.loc[0, ACTUAL_COL_NESTED] = (
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            period_2022_nan_warning = any(
                "All scores for period '2022' are NaN"
                in str(warn.message)
                for warn in w
            )
            ...
            assert period_2022_nan_warning or len(
                ax.patches
            ) < len(QUANTILE_LEVELS_NESTED)
    ...
    def test_all_periods_empty_or_all_nan(self):
        df_empty_periods = DF_NESTED_DONUT[
            DF_NESTED_DONUT["year"] == 2025
        ]  # Empty
        with warnings.catch_warnings(record=True) as w:
            ax = plot_nested_quantiles(
                df=df_empty_periods,
            ...
            assert any(
                "No periods to plot" in str(warn.message)
                for warn in w
            )
    ...
    def test_invalid_inputs(self):
        ...
        with pytest.raises(
            TypeError,
            match="Input 'df' must be a pandas DataFrame",
        ):
            plot_nested_quantiles(
                df="not_a_dataframe",
                actual_col="",
        ...
        with pytest.raises(
            ...
            plot_nested_quantiles(
                df=DF_SINGLE_PERIOD,
                actual_col="wrong_actual",
                quantile_cols=QUANTILE_COLS_NESTED,
                quantile_levels=QUANTILE_LEVELS_NESTED,
            )
        with pytest.raises(
            ValueError,
            match="Length of 'quantile_cols' must match",
        ):
            plot_nested_quantiles(
                df=DF_SINGLE_PERIOD,
                actual_col=ACTUAL_COL_NESTED,
                quantile_cols=QUANTILE_COLS_NESTED[:2],
                quantile_levels=QUANTILE_LEVELS_NESTED,
            )
        with pytest.raises(
            ValueError,
            ...
            plot_nested_quantiles(
                ...
                actual_col=ACTUAL_COL_NESTED,
                quantile_cols=QUANTILE_COLS_NESTED,
                quantile_levels=[0.1, 0.5, 1.1],
            )
        with pytest.raises(
            ValueError,
            match="None of the specified 'periods' found",
        ):
            plot_nested_quantiles(
                df=DF_NESTED_DONUT,
                actual_col=ACTUAL_COL_NESTED,
