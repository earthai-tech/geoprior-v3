...
try:
    ...
    print(
        f"Skipping visualize_forecasts tests due to import error: {e}"
    )
...
def forecast_df_for_viz() -> pd.DataFrame:
    ...
    for year in [2022, 2023]:
        for _i in range(N_SAMPLES_PER_PERIOD):
            data.append(
                {
                    DT_COL_TEST: year,
                    SPATIAL_X_TEST: 113.0
                    + np.random.rand() * 0.1,
                    SPATIAL_Y_TEST: 22.5
                    + np.random.rand() * 0.1,
                    f"{TARGET_NAME_TEST_VIZ}_q50": np.random.rand()
                    * 10,  # For quantile mode
                    f"{TARGET_NAME_TEST_VIZ}_pred": np.random.rand()
                    * 10,  # For point mode
                    # Actuals might also be in this df if test_data is None
                    f"{TARGET_NAME_TEST_VIZ}": np.random.rand()
                    * 10
                    + 1,
                }
            )
...
def test_data_for_viz() -> pd.DataFrame:
    ...
    for year in [
        ...
        2023,
        2024,
    ]:  # Include an extra year not in forecast_df
        for _i in range(N_SAMPLES_PER_PERIOD):
            data.append(
                {
                    DT_COL_TEST: year,
                    ...
                    SPATIAL_Y_TEST: 22.5
                    + np.random.rand() * 0.1,
                    TARGET_NAME_TEST_VIZ: np.random.rand()
                    * 10
                    + 1.5,  # Actual values
                }
            )
...
def test_visualize_spatial_quantile_with_test_data(
    ...
    try:
        ...
        pytest.fail(
            f"visualize_forecasts (spatial, quantile, test_data) failed: {e}"
        )
...
def test_visualize_spatial_point_no_test_data(
    ...
    df[TARGET_NAME_TEST_VIZ] = df[
        f"{TARGET_NAME_TEST_VIZ}_pred"
    ...
    try:
        ...
        pytest.fail(
            f"visualize_forecasts (spatial, point, no test_data) failed: {e}"
        )
...
def test_visualize_non_spatial_plot(
    ...
    df["time_step_plot"] = np.tile(
        np.arange(N_SAMPLES_PER_PERIOD), 2
    )
    ...
    try:
        visualize_forecasts(
            ...
            x="time_step_plot",
            y=f"{TARGET_NAME_TEST_VIZ}_pred",
            verbose=0,
        )
        mock_fig.assert_called()
        mock_show.assert_called_once()
        ...
        pytest.fail(
            f"visualize_forecasts (non-spatial) failed: {e}"
        )
...
def test_visualize_error_missing_columns(forecast_df_for_viz):
    ...
    with pytest.raises(
        KeyError
    ):  # filter_by_period will raise KeyError
        visualize_forecasts(
            df_no_dt,
            dt_col=DT_COL_TEST,
            tname=TARGET_NAME_TEST_VIZ,
    ...
    df_no_x = forecast_df_for_viz.drop(
        columns=[SPATIAL_X_TEST]
    )
    with pytest.raises(
        ValueError,
        match=re.escape(
...
def test_visualize_error_invalid_mode_kind(
    forecast_df_for_viz,
):
    """Test ValueErrors for invalid mode or kind."""
    with pytest.raises(
        ValueError,
        match="Mode must be either 'quantile' or 'point'",
    ):
        visualize_forecasts(
            forecast_df_for_viz,
            dt_col=DT_COL_TEST,
...
def test_visualize_eval_periods_auto_selection(
    ...
    try:
        ...
        pytest.fail(
            f"visualize_forecasts (auto eval_periods) failed: {e}"
        )
