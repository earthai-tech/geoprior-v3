...
class TestPlotRadarScores:
    def test_with_dict_values(self):
        ...
        assert len(ax.get_xticklabels()) == len(
            RADAR_VALUES_DICT
        )

    def test_with_list_values_and_names(self):
        ax = plot_radar_scores(
            data_values=RADAR_VALUES_LIST,
    ...
    def test_with_y_true_y_pred_default_metrics(self):
        ax = plot_radar_scores(
            y_true=Y_TRUE_RADAR, y_pred=Y_PRED_RADAR
        )
        assert isinstance(ax, Axes)
        assert (
            len(ax.get_xticklabels()) == 3
        )  # MAE, RMSE, MAPE
    ...
    def test_normalize_values(self):
        ax = plot_radar_scores(
            data_values=RADAR_VALUES_DICT,
            normalize_values=True,
        )
        assert isinstance(ax, Axes)
        # Check if y-ticks are roughly between 0 and 1
        yticks = ax.get_yticks()
        assert np.all(yticks >= 0) and np.all(
            yticks <= 1.01
        )  # Allow for slight overshoot
    ...
    def test_custom_ax_provided(self):
        ...
        ax = plot_radar_scores(
            data_values=RADAR_VALUES_DICT, ax=custom_ax
        )
    ...
    def test_no_data_values_no_true_pred(self):
        with pytest.raises(
            ValueError,
            match="y_true' and 'y_pred' must be provided",
        ):
            plot_radar_scores()

    def test_empty_values_list(self):
        with pytest.raises(
            ValueError, match="it must not be empty"
        ):
...
class TestPlotQCEDonut:
    ...
    def test_nan_handling_propagate_warns_or_empty_plot(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            nan_warning_found = any(
                "NaNs found with nan_policy='propagate'"
                in str(warn.message)
                for warn in w
            )
            plot_text_indicates_issue = any(
                "Data Issues" in child.get_text()
                for child in ax.texts
            )
            assert (
                nan_warning_found or plot_text_indicates_issue
            )
    ...
    def test_perfect_calibration_donut(self):
        ...
        pd.DataFrame(
            {
                ...
                "q0.25": np.full(
                    100,
                    np.percentile(
                        np.linspace(0.01, 0.99, 100), 25
                    ),
                ),
                "q0.50": np.full(
                    100,
                    np.percentile(
                        np.linspace(0.01, 0.99, 100), 50
                    ),
                ),
                "q0.75": np.full(
                    100,
                    np.percentile(
                        np.linspace(0.01, 0.99, 100), 75
                    ),
                ),
            }
        )
        ...
        with warnings.catch_warnings(record=True):
            ax = plot_qce_donut(
                df=DF_QCE_DONUT,  # Use original data
                actual_col=ACTUAL_COL_DONUT,
                quantile_cols=QUANTILE_COLS_DONUT,
                quantile_levels=QUANTILE_LEVELS_DONUT,
