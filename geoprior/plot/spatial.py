...
def plot_spatial(
    ...
    dt_values: list[int | str] | None = None,
    *,
    cmap: str = "viridis",
    marker_size: int = 50,
    alpha: float = 0.8,
    ...
    **plot_kws,
    ...
    if dt_col:
        ...
        dt_values = dt_values or sorted(
            df[dt_col].dropna().unique()
        )
    elif dt_values is None:
        raise ValueError(
            "Either 'dt_col' or 'dt_values' must be provided"
        )
    ...
    for start in range(0, n, ncols):
        ...
        for ax, dt in zip(axes, row_dts, strict=False):
            ...
            sc = ax.scatter(
                ...
                vmin=vmin
                if vmin is not None
                else slice_df[value_col].min(),
                vmax=vmax
                if vmax is not None
                else slice_df[value_col].max(),
                **plot_kws,
            )
            scatters.append(sc)
            if show_grid:
            ...
            flag = (
                (show_axis.lower() not in ("off", "false"))
                if isinstance(show_axis, str)
                else bool(show_axis)
            )
            if not flag:
                ax.set_axis_off()
        ...
        if cbar == "uniform":
            ...
            for ax_i, sc_i in zip(
                axes, scatters, strict=False
            ):
                fig.colorbar(
                    sc_i,
                    ax=ax_i,
        ...
        if savefig:
            ...
            for fmt in (
                fmts if isinstance(fmts, list) else [fmts]
            ):
                ext = (
                    fmt if fmt.startswith(".") else "." + fmt
                )
                out = f"{base}{prefix}{ext}"
                os.makedirs(
                    os.path.dirname(out) or ".", exist_ok=True
                )
...
plot_spatial.__doc__ = rf"""
Plot spatial scatter maps for multiple time slices.

{_param_docs.base.spatial_cols}
{_param_docs.base.dt_col}
{_param_docs.base.dt_values}
...
{_param_docs.base.cmap}
{_param_docs.base.marker_size}
{_param_docs.base.alpha}
{_param_docs.base.vmin}
{_param_docs.base.vmax}
{_param_docs.base.show_grid}
{_param_docs.base.grid_props}
{_param_docs.base.savefig}
{_param_docs.base.save_fmts}
{_param_docs.base.max_cols}
{_param_docs.base.prefix}
{_param_docs.base.verbose}
{_param_docs.base.cbar}
{_param_docs.base.show_axis}
{_param_docs.base._logger}
...
def plot_spatial_roi(
    ...
    **plot_kws,
    ...
    if cbar == "uniform":
        ...
        for ax_i, sc_i in zip(
            axes.flatten(), scatters, strict=False
        ):
            fig.colorbar(
                sc_i,
                ax=ax_i,
...
def plot_spatial_contours(
    ...
    **contour_kws,
    ...
    if cbar == "uniform":
        ...
        for ax_i, cf_i in zip(
            axes_flat[: idx + 1], scatters, strict=False
        ):
            fig.colorbar(
                cf_i,
                ax=ax_i,
...
plot_spatial_contours.__doc__ = rf"""
...
def plot_hotspots(
    ...
    dt_values: list[int | str] | None = None,
    *,
    grid_size: int | tuple[int, int] = 50,
    ...
    **hexbin_kws,
    ...
    for idx, dt in enumerate(dt_values):
        ...
        thr = (
            ...
            else np.percentile(z, percentile)
        )
        df_hot = df_dt[z >= thr]
    ...
    if cbar == "uniform":
        ...
        for ax_i, hb_i in zip(
            axes_flat[: idx + 1], hexbs, strict=False
        ):
            fig.colorbar(
                hb_i,
                ax=ax_i,
...
def plot_spatial_voronoi(
    ...
    *,
    ...
    if cbar == "uniform":
        ...
        for ax_i, m_i in zip(
            axes_flat[: idx + 1], mosaics, strict=False
        ):
            fig.colorbar(
                m_i,
                ax=ax_i,
...
plot_spatial_voronoi.__doc__ = rf"""
    ...
    *,
    ...
    for idx, dt in enumerate(dt_values):
        ...
        if method == "grid":
            ...
            raise ValueError(
                "Unsupported method: choose 'grid' or 'kde'."
            )

        # plot heatmap
        im = ax.pcolormesh(
            Xi, Yi, Zi, cmap=cmap, shading="auto", alpha=alpha
        )
        imaps.append(im)
    ...
    if cbar == "uniform":
        fig.colorbar(
            imaps[0], ax=axes_flat[: idx + 1], label=value_col
        )
    else:
        for ax_i, im_i in zip(
            axes_flat[: idx + 1], imaps, strict=False
        ):
            fig.colorbar(
                im_i,
                ax=ax_i,
