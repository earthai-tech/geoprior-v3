...
r"""Plotting helpers for evaluation metrics."""
...
def plot_theils_u_score(
    y_true: np.ndarray,
    y_pred: np.ndarray,
    metric_values: float | np.ndarray | None = None,
    metric_kws: dict[str, Any] | None = None,
    kind: PlotKindTheilU = "summary_bar",
    figsize: tuple[float, float] = (8, 6),
    title: str | None = "Theil's U Statistic",
    ylabel: str | None = None,
    bar_color: str | list[str] = "chocolate",
    bar_width: float = 0.8,
    score_annotation_format: str = "{:.4f}",
    reference_line_at_1: bool = True,
    reference_line_props: dict[str, Any] | None = None,
    show_grid: bool = True,
    ...
    **kwargs: Any,
    ...
    if y_true_arr.ndim == 3:  # (N,O,T)
        ...
        y_true_arr.ndim == 1 and y_true_arr.shape[0] < 2
    ):  # (T,) with T<2
        pass  # Will be caught by metric
    elif (
        y_true_arr.ndim == 2 and y_true_arr.shape[1] < 2
    ):  # (N,T) with T<2
        pass  # will be caught by metric
    ...
    if y_true_arr.size == 0 or (
        ...
        ax.set_title(
            title or "Theil's U Plot (No/Invalid Data)"
        )
        if show_grid:
            ax.grid(**(grid_props or {}))
        return ax
    ...
    # --- Plotting Logic ---
    if kind == "summary_bar":
        scores_to_plot: float | np.ndarray
        if metric_values is not None:
            scores_to_plot = metric_values
            if verbose > 0:
                print(
                    f"Using pre-computed Theil's U: {scores_to_plot}"
                )
        else:
            # For summary bar, respect user's multioutput choice
            summary_bar_default_kws = (
                default_kws_for_metric.copy()
            )
            if "multioutput" not in current_metric_kws:
                summary_bar_default_kws["multioutput"] = (
                    "uniform_average"
                )
            ...
            if verbose > 0:
                print(
                    f"Computed Theil's U for summary: {scores_to_plot}"
                )
        ...
        multioutput_used = (current_metric_kws or {}).get(
            "multioutput",
            default_kws_for_metric["multioutput"],
        )

        if np.isscalar(scores_to_plot) or (
            isinstance(scores_to_plot, np.ndarray)
            and scores_to_plot.ndim == 0
        ):
            scores_arr_bar = np.array([scores_to_plot])
            x_labels_bar = ["Theil's U"]
        elif (
            isinstance(scores_to_plot, np.ndarray)
            and scores_to_plot.ndim == 1
        ):
            scores_arr_bar = scores_to_plot
            x_labels_bar = [
                f"Output {i}"
                for i in range(len(scores_arr_bar))
            ]
            if (
                plot_title_str
                and multioutput_used == "raw_values"
            ):
        ...
        if reference_line_at_1:
            ...
            ref_props = {
                **ref_line_defaults,
                **(reference_line_props or {}),
            }
            ax.axhline(1, **ref_props)  # type: ignore

        # Auto-adjust y-limits
        if scores_arr_bar.size > 0 and not np.all(
            np.isnan(scores_arr_bar)
        ):
            ...
            plot_min_y = min(
                min_val,
                0.8 if reference_line_at_1 else min_val,
            )
            plot_max_y = max(
                max_val,
                1.2 if reference_line_at_1 else max_val,
            )

            padding = (
                0.1 * abs(plot_max_y - plot_min_y)
                if abs(plot_max_y - plot_min_y) > 1e-6
                else 0.1
            )
            ax.set_ylim(
                plot_min_y - padding,
                plot_max_y + padding + 0.05,
            )
...
plot_theils_u_score.__doc__ = rf""" 
...
Parameters
----------
{_param_docs.base.y_true}
{_param_docs.base.y_pred}
...
metric_kws : dict, optional  
    ...
    :func:`geoprior.metrics.theils_u_score`
...
{_param_docs.base.figsize}
{_param_docs.base.title}
{_param_docs.base.ylabel}
{_param_docs.base.bar_color}
{_param_docs.base.bar_width}
{_param_docs.base.score_annotation_format}
{_param_docs.base.show_grid}
{_param_docs.base.grid_props}
{_param_docs.base.ax}
{_param_docs.base.verbose}
{_param_docs.base.kwargs}
...
The helper calls :func:`geoprior.metrics.theils_u_score` for the
computation.
...
See Also
--------
geoprior.metrics.theils_u_score  
    Metric implementation.
geoprior.plot.evaluation.plot_crps  
    Continuous Ranked Probability Score visualiser.
geoprior.plot.evaluation.plot_wis  
...
def _calculate_per_sample_output_wis(
    ...
    nan_mask_base_so = np.isnan(y_true_so) | np.isnan(
        y_median_so
    )
    ...
    combined_nan_mask_so = (
        nan_mask_base_so | nan_mask_bounds_so
    )  # (N,O)
    ...
    if nan_policy == "raise" and np.any(combined_nan_mask_so):
        raise ValueError(
            "NaNs found in inputs for per-sample WIS."
        )
    ...
    if K_intervals > 0:
        interval_width_sok = (
            y_upper_sok - y_lower_sok
        )  # (N,O,K)
        if warn_invalid_bounds and np.any(
            y_lower_sok > y_upper_sok
        ):
            warnings.warn(
                "y_lower > y_upper found in inputs for per-sample WIS. "
                "Widths will be negative, affecting score.",
                UserWarning,
                stacklevel=2,
            )
        ...
        sum_interval_comps_so = np.sum(  # Sum over K
            wis_sharp_sok + wis_under_sok + wis_over_sok,
            axis=2,
        )  # (N,O)
        wis_values_so = (
            mae_term_so + sum_interval_comps_so
        ) / (K_intervals + 1.0)
...
def plot_weighted_interval_score(
    ...
    hist_bins: int | Sequence[Real] | str = "auto",
    hist_color: str = "mediumseagreen",
    hist_edgecolor: str = "black",
    figsize: tuple[float, float] = (10, 6),
    title: str | None = "Weighted Interval Score (WIS)",
    xlabel: str | None = None,
    ylabel: str | None = None,
    bar_color: str | list[str] = "mediumseagreen",
    bar_width: float = 0.8,
    score_annotation_format: str = "{:.4f}",
    ...
    **kwargs: Any,
    ...
    weighted_interval_score = get_metric(
    ...
    if not (
        y_median_proc.shape == shapes_to_match
        and y_lower_proc.shape[:2] == shapes_to_match
        and y_upper_proc.shape[:2] == shapes_to_match
        and y_lower_proc.shape[2] == K_intervals
        and y_upper_proc.shape[2] == K_intervals
    ):
        raise ValueError(
            "Processed input shapes are inconsistent."
        )
    ...
    if n_samples == 0:
        warnings.warn(
            "Input arrays are empty. Cannot generate plot.",
            stacklevel=2,
        )
        if ax is None:
            _, ax = plt.subplots(figsize=figsize)
    ...
    if kind == "scores_histogram":
            ...
            if nan_policy_hist == "raise":
                ...
                if not np.any(rows_to_keep):
                    ax.text(
                        0.5,
                        0.5,
                        "All samples omitted due to NaNs.",
                        ha="center",
                        va="center",
        ...
        if y_t_hist.shape[0] == 0:  # All samples omitted
            ax.text(
                0.5,
                0.5,
                "No valid samples for WIS histogram.",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
            if show_grid:
                ax.grid(**(grid_props or {}))
            ax.set_title(
                plot_title_str or "WIS Scores (No Data)"
            )
            return ax
        ...
        if n_outputs > 1:
            ...
            if not (0 <= output_idx < n_outputs):
                raise ValueError(
                    f"output_idx {output_idx} out of bounds."
                )
            scores_to_plot_hist = per_so_wis_scores[
                :, output_idx
            ]
        ...
        if valid_scores_for_hist.size > 0:
            ...
            if (
                ...
                mean_of_plotted_wis = np.mean(
                    valid_scores_for_hist
                )
                score_text = (
                    f"Mean WIS: {mean_of_plotted_wis:.4f}"
                )
                current_title = (
                    plot_title_str
                    or "Distribution of WIS Values"
                )
                plot_title_str = f"{current_title}{current_output_label}\n({score_text})"
        else:
            ax.text(
                0.5,
                0.5,
                "No valid WIS values for histogram.",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
            current_title = (
                plot_title_str or "Distribution of WIS Values"
            )
        ...
        scores_to_plot_bar: float | np.ndarray
        if metric_values is not None:
            scores_to_plot_bar = metric_values
            if verbose > 0:
                print(
                    f"Using pre-computed WIS values: {scores_to_plot_bar}"
                )
        else:
            summary_bar_default_kws = (
                default_kws_for_metric.copy()
            ...
            if verbose > 0:
                print(
                    f"Computed WIS for summary: {scores_to_plot_bar}"
                )
        ...
        if np.isscalar(scores_to_plot_bar) or (
            isinstance(scores_to_plot_bar, np.ndarray)
            and scores_to_plot_bar.ndim == 0
        ):
            scores_arr_bar = np.array([scores_to_plot_bar])
            x_labels_bar = ["Mean WIS"]
        elif (
            isinstance(scores_to_plot_bar, np.ndarray)
            and scores_to_plot_bar.ndim == 1
        ):
        ...
        ax.set_ylabel(
            ylabel or "Weighted Interval Score (WIS)"
        )
        ...
        if scores_arr_bar.size > 0 and not np.all(
            ...
            max_val = np.nanmax(scores_arr_bar)
            padding = (
                0.1 * abs(max_val - min_val)
                if abs(max_val - min_val) > 1e-6
                else 0.1
            )
            ax.set_ylim(
                min(0, min_val - padding),
...
plot_weighted_interval_score.__doc__ = rf"""
...
y_median : ndarray
    ...
    ``y_true``.
{_param_docs.base.y_lower}
{_param_docs.base.y_upper}
...
metric_kws : dict, optional
    ...
    :func:`geoprior.metrics.weighted_interval_score`
    (e.g. ``multioutput='raw_values'``).
...
See Also
--------
geoprior.metrics.weighted_interval_score  
    Numerical implementation of WIS.
geoprior.plot.evaluation.plot_crps  
    Continuous Ranked Probability Score visualiser.
geoprior.plot.evaluation.plot_theils_u_score  
...
def _calculate_per_timestep_values(
    ...
    y_pred_sot: np.ndarray | None = None,  # Shape (N, O, T)
    y_median_sot: np.ndarray | None = None,  # Shape (N, O, T)
    y_lower_sokt: np.ndarray
    | None = None,  # Shape (N, O, K, T)
    y_upper_sokt: np.ndarray
    | None = None,  # Shape (N, O, K, T)
    alphas_k: np.ndarray | None = None,  # Shape (K,)
    nan_policy: Literal[
        "omit", "propagate", "raise"
    ] = "propagate",
    verbose: int = 0,
    ...
    per_timestep_vals = np.full(
        (n_samples, n_outputs, n_timesteps), np.nan
    )
    ...
    if metric_type == "mae":
        ...
        if nan_policy == "propagate":
            per_timestep_vals = np.where(
                combined_nan_mask, np.nan, abs_errors
            )
        elif (
            nan_policy == "omit"
        ):  # Omit NaNs per S,O,T point for this calculation
            per_timestep_vals = np.where(
                combined_nan_mask, np.nan, abs_errors
            )
        elif nan_policy == "raise" and np.any(
            combined_nan_mask
        ):
            raise ValueError(
                "NaNs found in inputs for per-timestep MAE."
            )
        ...
        if y_pred_sot is None:
            raise ValueError(
                "y_pred is required for accuracy."
            )
        nan_mask_pred = np.isnan(y_pred_sot)
        combined_nan_mask = nan_mask_base | nan_mask_pred

        correct_preds = (y_true_sot == y_pred_sot).astype(
            float
        )
        if nan_policy == "propagate":
            per_timestep_vals = np.where(
                combined_nan_mask, np.nan, correct_preds
            )
            ...
            raise ValueError(
                "NaNs found for per-timestep accuracy."
            )
        else:
        ...
        if K_intervals == 0 and verbose > 0:  # type: ignore
            warnings.warn(
                "TWIS with K=0 intervals; effectively Time-Weighted MAE.",
                stacklevel=2,
            )

        nan_mask_median = np.isnan(y_median_sot)  # type: ignore
        nan_mask_bounds = (
            np.any(  # True if any K for that S,O,T is NaN
                np.isnan(y_lower_sokt)
                | np.isnan(y_upper_sokt),
                axis=2,  # type: ignore
            )
        )  # (N,O,T)
        combined_nan_mask = (
            nan_mask_base | nan_mask_median | nan_mask_bounds
        )

        # Calculate WIS_sot (non-time-weighted)
        mae_term_sot = np.abs(
            y_median_sot - y_true_sot
        )  # (N,O,T) type: ignore

        # Expand y_true_sot for broadcasting with K: (N,O,1,T)
        y_t_exp_sokt = y_true_sot[..., np.newaxis, :]
        y_t_exp_sokt = np.swapaxes(
            y_t_exp_sokt, 2, 3
        )  # (N,O,T,1)
        ...
        wis_under_sokt = (y_lower_sokt - y_t_exp_sokt) * (
            y_t_exp_sokt < y_lower_sokt
        )  # type: ignore
        wis_over_sokt = (y_t_exp_sokt - y_upper_sokt) * (
            y_t_exp_sokt > y_upper_sokt
        )  # type: ignore

        sum_interval_wis_comps_sot = np.sum(  # Sum over K
            wis_sharp_sokt + wis_under_sokt + wis_over_sokt,
            axis=2,
        )  # (N,O,T)

        wis_sot = (
            (mae_term_sot + sum_interval_wis_comps_sot)
            / (K_intervals + 1.0)
            if K_intervals > 0
            else mae_term_sot
        )
        ...
        if nan_policy == "propagate":
            per_timestep_vals = np.where(
                combined_nan_mask, np.nan, wis_sot
            )
        elif nan_policy == "omit":
            per_timestep_vals = np.where(
                combined_nan_mask, np.nan, wis_sot
            ...
            raise ValueError(
                "NaNs found for per-timestep interval score."
            )
        else:
            per_timestep_vals = wis_sot
    else:
        raise ValueError(
            f"Unsupported metric_type for per-timestep: {metric_type}"
        )
...
def plot_time_weighted_metric(
    ...
    **kwargs: Any,
    ...
    current_metric_kws["time_weights"] = (
        time_weights  # Override/set
    )
    ...
    y_pred_proc = y_median_proc = y_lower_proc = (
        y_upper_proc
    ) = None
    alphas_proc = None
    ...
    if metric_type in ["mae", "accuracy"]:
        if y_pred is None:
            raise ValueError(
                "y_pred is required for MAE/Accuracy."
            )
        ...
        if y_p.shape != y_true_arr.shape:
            raise ValueError(
                "Shape mismatch: y_true vs y_pred."
            )
        y_pred_proc = y_p.reshape(
            n_samples, n_outputs, n_timesteps
        )
        ...
        if not all(
            v is not None
            for v in [y_median, y_lower, y_upper, alphas]
        ):
            raise ValueError(
        ...
        if alphas_proc.ndim == 0:
            alphas_proc = alphas_proc.reshape(
                1,
            )
        ...
        if y_m.shape != y_true_arr.shape:
            raise ValueError(
                "Shape mismatch: y_true vs y_median."
            )
        ...
        if (
            y_l.ndim == 2
            and K_intervals == y_l.shape[0]
            and n_timesteps == y_l.shape[1]
            and n_samples == 1
            and n_outputs == 1
        ):  # (K,T)
            y_lower_proc = y_l.reshape(
                1, 1, K_intervals, n_timesteps
            )
            y_upper_proc = y_u.reshape(
                1, 1, K_intervals, n_timesteps
            )
        elif (
            y_l.ndim == 3
            and y_l.shape[:1] == (n_samples,)
            and y_l.shape[1] == K_intervals
            and n_outputs == 1
        ):  # (N,K,T)
            y_lower_proc = y_l.reshape(
                n_samples, 1, K_intervals, n_timesteps
            )
            y_upper_proc = y_u.reshape(
                n_samples, 1, K_intervals, n_timesteps
            )
            ...
            y_l.ndim == 4
            and y_l.shape[:3] == expected_bounds_shape_prefix
        ):  # (N,O,K,T)
        ...
        if (
            ...
            raise ValueError(
                "Timestep dimension mismatch in bounds."
            )

    if (
        n_samples == 0 or n_timesteps == 0
    ):  # Handled after reshaping for clarity
        warnings.warn(
            "Effective data is empty. Cannot generate plot.",
            stacklevel=2,
        )
        if ax is None:
            _, ax = plt.subplots(figsize=figsize)
        ax.set_title(
            title or f"{metric_type.upper()} Plot (No Data)"
        )
        if show_grid:
            ax.grid(**(grid_props or {}))
        return ax
    ...
    plot_title_str = (
        ...
        else f"Time-Weighted {metric_type.replace('_', ' ').title()}"
    )
    ...
    if kind == "time_profile":
        if (
            n_timesteps < 1
            and metric_type != "interval_score"
        ):  # IS can have K=0
            warnings.warn(
                "Need at least 1 timestep for time_profile plot.",
                stacklevel=2,
            )
            # Fallback to empty plot
            ax.set_title(
                plot_title_str + " (Not Enough Data)"
            )
            if show_grid:
                ax.grid(**(grid_props or {}))
            return ax
        ...
        sample_weights_for_avg = (
            current_metric_kws or {}
        ).get("sample_weight", None)

        if sample_idx is not None:
            if not (0 <= sample_idx < n_samples):
                raise ValueError(
                    f"sample_idx {sample_idx} out of bounds."
                )
            profile_data_ot = per_timestep_sot[
                sample_idx, :, :
            ]  # (O,T)
            ...
            if sample_weights_for_avg is not None:
                ...
                if (
                    np.sum(s_w)
                    < default_overall_metric_kws["eps"]
                ):  # type: ignore
                    profile_data_ot = np.full(
                        (n_outputs, n_timesteps), np.nan
                    )
                    ...
                    if isinstance(
                        profile_data_ot, np.ma.MaskedArray
                    ):
                        profile_data_ot = (
                            profile_data_ot.filled(np.nan)
                        )
            else:
                profile_data_ot = np.nanmean(
                    per_timestep_sot, axis=0
                )
        ...
        if n_outputs > 1:
            if output_idx is None:
                ...
                profile_to_plot_t = profile_data_ot[
                    output_idx, :
                ]
                current_output_label = (
                    f" (Output {output_idx})"
        ...
        ax.set_ylabel(
            ylabel or f"Per-Timestep {metric_type.upper()}"
        )
        ...
        if show_time_weights_on_profile and n_timesteps > 0:
            ...
            if time_weights is None:
                ...
                if w_t_plot.shape[0] != n_timesteps:
                    raise ValueError(
                ...
                if (
                    ...
                    w_t_plot = (
                        ...
                        if not np.any(w_t_plot != 0)
                        else w_t_plot
                    )
                else:
                    w_t_plot = w_t_plot / sum_w_t_plot

            if (
                w_t_plot.size == n_timesteps
                ...
                ax2.bar(
                    ...
                    alpha=0.3,
                    width=0.8,
                    color=time_weights_color,
                    label="Time Weights",
                )
                ax2.set_ylabel(
                    "Time Weight", color=time_weights_color
                )
                ax2.tick_params(
                    axis="y", labelcolor=time_weights_color
                )
        ...
        if show_score_on_title:
            score_for_title: float | np.ndarray | None = None
            if metric_values is not None:  # Use pre-computed
                ...
                if (
                    ...
                    score_for_title = (
                        ...
                        and output_idx_to_use
                        < len(metric_values)
                        else np.nanmean(metric_values)
                    )
                else:
                    score_for_title = metric_values
                ...
                cleaned_title_kws = _get_valid_kwargs(
                    metric_func, title_score_kws
                )

                # Prepare inputs for metric_func based on metric_type
                metric_inputs = {"y_true": y_true_arr}
                if metric_type in ["mae", "accuracy"]:
                    metric_inputs["y_pred"] = y_pred  # type: ignore
                elif metric_type == "interval_score":
                    metric_inputs.update(
                        {
                            "y_median": y_median,
                            "y_lower": y_lower,  # type: ignore
                            "y_upper": y_upper,
                ...
                try:
                    calculated_scores = metric_func(
                        **metric_inputs, **cleaned_title_kws
                    )
                    if (
                        isinstance(
                            calculated_scores, np.ndarray
                        )
                        and n_outputs > 1
                        and output_idx is not None
                    ):
                        score_for_title = calculated_scores[
                            output_idx_to_use
                        ]
                    else:
                        score_for_title = float(
                            np.ravel(calculated_scores)[0]
                        )  # type: ignore
                except Exception as e:
                    warnings.warn(
                        f"Could not calculate score for title: {e}",
                        stacklevel=2,
                    )

            if score_for_title is not None and not np.isnan(
                score_for_title
            ):  # type: ignore
                score_text = (
                    f"Overall Score: {score_for_title:.4f}"
                )
        ...
        if metric_values is not None:
            ...
            if verbose > 0:
                print(
                    f"Using pre-computed scores: {scores_to_plot}"
                )
        else:
            summary_bar_kws = {
                **overall_score_kws,
            ...
            if "multioutput" not in current_metric_kws:
                summary_bar_kws["multioutput"] = (
                    "uniform_average"
                )

            cleaned_kws = _get_valid_kwargs(
                metric_func, summary_bar_kws
            )
            ...
            scores_to_plot = metric_func(
                **metric_inputs, **cleaned_kws
            )
            if verbose > 0:
                print(
                    f"Computed scores for summary: {scores_to_plot}"
                )
        ...
        multioutput_used = (current_metric_kws or {}).get(
            "multioutput",
            default_overall_metric_kws["multioutput"],
        )
        ...
        if np.isscalar(scores_to_plot) or (
            ...
            raise TypeError(
                f"Unexpected scores type/shape: {type(scores_to_plot)}"
            )
        ...
        ax.set_ylabel(
            ylabel or f"{metric_type.upper()} Score"
        )
        ...
        if scores_arr_bar.size > 0 and not np.all(
            ...
            if metric_type == "accuracy":
                ax.set_ylim(
                    ...
                    min(1, max_val + padding + 0.05),
                )
            else:  # MAE, Interval Score (errors, can be >1 or <0 for IS)
                ax.set_ylim(
                    min_val - padding if min_val < 0 else 0,
                    max_val + padding + 0.05,
                )
...
plot_time_weighted_metric.__doc__ = rf"""
...
metric in :pymod:`geoprior.metrics` and applies the chosen *time
...
{_param_docs.base.y_true}
{_param_docs.base.y_pred}
{_param_docs.base.y_median}
{_param_docs.base.y_lower}
{_param_docs.base.y_upper}
{_param_docs.base.alphas}
...
^^^^^^^^^^^^^^^^^^^^
{_param_docs.base.figsize}
{_param_docs.base.show_score_on_title}
{_param_docs.base.show_grid}
{_param_docs.base.grid_props}
{_param_docs.base.ax}
{_param_docs.base.verbose}
...
See Also
--------
geoprior.metrics.time_weighted_mae  
geoprior.metrics.time_weighted_accuracy  
geoprior.metrics.time_weighted_interval_score  
...
def plot_quantile_calibration(
    ...
    qce_values: float | np.ndarray | None = None,
    metric_kws: dict[str, Any] | None = None,
    kind: Literal[
        "reliability_diagram", "summary_bar"
    ] = "reliability_diagram",
    output_idx: int | None = None,
    ...
    title: str | None = "Quantile Calibration Error (QCE)",
    xlabel: str | None = None,
    ylabel: str | None = None,
    bar_color: str | list[str] = "darkcyan",
    bar_width: float = 0.8,
    score_annotation_format: str = "{:.4f}",
    ...
    **kwargs: Any,
    ...
    quantile_calibration_error = get_metric(
    ...
    if y_true_ndim_orig == 1:  # (N,)
        ...
        if (
            y_pred_q_arr.ndim == 2
            and y_pred_q_arr.shape[1] == n_quantiles
        ):  # (N,Q)
            y_pred_proc = y_pred_q_arr.reshape(
                y_pred_q_arr.shape[0],
                1,
                -1,  # (N,1,Q)
            )
        else:
        ...
        if (
            y_pred_q_arr.ndim == 3
            and y_pred_q_arr.shape[1] == y_true_proc.shape[1]
            and y_pred_q_arr.shape[2] == n_quantiles
        ):  # (N,O,Q)
    ...
    if (
        ...
        raise ValueError(
            "y_true and y_pred_quantiles n_samples mismatch."
        )
    ...
    if n_quantiles == 0 and kind != "summary_bar":
        warnings.warn(
            "No quantiles provided. Cannot generate plot.",
            stacklevel=2,
        )
        if ax is None:
            _, ax = plt.subplots(figsize=figsize)
    ...
    if kind == "reliability_diagram":
        ...
        if output_idx is not None and not (
            0 <= output_idx < n_outputs
        ):
            raise ValueError(
                f"output_idx {output_idx} out of bounds."
            )
        ...
        current_output_to_plot = (
            output_idx if output_idx is not None else 0
        )
        ...
        sample_weight_plot = current_metric_kws.get(
            "sample_weight", None
        )
        eps_plot = current_metric_kws.get(
            "eps", default_kws_for_metric["eps"]
        )
        ...
        if np.any(nan_mask_sq):
            if nan_policy_plot == "raise":
                ...
                if not np.any(rows_to_keep):
                    ...
                    ax.set_title(
                        plot_title_str
                        or "Reliability Diagram (No Data)"
                    )
                    return ax
                indicators = indicators[rows_to_keep]
                if sample_weight_plot is not None:
                    sample_weight_plot = sample_weight_plot[
                        rows_to_keep
                    ]
                nan_mask_sq = nan_mask_sq[
                    rows_to_keep
                ]  # For propagate consistency
        ...
        if indicators.shape[0] == 0:  # All samples omitted
            ax.text(
                0.5,
                0.5,
                "No valid samples for reliability diagram.",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
                ...
                for q_idx in range(n_quantiles):
                    ...
                    if np.any(finite_mask_q):
                        ...
                        if sum_finite_weights >= eps_plot:
                            ...
                            temp_props.append(np.nan)
                    else:
                        temp_props.append(np.nan)
                observed_proportions = np.array(temp_props)
        else:
            observed_proportions = np.nanmean(
                indicators, axis=0
            )
        ...
        ax.plot(
            ...
            [0, 1],
            linestyle="--",
            color=perfect_calib_color,
            label="Perfect Calibration",
        )
        ax.plot(
            q_arr,
            observed_proportions,
            marker=observed_prop_marker,
            linestyle="-",
            color=observed_prop_color,
            label="Observed Proportion",
        )

        if show_score:
            score_for_title: float | None = None
            if qce_values is not None:
                if n_outputs > 1 and output_idx is not None:
                    if (
                        isinstance(qce_values, np.ndarray)
                        and qce_values.ndim == 1
                        and output_idx < len(qce_values)
                    ):
                        score_for_title = qce_values[
                            output_idx
                        ]
                elif np.isscalar(qce_values) or (
                    isinstance(qce_values, np.ndarray)
                    and qce_values.size == 1
                ):
                    score_for_title = float(
                        np.ravel(qce_values)[0]
                    )
                ...
                if n_outputs > 1 and output_idx is not None:
                    ...
                    cleaned_kws = _get_valid_kwargs(
                        quantile_calibration_error, title_kws
                    )
                    try:
                        all_output_scores = (
                            quantile_calibration_error(
                        ...
                        score_for_title = all_output_scores[
                            output_idx
                        ]
                    except Exception as e:
                        warnings.warn(
                            f"Could not calculate QCE for title: {e}",
                            stacklevel=2,
                        )
                else:  # Overall score
                    title_kws["multioutput"] = (
                        "uniform_average"
                    )
                    cleaned_kws = _get_valid_kwargs(
                        quantile_calibration_error, title_kws
            ...
            if score_for_title is not None and not np.isnan(
                ...
                score_text = (
                    f"Avg. QCE: {score_for_title:.4f}"
                )
                current_title = (
                    plot_title_str
                    or "Quantile Reliability Diagram"
                )
                output_label_title = (
                    f" (Output {output_idx})"
                    if n_outputs > 1
        ...
        ax.set_ylabel(
            ylabel or "Observed Proportion (y <= Q_pred(q))"
        )
        ax.legend(loc="best")
        ...
        if qce_values is not None:
            ...
            if verbose > 0:
                print(
                    f"Using pre-computed QCE: {scores_to_plot}"
                )
        else:
            summary_bar_default_kws = (
                default_kws_for_metric.copy()
            ...
            cleaned_kws = _get_valid_kwargs(
                quantile_calibration_error, effective_kws
            )
            ...
            if verbose > 0:
                print(
                    f"Computed QCE for summary: {scores_to_plot}"
                )
        ...
        if np.isscalar(scores_to_plot) or (
            ...
            raise TypeError(
                f"Unexpected QCE scores type/shape: {type(scores_to_plot)}"
            )
        ...
        ax.set_ylabel(
            ylabel or "Quantile Calibration Error (QCE)"
        )
        ...
        if scores_arr_bar.size > 0 and not np.all(
            ...
            max_val = np.nanmax(scores_arr_bar)
            ax.set_ylim(
                0, max(0.1, max_val + 0.1 * max_val + 0.05)
            )
...
plot_quantile_calibration.__doc__ = rf"""
...
----------
{_param_docs.base.y_true}
{_param_docs.base.y_pred_quantiles}
{_param_docs.base.quantiles}
...
metric_kws : dict, optional  
    ...
    :func:`geoprior.metrics.quantile_calibration_error`.
...
See Also
--------
geoprior.metrics.quantile_calibration_error  
...
def plot_coverage(
    ...
    legend: bool = True,
    show_score: bool = True,
    bar_color: str | list[str] = "cornflowerblue",
    bar_width: float = 0.8,
    ...
    **kwargs: Any,
    ...
    n_outputs = (
        y_true_arr.shape[1] if y_true_arr.ndim == 2 else 1
    )
    ...
    if kind == "intervals":
        ...
        if y_true_arr.ndim == 2:  # Multi-output or (N,1)
            ...
            if (
                n_outputs > 1
            ):  # Add label only if truly multi-output
                current_output_label = (
                    f" (Output {output_idx_to_use})"
                )

        if sample_indices is None:
            x_indices = np.arange(n_samples)
        else:
            x_indices = check_array(
                sample_indices, ensure_2d=False, copy=False
            )
        ...
        ax.fill_between(
            ...
            **kwargs.get("fill_between_kwargs", {}),
        )
        if line_color:  # Renamed from true_line_color
            ax.plot(
                ...
                linewidth=line_width,  # Renamed
                label="True Values (line)",
                **kwargs.get("plot_kwargs", {}),
            )
        ax.scatter(
            x_plot[covered_mask],
            y_t_plot_valid[covered_mask],
            color=covered_color,
            marker=marker,
            s=marker_size,  # Renamed
            label="Covered True Value",
            zorder=3,
            ...
            if coverage_values is not None:
                if y_true_arr.ndim == 2 and n_outputs > 1:
                    if (
                        ...
                        and coverage_values.ndim == 1
                        and output_idx_to_use
                        < len(coverage_values)
                    ):
                        score_for_title = coverage_values[
                            output_idx_to_use
                        ]
                    elif np.isscalar(
                        coverage_values
                    ):  # If user passed overall avg
                        score_for_title = float(
                            coverage_values
                        )
                elif np.isscalar(coverage_values) or (
                    isinstance(coverage_values, np.ndarray)
                    and coverage_values.size == 1
                ):
                    score_for_title = float(
                        np.ravel(coverage_values)[0]
                    )
                ...
                try:
                    ...
                    if verbose > 0:
                        print(
                            f"Coverage score (for title, output "
        ...
        if coverage_values is not None:
            ...
            if verbose > 0:
                print(
                    f"Using pre-computed coverage values: {scores_to_plot}"
                )
        else:
            ...
            if (
                "multioutput" not in current_metric_kws
            ...
            if verbose > 0:
                print(
                    f"Computed coverage score(s) for summary: {scores_to_plot}"
                )

        scores_arr: np.ndarray
        x_labels: list[str]
        multioutput_used_for_score = (
            current_metric_kws or {}
        ).get(
            "multioutput",
            default_kws_for_metric["multioutput"],
        ...
        if np.isscalar(scores_to_plot) or (
            ...
            x_labels = [
                f"Output {i}" for i in range(len(scores_arr))
            ]
            if (
                plot_title_str
                and multioutput_used_for_score == "raw_values"
            ):
                plot_title_str += " (Per Output)"
        else:
            raise TypeError(
        ...
        ax.set_ylim(
            ...
            max(
                1.1,
                np.nanmax(scores_arr) + 0.1
                if scores_arr.size > 0
                and not np.all(np.isnan(scores_arr))
                else 1.1,
            ),
        )
...
coverage_values : float or ndarray, optional  
    ...
    internal call to :func:`geoprior.metrics.coverage_score`.
metric_kws : dict, optional  
    Extra keyword arguments forwarded to
    :func:`geoprior.metrics.coverage_score` when
...
geoprior.plot.evaluation.plot_quantile_calibration  
...
def plot_crps(
    ...
    crps_values: float | np.ndarray | None = None,
    metric_kws: dict[str, Any] | None = None,
    kind: Literal[
        "ensemble_ecdf", "scores_histogram", "summary_bar"
    ] = "summary_bar",
    sample_idx: int = 0,
    output_idx: int = 0,
    ...
    figsize: tuple[float, float] = (10, 6),
    title: str
    | None = "Continuous Ranked Probability Score (CRPS)",
    xlabel: str | None = None,
    ylabel: str | None = None,
    bar_color: str | list[str] = "cornflowerblue",
    bar_width: float = 0.8,
    ...
    **kwargs: Any,
    ...
    if y_true_arr.ndim == 1:  # (N,) implies single output
        ...
        if y_pred_ensemble_arr.ndim == 2:  # (N, M)
            y_pred_proc = y_pred_ensemble_arr.reshape(
                y_pred_ensemble_arr.shape[0],
                1,
                -1,  # (N, 1, M)
            )
        elif (
            y_pred_ensemble_arr.ndim == 3
            and y_pred_ensemble_arr.shape[1] == 1
        ):  # (N, 1, M)
        ...
        if (
            y_pred_ensemble_arr.ndim == 3
            and y_pred_ensemble_arr.shape[1]
            == y_true_proc.shape[1]
        ):  # (N,O,M)
            y_pred_proc = y_pred_ensemble_arr
        else:
    ...
    if n_samples == 0:
        ...
        return ax
    if (
        n_members == 0 and kind != "summary_bar"
    ):  # summary_bar might use precomputed
        warnings.warn(
        ...
        if ax is None:
            _, ax = plt.subplots(figsize=figsize)
        ax.set_title(
            title or f"CRPS {kind} (No Ensemble Members)"
        )
        if show_grid:
            ax.grid(**(grid_props or {}))
        return ax
    ...
    if kind == "ensemble_ecdf":
        ...
        sample_ensemble = y_pred_proc[
            sample_idx, output_idx, :
        ]
        sample_true_value = y_true_proc[
            sample_idx, output_idx
        ]

        if np.isnan(sample_true_value) or np.all(
            np.isnan(sample_ensemble)
        ):
            ax.text(
                0.5,
                0.5,
                "Data for ECDF contains NaNs",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
            ...
            sample_ensemble_valid = sample_ensemble[
                ~np.isnan(sample_ensemble)
            ]
            if sample_ensemble_valid.size == 0:
                ax.text(
                    0.5,
                    0.5,
                    "No valid ensemble members for ECDF",
                    ha="center",
                    va="center",
                    transform=ax.transAxes,
                )
            else:
                sorted_ensemble = np.sort(
                    sample_ensemble_valid
                )
                ecdf_y = np.arange(
                    1, len(sorted_ensemble) + 1
                ) / len(sorted_ensemble)
                ax.step(
                    sorted_ensemble,
                    ecdf_y,
                ...
                ax.plot(
                    sorted_ensemble,
                    np.zeros_like(sorted_ensemble) - 0.05,
                    "|",
                    color=ensemble_marker_color,
                    markersize=10,
                    alpha=0.5,
                ...
                ax.axvline(
                    ...
                    linestyle="--",
                    label=f"True Value: {sample_true_value:.2f}",
                )

                if show_score:
                    instance_crps = None
                    if (
                        crps_values is not None
                    ):  # User provided per-instance scores
                        ...
                        instance_kws["multioutput"] = (
                            "raw_values"  # Get raw for this one
                        )
                        cleaned_kws = _get_valid_kwargs(
                            continuous_ranked_probability_score,
                            instance_kws,
                        ...
                        try:
                            ...
                            temp_y_true = y_true_proc[
                                sample_idx : sample_idx + 1,
                                output_idx : output_idx + 1,
                            ]
                            temp_y_pred = y_pred_proc[
                                sample_idx : sample_idx + 1,
                                output_idx : output_idx + 1,
                                :,
                            ]
                            ...
                            warnings.warn(
                                f"Could not calculate CRPS for ECDF title: {e}",
                                stacklevel=2,
                            )

                    if (
                        instance_crps is not None
                        and not np.isnan(instance_crps)
                    ):
                        score_text = (
                            f"CRPS: {instance_crps:.4f}"
                        )
                        current_title = (
                            plot_title_str
                            or "Ensemble ECDF vs True Value"
                        )
                        plot_title_str = (
                            f"{current_title}\n({score_text})"
                        )
        ...
        if crps_values is not None:
            ...
            for s_idx in range(n_samples):
                ...
                single_sample_kws["multioutput"] = (
                    "raw_values"  # get (O,) scores
                )
            ...
            per_sample_output_crps = np.array(
                all_sample_crps_list
            )
        ...
        if per_sample_output_crps is not None:
            ...
            if valid_crps_for_hist.size > 0:
                ...
                avg_crps_for_title = np.nanmean(
                    valid_crps_for_hist
                )
                if show_score and not np.isnan(
                    avg_crps_for_title
                ):
                    current_title = (
                        plot_title_str
                        or "Distribution of CRPS Values"
                    )
                ...
                ax.text(
                    0.5,
                    0.5,
                    "No valid CRPS scores for histogram",
                    ha="center",
                    va="center",
                    transform=ax.transAxes,
                )
            ...
            ax.text(
                0.5,
                0.5,
                "CRPS scores not available for histogram",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
        ...
        if crps_values is not None:
            ...
            if verbose > 0:
                print(
                    f"Using pre-computed CRPS: {scores_to_plot}"
                )
        else:
            summary_bar_default_kws = (
                default_kws_for_metric.copy()
            ...
            if verbose > 0:
                print(
                    f"Computed CRPS for summary: {scores_to_plot}"
                )
        ...
        min_score = (
            np.nanmin(scores_arr)
            if scores_arr.size > 0
            else 0
        )
        max_score = (
            np.nanmax(scores_arr)
            if scores_arr.size > 0
            else 0.1
        )
        ax.set_ylim(
            min(0, min_score - 0.1 * abs(min_score)),
...
crps_values : float or ndarray, optional  
    ...
    calls to :func:`geoprior.metrics.continuous_ranked_probability_score`.
{_param_docs.base.metric_kws}
...
See Also
--------
geoprior.metrics.continuous_ranked_probability_score  
...
def plot_mean_interval_width(
    y_lower: np.ndarray,
    y_upper: np.ndarray,
    miw_values: float | np.ndarray | None = None,
    metric_kws: dict[str, Any] | None = None,
    kind: Literal[
        "widths_histogram", "summary_bar"
    ] = "summary_bar",
    output_idx: int | None = None,
    hist_bins: int | Sequence[Real] | str = "auto",
    ...
    hist_edgecolor: str = "black",
    figsize: tuple[float, float] = (10, 6),
    title: str | None = "Mean Interval Width (Sharpness)",
    xlabel: str | None = None,
    ylabel: str | None = None,
    bar_color: str | list[str] = "mediumpurple",
    bar_width: float = 0.8,
    score_annotation_format: str = "{:.4f}",
    show_score: bool = True,
    show_grid: bool = True,
    ...
    **kwargs: Any,
    ...
    if y_lower_arr.ndim > 2:
        raise ValueError(
            "Inputs y_lower/y_upper must be 1D or 2D."
        )

    # Reshape for consistent processing: (N, O)
    y_lower_proc = (
        y_lower_arr.reshape(-1, 1)
        if y_lower_arr.ndim == 1
        else y_lower_arr
    )
    y_upper_proc = (
        y_upper_arr.reshape(-1, 1)
        if y_upper_arr.ndim == 1
        else y_upper_arr
    )
    ...
    if kind == "widths_histogram":
        ...
        temp_y_lower, temp_y_upper = (
            y_lower_proc,
            y_upper_proc,
        )
        nan_mask_inputs = np.isnan(temp_y_lower) | np.isnan(
            temp_y_upper
        )
        ...
        if np.any(nan_mask_inputs):
            if nan_policy_hist == "raise":
                ...
                nan_mask_inputs = nan_mask_inputs[
                    rows_to_keep
                ]

        individual_widths = (
            temp_y_upper - temp_y_lower
        )  # (N_calc, O)
        ...
        if valid_widths_for_hist.size > 0:
            ...
            if show_score:
                mean_of_plotted_widths = np.mean(
                    valid_widths_for_hist
                )
                score_text = f"Mean Width: {mean_of_plotted_widths:.4f}"
                current_title = (
                    plot_title_str
                    or "Distribution of Interval Widths"
                )
                plot_title_str = f"{current_title}{current_output_label}\n({score_text})"
        else:
            ax.text(
                0.5,
                0.5,
                "No valid interval widths for histogram.",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
        ...
        if miw_values is not None:
            ...
            if verbose > 0:
                print(
                    f"Using pre-computed MIW values: {scores_to_plot}"
                )
        else:
            # For summary bar, respect user's multioutput choice in metric_kws
            summary_bar_default_kws = (
            ...
            if verbose > 0:
                print(
                    f"Computed MIW score(s) for summary: {scores_to_plot}"
                )
        ...
        if scores_arr.size > 0 and not np.all(
            np.isnan(scores_arr)
        ):
...
plot_mean_interval_width.__doc__ = rf"""
...
miw_values : float or ndarray, optional  
    ...
    internal call to
    :func:`geoprior.metrics.mean_interval_width_score`.
metric_kws : dict, optional  
    Extra keyword arguments forwarded to
    :func:`geoprior.metrics.mean_interval_width_score`.
...
See Also
--------
geoprior.metrics.mean_interval_width_score  
...
def plot_prediction_stability(
    y_pred: np.ndarray,
    pss_values: float | np.ndarray | None = None,
    metric_kws: dict[str, Any] | None = None,
    kind: Literal[
        "scores_histogram", "summary_bar"
    ] = "summary_bar",
    output_idx: int | None = None,
    hist_bins: int | Sequence[Real] | str = "auto",
    hist_color: str = "teal",
    hist_edgecolor: str = "black",
    figsize: tuple[float, float] = (10, 6),
    title: str | None = "Prediction Stability Score (PSS)",
    xlabel: str | None = None,
    ylabel: str | None = None,
    bar_color: str | list[str] = "teal",
    bar_width: float = 0.8,
    score_annotation_format: str = "{:.4f}",
    show_score: bool = True,
    show_grid: bool = True,
    ...
    **kwargs: Any,
    ...
    prediction_stability_score = get_metric(
    ...
    if n_samples == 0:
        warnings.warn(
            "Input y_pred is empty. Cannot generate plot.",
            stacklevel=2,
        )
        if ax is None:
            _, ax = plt.subplots(figsize=figsize)
    ...
    if n_timesteps < 2 and kind != "summary_bar":
        ...
        if ax is None:
            _, ax = plt.subplots(figsize=figsize)
        ax.set_title(
            title or f"PSS {kind} (Not Enough Timesteps)"
        )
        if show_grid:
            ax.grid(**(grid_props or {}))
        return ax
    ...
    if kind == "scores_histogram":
        ...
        per_trajectory_scores: np.ndarray | None = None
        if pss_values is not None:
            ...
            if (
                pss_values.ndim == 1
                and n_outputs == 1
                and len(pss_values) == n_samples
            ):
                per_trajectory_scores = pss_values.reshape(
                    -1, 1
                )  # Ensure (N,1)
            elif (
                pss_values.ndim == 2
                and pss_values.shape[0] == n_samples
                and pss_values.shape[1] == n_outputs
            ):
        ...
        if per_trajectory_scores is None:
            ...
            nan_policy_hist = current_metric_kws.get(
                "nan_policy",
                default_kws_for_metric["nan_policy"],
            )
            temp_y_pred = (
                y_pred_proc.copy()
            )  # Use copy for local NaN handling

            nan_mask_sot = np.isnan(temp_y_pred)
            nan_mask_so_hist = nan_mask_sot.any(
                axis=2
            )  # (N,O)
            ...
            if np.any(nan_mask_so_hist):
                if nan_policy_hist == "raise":
                    ...
                    rows_with_nan = nan_mask_so_hist.any(
                        axis=1
                    )  # (N,)
                    rows_to_keep = ~rows_with_nan
                    if not np.any(rows_to_keep):
                        ...
                        ax.set_title(
                            plot_title_str
                            or "PSS Scores (No Data)"
                        )
                        return ax
                    temp_y_pred = temp_y_pred[rows_to_keep]
                    nan_mask_so_hist = nan_mask_so_hist[
                        rows_to_keep
                    ]

            if (
                temp_y_pred.shape[0] == 0
                or temp_y_pred.shape[2] < 2
            ):
                ax.text(
                    0.5,
                    0.5,
                    "Not enough data for PSS histogram.",
                    ha="center",
                    va="center",
                    transform=ax.transAxes,
                )
                if show_grid:
                    ax.grid(**(grid_props or {}))
                ax.set_title(
                    plot_title_str or "PSS Scores (No Data)"
                )
                return ax
        ...
        if n_outputs > 1:
            ...
            scores_to_plot_hist = per_trajectory_scores[
                :, output_idx
            ]
            current_output_label = f" (Output {output_idx})"
        else:  # Single output
            scores_to_plot_hist = (
                per_trajectory_scores.ravel()
            )
        ...
        if valid_scores_for_hist.size > 0:
            ...
            if show_score:
                mean_of_plotted_pss = np.mean(
                    valid_scores_for_hist
                )
                score_text = (
                    f"Mean PSS: {mean_of_plotted_pss:.4f}"
                )
                current_title = (
                    plot_title_str
                    or "Distribution of PSS Values"
                )
                plot_title_str = f"{current_title}{current_output_label}\n({score_text})"
        else:
            ax.text(
                0.5,
                0.5,
                "No valid PSS values for histogram.",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
            current_title = (
                plot_title_str or "Distribution of PSS Values"
            )
            plot_title_str = f"{current_title}{current_output_label} (No Data)"
        ...
        if pss_values is not None:
            ...
            if verbose > 0:
                print(
                    f"Using pre-computed PSS values: {scores_to_plot_bar}"
                )
        else:
            summary_bar_default_kws = (
                default_kws_for_metric.copy()
            ...
            if verbose > 0:
                print(
                    f"Computed PSS for summary: {scores_to_plot_bar}"
                )
        ...
        ax.set_ylabel(
            ylabel or "Prediction Stability Score (PSS)"
        )
...
plot_prediction_stability.__doc__ = rf"""
...
pss_values : float or ndarray, optional  
    ...
    calls to
    :func:`geoprior.metrics.prediction_stability_score`.
metric_kws : dict, optional  
...
See Also
--------
geoprior.metrics.prediction_stability_score  
...
def plot_qce_donut(
    ...
    figsize: tuple[float, float] = (8, 8),
    title: str
    | None = "Quantile Calibration Error Contributions",
    ...
    legend_title: str | None = "Quantiles",
    ...
    **kwargs: Any,  # For future matplotlib extensions
    ...
    y_pred_quantiles_np = df[quantile_cols].to_numpy(
        dtype=float
    )  # (N, Q)
    quantile_levels_np = np.array(
    ...
    if n_samples != len(y_true_np):
        raise ValueError(
            "Length mismatch: actual_col vs quantile_cols."
        )

    # --- 2. Handle NaNs and Sample Weights (from metric_kws) ---
    current_metric_kws = metric_kws or {}
    nan_policy = current_metric_kws.get(
        "nan_policy", "propagate"
    )
    sample_weight = current_metric_kws.get(
        "sample_weight", None
    )
        ...
        if nan_policy == "raise":
            ...
            if s_weights_calc is not None:
                ...
                check_consistent_length(
                    y_true_np, s_weights_calc
                )
                s_weights_calc = s_weights_calc[rows_to_keep]

    if (
        y_t_calc.shape[0] == 0
    ):  # All samples omitted or original empty
        if verbose > 0:
            warnings.warn(
                "No valid samples for QCE calculation.",
                stacklevel=2,
            )
        if ax is None:
            _, ax = plt.subplots(figsize=figsize)
    ...
    indicators = (y_t_calc[:, np.newaxis] <= y_p_calc).astype(
        float
    )
    ...
    if s_weights_calc is not None:
        ...
        if sum_sw < eps:
            warnings.warn(
                f"Sum of sample_weight ({sum_sw}) < eps ({eps}). "
                "Observed proportions may be unstable or NaN.",
                stacklevel=2,
            )
            observed_proportions_q = np.full(
                n_quantiles, np.nan
            )
        else:
        ...
        observed_proportions_q = np.nanmean(
            indicators, axis=0
        )
    ...
    if (
        not np.any(valid_miscal_mask)
        or np.sum(miscalibrations_q[valid_miscal_mask]) < eps
    ):
        ...
        ax.set_title(
            title
            or "QCE Donut (No Miscalibration / Data Issues)"
        )
    ...
    wedges, texts, autotexts = ax.pie(
        ...
        or dict(width=donut_width, edgecolor="w"),
    )

    # Center circle to make it a donut
    center_circle = plt.Circle(
        (0, 0), 1 - donut_width, fc="white"
    )
    ...
    avg_qce = np.nanmean(
        miscalibrations_q
    )  # Mean of non-NaN miscalibrations
    ...
    if show_legend:
        ...
        legend_handles = []
        for _i, w in enumerate(wedges):
            # Ensure color is RGBA for legend patch
            face_color = w.get_facecolor()
            if (
                isinstance(face_color, tuple)
                and len(face_color) == 4
            ):
    ...
    if show_grid:
        if verbose > 0:
            warnings.warn(
                "'show_grid' is True, but grids are not "
...
def _validate_qce_plot_inputs(
    ...
    exist_features(
        df, features=actual_col, error=error_policy
    )

    # Validate quantile_cols type and existence
    if not (
        isinstance(quantile_cols, list)
        and all(isinstance(qc, str) for qc in quantile_cols)
    ):
        raise TypeError(
    ...
    exist_features(
        df, features=quantile_cols, error=error_policy
    )
    ...
    if len(quantile_cols) != len(validated_levels_np):
        raise ValueError(
            f"Length of 'quantile_cols' ({len(quantile_cols)}) must match the length of "
            f"validated 'quantile_levels' ({len(validated_levels_np)})."
        )
...
def plot_radar_scores(
    ...
    y_pred: np.ndarray | None = None,
    metric_functions: MetricFunctionType
    | list[MetricFunctionType]
    | None = None,
    metric_kwargs_list: dict[str, Any]
    | list[dict[str, Any]]
    | None = None,
    normalize_values: bool = False,
    plot_target_type: Literal[
        "metric"
    ] = "metric",  # For future expansion
    # Plotting customizations
    figsize: tuple[float, float] = (8, 8),
    title: str | None = "Metric Scores Radar Plot",
    value_annotations: bool = True,
    annotation_format: str = "{:.2f}",
    fill_radar: bool = True,
    fill_alpha: float = 0.25,
    line_color: str
    | None = None,  # Auto-cycles if multiple lines
    line_width: float = 2,
    marker: str | None = "o",
    # Radial axis customization
    r_min: Real | None = None,
    r_max: Real | None = None,
    r_ticks_count: int = 5,
    # Grid and labels
    show_grid: bool = True,
    grid_props: dict[str, Any] | None = None,
    category_label_props: dict[str, Any] | None = None,
    value_label_props: dict[str, Any] | None = None,
    legend_label: str
    | None = None,  # For when plotting single entity
    ax: plt.Axes | None = None,
    verbose: int = 0,
    **kwargs: Any,  # For future matplotlib extensions
) -> plt.Axes:
    ...
    if data_values is not None:
        if isinstance(data_values, dict):
            ...
            final_values_to_plot = np.array(
                list(data_values.values()), dtype=float
            )
            ...
            if (
                managed_values is None
                or managed_values.size == 0
            ):
                raise ValueError(
                    "If 'data_values' is not a dict, it must not be empty "
                    "after processing."
                )
            final_values_to_plot = managed_values.astype(
                float
            )

            if category_names is not None:
                if len(category_names) < len(
                    final_values_to_plot
                ):
                    if verbose > 0:
                        warnings.warn(
                    ...
                    auto_names = [
                        f"{base_name}{i + len(category_names) + 1}"
                        for i in range(num_missing)
                    ]
                    final_category_names = (
                        category_names + auto_names
                    )
                elif len(category_names) > len(
                    final_values_to_plot
                ):
                    if verbose > 0:
                        warnings.warn(
                ...
                final_category_names = [
                    f"{base_name}{i + 1}"
                    for i in range(len(final_values_to_plot))
                ]
        ...
        if metric_functions is None:
            metric_funcs_to_use: list[MetricFunctionType] = [
                mean_absolute_error,
                lambda yt, yp, **kws: np.sqrt(
                    mean_squared_error(yt, yp, **kws)
                ),  # RMSE
                mean_absolute_percentage_error,
            ]
            default_names = ["MAE", "RMSE", "MAPE"]
            # Ensure default kwargs for RMSE's mean_squared_error if any
            default_metric_kws_list: list[dict] = [
                {},
                {"squared": False},
                {},
            ]
            ...
            if metric_kwargs_list is None:
                ...
                metric_kwargs_list, dict
            ):  # Apply to all defaults
                metric_kws_list_proc = [
                    {**dkw, **metric_kwargs_list}
                    for dkw in default_metric_kws_list
                ]
            elif isinstance(metric_kwargs_list, list) and len(
                metric_kwargs_list
            ) == len(metric_funcs_to_use):
                metric_kws_list_proc = [
                    {**dkw, **ukw}
                    for dkw, ukw in zip(
                        default_metric_kws_list,
                        metric_kwargs_list,
                        strict=False,
                    )  # type: ignore
                ]
            ...
            final_category_names = (
                category_names
                if category_names
                and len(category_names) == len(default_names)
                else default_names
            )
            ...
            metric_kws_list_proc = (
                [metric_kwargs_list or {}]
                if isinstance(metric_kwargs_list, dict)
                or metric_kwargs_list is None
                else metric_kwargs_list
            )  # Should be list of one dict

            if category_names:
                final_category_names = category_names
            else:
                func_name = getattr(
                    metric_functions, "__name__", "metric_1"
                )
            ...
            if category_names and len(category_names) == len(
                metric_funcs_to_use
            ):
                final_category_names = category_names
    ...
    original_values_for_annotation = (
        final_values_to_plot.copy()
    )
    ...
    if np.all(nan_mask):  # All values are NaN
        warnings.warn(
            "All values for radar plot are NaN. Plot will be empty.",
            stacklevel=2,
        )
    ...
    plot_values_for_structure = np.nan_to_num(
        plot_values, nan=0.0
    )
    ...
    if normalize_values:
        ...
        if valid_plot_vals.size > 0:
            ...
            if (
                ...
                temp_scaled_values = np.full_like(
                    plot_values, np.nan
                )
                ...
                if verbose > 0:
                    print(
                        f"Values normalized. Original range: [{min_val:.2f}, {max_val:.2f}]."
                    )
            ...
            if verbose > 0:
                warnings.warn(
                    "All values are NaN, cannot normalize.",
                    stacklevel=2,
                )
    ...
    if ax is None:
        fig, ax = plt.subplots(
            figsize=figsize, subplot_kw=dict(polar=True)
        )  # type: ignore
    elif not hasattr(
        ax, "plot"
    ...
    plot_data_closed = np.concatenate(
        (
            plot_values_for_structure,
            [plot_values_for_structure[0]],
        )
    )
    ...
    original_values_closed = np.concatenate(  # noqa
        (
            original_values_for_annotation,
            [original_values_for_annotation[0]],
        )
    )
    nan_mask_closed = np.concatenate(
        (nan_mask, [nan_mask[0]])
    )
    ...
    segments = np.ma.masked_where(  # noqa
        nan_mask_closed, plot_data_closed
    ).tolist()
    ...
    ax.set_xticklabels(
        final_category_names, **category_label_final_props
    )
    ...
    r_tick_locs = locator.tick_values(
        current_r_lim[0], current_r_lim[1]
    )
    ...
    if (
        (r_min is None or r_min <= 0)
        and 0 not in r_tick_locs
        and current_r_lim[0] <= 0
    ):
        r_tick_locs = np.unique(
            np.sort(np.concatenate(([0], r_tick_locs)))
        )
    ...
    ax.set_yticklabels(
        [
            annotation_format.format(tick)
            for tick in r_tick_locs
        ],
        **value_label_final_props,
    )
    ...
    if value_annotations:
        for _i, (angle, val_plot, val_orig) in enumerate(
            zip(
                angles,
                plot_values,
                original_values_for_annotation,
            ...
            if not np.isnan(val_plot) and not np.isnan(
                val_orig
            ):
                # Use original value for annotation text
                annotation_text = annotation_format.format(
                    val_orig
                )
                ax.text(
                    angle,
                    ...
                    annotation_text,
                    ha="center",
                    va="bottom" if val_plot >= 0 else "top",
                    fontsize=category_label_final_props.get(
                        "size", "small"
                    ),  # type: ignore
                    bbox=dict(
                        facecolor="white",
    ...
    if title:
        ax.set_title(
            ...
            va="bottom",
            fontdict={
                "fontsize": plt.rcParams["axes.titlesize"],  # type: ignore
                "fontweight": plt.rcParams[
                    "axes.titleweight"
                ],
            },
        )  # type: ignore
...
def _calculate_qce_miscalibrations(
    ...
    sample_weight_period: np.ndarray | None,  # (N_period,)
    eps: float,
) -> np.ndarray:
    ...
    if sample_weight_period is not None:
        ...
        for q_idx in range(quantile_levels.shape[0]):
            ...
            finite_mask_q = ~np.isnan(valid_inds_q)
            if np.any(finite_mask_q):
                current_weights = sample_weight_period[
                    finite_mask_q
                ]
...
def plot_nested_quantiles(
    ...
    quantile_cols: list[str],
    quantile_levels: list[Real],
    actual_col: str | None = None,  # Now optional
    dt_col: str | None = None,
    periods: list[Any] | None = None,  # Renamed
    metric_func: MetricFunctionType | None = None,
    metric_kws: dict[str, Any] | None = None,
    # Plotting customizations
    figsize: tuple[float, float] = (10, 10),
    title: str
    | None = None,  # Default title set based on mode
    colors: list[str] | None = None,
    ...
    startangle: float = 90,
    counterclock: bool = False,
    wedgeprops: dict[str, Any]
    | None = None,  # User can set gap here
    donut_width: float = 0.3,
    donut_ring_spacing: float = 0.05,
    donut_base_radius: float = 0.3,
    segment_explode: float | list[float] | None = None,
    # Legend
    ...
    **kwargs: Any,
    ...
    plot_mode: Literal[
        "qce", "custom_metric", "avg_quantiles"
    ]
    if actual_col is None and metric_func is None:
        ...
        func_name = getattr(
            metric_func, "__name__", "Custom Metric"
        )
    ...
    exist_features(
        df, features=cols_to_check_existence, error="raise"
    )
    ...
    try:
        ...
        if (
            plot_mode != "avg_quantiles"
        ):  # Avg quantiles don't impose this
            if not np.all(
                (q_levels_np > 0) & (q_levels_np < 1)
            ):
                raise ValueError(
    ...
    sample_weight_col = metric_kws.get(
        "sample_weight_col", None
    )
    if sample_weight_col:
        exist_features(
            df, features=sample_weight_col, error="raise"
        )
    ...
    if num_periods == 0:
        warnings.warn("No periods to plot.", stacklevel=2)
        if ax is None:
            _, ax = plt.subplots(figsize=figsize)
    ...
    for period_name in periods_val:
        period_df_slice = (
            df
            if dt_col is None
            else df[df[dt_col] == period_name]
        )
        if period_df_slice.empty:
            period_values_dict[period_name] = np.full(
                len(q_levels_np), np.nan
            )
            if verbose > 0:
                warnings.warn(
        ...
        y_p_q_period = period_df_slice[
            quantile_cols
        ].to_numpy(dtype=float)
        s_weights_period = (
            period_df_slice[sample_weight_col].to_numpy(
                dtype=float
            )
            if sample_weight_col
            else None
        ...
        nan_policy_metric = metric_kws.get(
            "nan_policy", "propagate"
        )
        ...
        if plot_mode == "avg_quantiles":
            ...
            nan_mask_preds_period = np.isnan(
                y_p_q_period
            )  # (N_period, Q)
            if np.any(nan_mask_preds_period):
                if nan_policy_metric == "raise":
                    ...
                    rows_with_nan_in_preds = (
                        nan_mask_preds_period.any(axis=1)
                    )
                    keep_rows = ~rows_with_nan_in_preds
                    if not np.any(keep_rows):
                        current_period_values = np.full(
                            len(q_levels_np), np.nan
                        )
                    else:
                        y_p_q_period_clean = y_p_q_period[
                            keep_rows
                        ]
                        s_weights_period_clean = (
                            s_weights_period[keep_rows]
                            if s_weights_period is not None
                            else None
                        )
                        if (
                            s_weights_period_clean is not None
                            and np.sum(s_weights_period_clean)
                            < eps
                        ):
                            current_period_values = np.full(
                                len(q_levels_np), np.nan
                    ...
                    current_period_values = np.nanmean(
                        y_p_q_period, axis=0
                    )
            else:  # No NaNs in y_p_q_period
                if (
                    s_weights_period is not None
                    and np.sum(s_weights_period) < eps
                ):
                    current_period_values = np.full(
                        len(q_levels_np), np.nan
                    )
                    ...
                    current_period_values = np.average(
                        y_p_q_period,
                        axis=0,
                        weights=s_weights_period,
                    )
        else:  # 'qce' or 'custom_metric'
            y_t_period = period_df_slice[actual_col].to_numpy(
                dtype=float
            )  # type: ignore
            nan_mask_true_period = np.isnan(y_t_period)
            nan_mask_preds_period_any = np.isnan(
                y_p_q_period
            ).any(axis=1)
            combined_nan_samples = (
                nan_mask_true_period
                | nan_mask_preds_period_any
            )
        ...
        period_values_dict[period_name] = (
            current_period_values
        )
    ...
    if colors is None:
        ...
        warnings.warn(
            "Not enough colors for quantiles. Colors will cycle.",
            stacklevel=2,
        )
        plot_colors = [
            colors[i % len(colors)]
    ...
    elif isinstance(segment_explode, list) and len(
        segment_explode
    ) == len(q_levels_np):
        explodes = segment_explode
    else:
        warnings.warn(
            "Invalid 'segment_explode'. Using no explosion.",
            stacklevel=2,
        )
        explodes = [0] * len(q_levels_np)
    ...
    for i, period_name in enumerate(
        periods_val
    ):  # Use validated periods_val
        ...
        if not np.any(valid_scores_mask):
            ...
            current_outer_radius -= (
                donut_width + donut_ring_spacing
            )
            continue

        plot_data_period = values_for_period[
            valid_scores_mask
        ]
        # Use original quantile_cols for names if plot_mode is 'avg_quantiles'
        # and lengths match, otherwise generate from q_levels_np
        if plot_mode == "avg_quantiles" and len(
            quantile_cols
        ) == len(q_levels_np):
            period_segment_names_all = quantile_cols
        else:
            period_segment_names_all = [
                f"q={q:.2f}" for q in q_levels_np
            ]

        period_segment_names = [  # noqa
            period_segment_names_all[j]
            for j, keep in enumerate(valid_scores_mask)
            if keep
        ]
        period_colors = [
            plot_colors[j]
        ...
        if sum_plot_data < eps and not np.all(
            plot_data_period < eps
        ):
            if verbose > 0:
                warnings.warn(
        ...
        if (
            np.all(plot_data_period <= eps)
            and plot_data_period.size > 0
        ):
            if verbose > 0:
                warnings.warn(
            ...
            ring = plt.Circle(
                (0, 0),
                current_outer_radius - donut_width / 2,
                width=donut_width,
                color="lightgray",
                fill=False,
                linestyle="--",
            ...
            angle_for_label = startangle + (
                counterclock * 2 - 1
            ) * (i * 360 / num_periods)  # Distribute labels
            text_x = (
                current_outer_radius - donut_width / 2
            ...
            wedges, *texts_autotexts = ax.pie(
                ...
                labels=None,
                autopct=(
                    lambda pct: segment_label_format.format(
                        name="",
                        value=(pct / 100.0) * sum_plot_data
                        if sum_plot_data > eps
                        else 0,
                        percent=pct,
                    )
                )
                if show_segment_labels and sum_plot_data > eps
                else None,
                startangle=startangle,
                counterclock=counterclock,
        ...
        if show_center_text and plot_data_period.size > 0:
            ...
            if not np.isnan(avg_value_period):
                if i == num_periods - 1:
                    center_text_val = (
                        center_text_format.format(
                            value=avg_value_period
                        )
                    )
                    if (
                        plot_mode == "avg_quantiles"
                        and dt_col is None
                    ):  # Single overall donut
                        center_text_val = (
                            "Avg. Quantile\nValues"
                        )
                    ...
                    ax.text(
                        ...
                        fontsize="medium",
                        fontweight="bold",
                        path_effects=kwargs.get(
                            "center_text_path_effects", None
                        ),
                    )
    ...
    if show_overall_legend and legend_items_overall:
        ...
        if unique_legend_items:
            handles, labels = zip(
                *unique_legend_items, strict=False
            )
            ax.legend(
                handles,
                labels,
    ...
    if show_grid:
        ...
        ax.grid(
            **(grid_props or {"linestyle": ":", "alpha": 0.3})
        )
    else:
        ax.grid(False)
