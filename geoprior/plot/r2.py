...
from ..utils.validator import process_y_pairs, validate_yy
...
def plot_r2_in(
    ...
    **r2_score_kws : dict
    ...
    nrows = (
        int(np.ceil(n_pairs / ncols)) if n_pairs > 0 else 1
    )
    ...
    if line_colors is None:
        ...
        line_colors = is_iterable(
            line_colors, exclude_string=True, transform=True
        )
    ...
    if line_styles is None:
        ...
        line_styles = is_iterable(
            line_styles, exclude_string=True, transform=True
        )
    ...
    for idx in range(n_pairs):
        ...
        try:
            sub_title = (
                titles[idx] if titles else f"Pair {idx + 1}"
            )
