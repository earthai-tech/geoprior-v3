...
#   - DEVICE=auto (default): use GPU if detected, else CPU
#   - DEVICE=gpu           : require GPU
#   - DEVICE=cpu           : force CPU
#
# Speed knobs (all overridable by env or flags):
#   EPOCHS (default 40)
#   BATCH  (default 64)
#   PATIENCE (default 5)      [requires --patience support in python]
#   FAST=1 (default 1)        disables heavy per-epoch freeze callbacks
#
# Regime selection:
#   - default: all regimes
#   - select by name:     --regime base --regime anchored
#   - select by csv list: --regimes base,anchored
#   - select by ids:      --regime-ids 2,3   (1..5)
#
# Examples:
#   bash run_sm3_regimes_tau_50_auto.sh
#   bash run_sm3_regimes_tau_50_auto.sh --regime base
#   bash run_sm3_regimes_tau_50_auto.sh --regimes base,anchored
#   bash run_sm3_regimes_tau_50_auto.sh --regime-ids 2,4
#   EPOCHS=80 BATCH=16 PATIENCE=10 FAST=0 bash run_sm3_regimes_tau_50_auto.sh
#   DEVICE=cpu NTHREADS=12 bash run_sm3_regimes_tau_50_auto.sh

usage() {
  cat <<'USAGE'
Usage: run_sm3_regimes_tau_50_auto.sh [options]
...
CPU threading env:
  NTHREADS (default: nproc)
  TF_NUM_INTRAOP_THREADS (default: NTHREADS)
  TF_NUM_INTEROP_THREADS (default: 2)
USAGE
}
...
if [[ "$DEVICE" == "gpu" && "$has_gpu" -ne 1 ]]; then
  echo "[ERROR] DEVICE=gpu requested, but no NVIDIA GPU detected (nvidia-smi missing or returns none)." >&2
  exit 3
fi
...
if [[ "$DEVICE" == "gpu" ]]; then
  ...
  echo "[SM3] Using CPU"
  export CUDA_VISIBLE_DEVICES=""  # make TF ignore GPUs if present
...
for reg in "${RUN_REGIMES[@]}"; do
  OUTDIR="${SUITE_ROOT}/sm3_tau_${reg}_${NREAL}"
